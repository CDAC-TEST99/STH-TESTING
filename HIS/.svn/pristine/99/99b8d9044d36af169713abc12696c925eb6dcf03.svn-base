 ...
 */
	...
	/**
	 * alphaNumSpecialChar: Validates alphabets, numbers and special characters only
	 * USAGE: alhanumSpecialChar
	 */
	$.extend($.fn.validatebox.defaults.rules, {
		alphaNumSpecialChar:{
			validator:function(value){
				var r=/^[a-zA-Z0-9\,\.\;\s\&\-_!~`’@#$%^&*()+={}:";'<>?\/]+$/i;
				return value.match(r);
			},
			message:'Enter Aplhabets, Numbers with Special Characters Only.'
		}
	});
