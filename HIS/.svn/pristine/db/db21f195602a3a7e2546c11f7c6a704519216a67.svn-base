...
imgArray  = [
               ['save_tab.gif' ,'Save.gif' ],   //Array of Images Used for Save Event
               
               ['clear_tab.gif' ],   //Array of Images Used for Clear Event 
               
               ['cancel_tab.gif'],  //Array of Images Used for Cancel Event
               
               ['plus.gif'],//Array of Images Used for plus Event
            ];


keyCodeArray = new Array("45","46","35","107"); 
...
* Version  Copyright (C) Paul Johnston 1999 - 2002.
...
var createFHash = function(frmName){
	var form = $my_jQuery("form[name='"+frmName+"']");
	var datastring = $my_jQuery("form[name='"+frmName+"']").serializeArray();
	
	//alert("datastring"+datastring);
	$my_jQuery.each(form.find('input[type="file"]'), function(i, tag) {
	      $.each($(tag)[0].files, function(i, file) {
	    	  	var tObj = new Object();
	    	  //	alert("tag.name"+tag.name);
	    	  	//alert("file.name"+file.name);
				tObj.name = tag.name; 
				tObj.value = file.name;		
	     	    datastring.push(tObj);
	      });
	});
...
var createFHashAjaxQuery = function(str){
	//alert("str="+str)
	if(str.indexOf('?') != -1) {
		if(str.indexOf('requestMethod') == -1)
			str = str + '&requestMethod=GET';
	} else {
		if(str.indexOf('requestMethod') == -1)
			str = str + '?requestMethod=GET';
	}	
	...
	(function(){
		...
		//alert("inisde original submit ");
		  ...
		  document.forms[0].submit = function(){
		   ...
		   //	 alert("intercepted submit after"); 
	  ...
	  if (f) {
	    ...
	    r.onload = function(e) { 
	       ...
	       //  alert("contents >> "+contents);
	              ...
	              //alert("id"+id);
	        // alert(document.getElementById("UploadFile_hash_file_"+id));
	         
	         //alert(document.getElementByName("hash_file_"+id));
	    	 
	    	 var  encodedFileContent = hex_md5(contents);
	    	 
	    //	 console.log("encodedFileContent >> "+encodedFileContent);
	    	 
	    	//alert("before"+document.getElementById("UploadFile_hash_file_"+id).value);
	     
	    	 document.getElementById("UploadFile_hash_file_"+id).value = encodedFileContent;
	    	// console.log("after"+document.getElementById("UploadFile_hash_file_"+id).value);
