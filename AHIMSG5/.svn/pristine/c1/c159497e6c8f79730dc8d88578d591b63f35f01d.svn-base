	  ...
	  const d = new Date();
	    ...
	    $('#search').keypress(function(e) {
	        ...
	        if(e.which == 13) {
	        	 ...
	        	 $('#divResetFilter').show();
	        	 $('#tbl_Adt_List').show();
	            $('#tbl_Adt_List tbody').empty();	            
	        	getAdtListPageData("1");	        	
	        }
	    });
	 ...
	 $.each(modarr,function(indx, objModuleWiseLeafMenu){
		 if(objModuleWiseLeafMenu["processType"].toUpperCase()=="SERVICES" && objModuleWiseLeafMenu["parentMenuId"].toUpperCase()=="0" ){
			 ...
			 if(flagMenuWithoutPK){
				 buttonRowhtml+="<button class='button btnLeafMenu'  data-menuid='"+objModuleWiseLeafMenu["menuid"]+"'  data-pk='' title='"+objModuleWiseLeafMenu["menuName"]+"' data-url='"+objModuleWiseLeafMenu["url"]+"'>"+
...
function getAdtListPageData(pageNo){
	...
	var filterDate=$('#filterDate').val(); 
	 ...
	 var configJson={
   			serviceName:"/getData/getAdtlistpagedata",			    					
   			callBackFunctionName:"callGetAdtListPageData",
   			"inputData":data
   		};
...
function callGetAdtListPageData(configJson, dataJson){
	 //alert(JSON.stringify(dataJson));
	 
	
	 $('#tbl_Adt_List thead').empty();
	 if(dataJson["data"].length>0){
		...
		//alert("totalRecord===" + totalRecord + " recordPerPage=="+ recordPerPage +" noOfPages===" +noOfPages );
		 ...
		 html+="</tr>";
		 $('#tbl_Adt_List thead').append(html)
		 ...
		 $.each(dataJson["data"], function(indx, rowJson){
			 ...
			 $('#tbl_Adt_List tbody').append(html)
...
function getAdtkpidata(){
	
	var filterDate=$('#filterDate').val(); 
	var data={"moduleId":$('#selectedModuleId').val().toUpperCase(),"filterDate":filterDate};
	 
	 var configJson={
   			serviceName:"/getData/getAdtkpidata",			    					
   			callBackFunctionName:"callGetAdtkpidata",
   			"inputData":data
   		};
		
		callFormFlowXService(configJson);
} 
function callGetAdtkpidata(configJson, dataJson){
	// alert(JSON.stringify(dataJson));
