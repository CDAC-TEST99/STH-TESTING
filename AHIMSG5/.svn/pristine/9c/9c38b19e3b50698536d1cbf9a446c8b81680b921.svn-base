	  ...
	  const d = new Date();
	    ...
	    $('#search').keypress(function(e) {
	        ...
	        if(e.which == 13) {
	            ...
	            $('#tbl_Inventory_List tbody').empty();	            
	        	getInventoryListPageData("1");	        	
	        }
	    });
...
function getInventoryListPageData(pageNo){
	...
	var filterDate=$('#filterDate').val(); 
	 ...
	 var configJson={
   			serviceName:"/getData/getInventorylistpagedata",			    					
   			callBackFunctionName:"callGetInventoryListPageData",
   			"inputData":data
   		};
...
function callGetInventoryListPageData(configJson, dataJson){
	 //alert(JSON.stringify(dataJson));
	 
	
	 $('#tbl_Inventory_List thead').empty();
	 if(dataJson["data"].length>0){
		...
		//alert("totalRecord===" + totalRecord + " recordPerPage=="+ recordPerPage +" noOfPages===" +noOfPages );
		 ...
		 html+="</tr>";
		 $('#tbl_Inventory_List thead').append(html)
		 ...
		 $.each(dataJson["data"], function(indx, rowJson){
			 ...
			 $('#tbl_Inventory_List tbody').append(html)
...
function getInventorykpidata(){
	
	var filterDate=$('#filterDate').val(); 
	var data={"moduleId":$('#selectedModuleId').val().toUpperCase(),"filterDate":filterDate};
	 
	 var configJson={
   			serviceName:"/getData/getInventorykpidata",			    					
   			callBackFunctionName:"callGetInventorykpidata",
   			"inputData":data
   		};
		
		callFormFlowXService(configJson);
} 
function callGetInventorykpidata(configJson, dataJson){
	// alert(JSON.stringify(dataJson));
...
function getPageWiseData(){
	...
	getInventoryListPageData( nextPageNo);
}
