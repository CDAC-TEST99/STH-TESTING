<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>

<head>
 
<link href="/HIS/hisglobal/cdac_awesome/bootstap/css/bootstrap.min.css" rel="stylesheet">
<link href="/HIS/hisglobal/cdac_awesome/css/style.css" rel="stylesheet">
<link rel="stylesheet" href="/HIS/hisglobal/cdac_awesome/fonts/fontawesome/css/all.css">

</head>

<body style="background: #fff">
<div class="container mt-5">
<div class='row mt-5'>
	<div class='col-md-12  text-center'>
		<h1 class='text-primary'>Stay Tuned: Enhancements in Progress</h1>
    <p>Our team is hard at work implementing new features, improving functionality, and enhancing the overall user experience. While these enhancements are taking place, you may notice some changes and updates occurring across the site.</p>
    <p>Please check back soon!</p>
	 </div>
	<div class='col-md-12  text-center'><img src='/HIS/hisglobal/images/gears.gif' style='width:200px;'></div>

</div>
</div>
 
</body>
</html>