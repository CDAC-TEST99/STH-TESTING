...
public class HISSSOClientRequestFilter implements Filter
{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		...
		try
		{
			...
			if (strURI.endsWith(".html")|| strURI.contains(HISSSOClientConfig.SSO_ST_SERVICE_URL)||strURI.contains("labLink") ||strURI.contains("snomedct") )
			{
				// Forward as-it-is
			...
			else
			{
				...
				if (authenticate)
				{
					...
					if (objHISServiceTicket.getAuthorization().isAuthorizedURL(strURI))
					{
						...
						if(strURI.equalsIgnoreCase("/HISRegistration/registration/transactions/attachUploadFile"))
							//chain.doFilter(new RequestWrapper((HttpServletRequest) request), response);
							chain.doFilter(request, response);
						else
							chain.doFilter(request, response);
			...
			/**Added by Vasu on 17.May.18 for handling private browser vulnerability*/
	...
	private boolean setCredentialsInSession(HttpSession objSession_p, HISServiceTicket objST_p)
	{
		...
		//	objSession_p.setAttribute("HOLIDAY_DATES", _holDates);
			...
			objSession_p.setAttribute("HOSPITAL_NAME", voGlobalHospital.getHospitalName());
	...
	private void populateData(Object objTarget, Object objSource)
	{
		...
		//	objTar.setUserBoundDefaultGender(objSrc.getVarUserBoundDefaultGender());
		}
		else if (objSource instanceof HospitalMasterVO && objTarget instanceof HospitalMstVO)
		{
			...
			objTar.setIsLogoRequired(objSrc.getVarIsLogoRequired());
			objTar.setLogoType(objSrc.getVarLogoType());
			objTar.setStrLogoFileName(objSrc.getVarStrLogoFileName());
			...
			System.out.println("ahims >> logo name >>>>>>>>>>>>>>>>>>  "+objSrc.getVarStrLogoFileName());
