...
public class SecureHashAlgorithm
{
	...
	public static String generateRandom(HttpServletRequest request) throws Exception
	{
		SecureRandom sr = SecureRandom.getInstance("SHA1PRNG");
		
		String secureRandomNo = SHA1(String.valueOf(Math.abs(sr.nextInt())));
		
		request.getSession().setAttribute("x-auth-token", secureRandomNo);
		
		return secureRandomNo;
	}
