	  ...
	  const d = new Date();
	    ...
	    $('#search').keypress(function(e) {
	        ...
	        if(e.which == 13) {
	           ...
	           // alert("calling getpharmacylistpagedata on keypress");
	        	getpharmacylistpagedata("1");	        	
	        }
	    });
...
function getpharmacylistpagedata(pageNo){
	//alert("here");
	...
	var filterDate=$('#filterDate').val(); 
	 ...
	 var configJson={
   			serviceName:"/getData/getpharmacylistpagedata",			    					
   			callBackFunctionName:"callGetpharmacylistpagedata",
   			"inputData":data
   		};
...
function callGetpharmacylistpagedata(configJson, dataJson){
...
function getpharmacykpidata(){
	
	var filterDate=$('#filterDate').val(); 
	var data={"moduleId":$('#selectedModuleId').val().toUpperCase(),"filterDate":filterDate};
	 
	 var configJson={
   			serviceName:"/getData/getpharmacykpidata",			    					
   			callBackFunctionName:"callGetpharmacykpidata",
   			"inputData":data
   		};
		
		callFormFlowXService(configJson);
} 
function callgetpharmacykpidata(configJson, dataJson){
	// alert(JSON.stringify(dataJson));
...
function callbackGetCurrentDate(configJson, dataJson){
	if(dataJson["data"]!=undefined &&  dataJson["data"].length>0){
		...
		//alert("calling callbackGetCurrentDate");
		getpharmacykpidata();
		getpharmacylistpagedata("1");
...
function getPageWiseData(){
	...
	//alert("calling getPageWiseData");
	getpharmacylistpagedata( nextPageNo);
}
