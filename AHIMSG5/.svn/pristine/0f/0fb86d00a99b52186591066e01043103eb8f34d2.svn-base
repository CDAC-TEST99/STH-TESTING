...
function initAppointmentListPage(){
	  ...
	  const d = new Date();
	    ...
	    $('#search').keypress(function(e) {
	        ...
	        if(e.which == 13) {
	        	 ...
	        	 $('#divrowkpis').hide();
	        	 $('#divResetFilter').show();
	        	 $('#tbl_appointment_List').show();
	            $('#tbl_appointment_List tbody').empty();	            
	        	getAppointmentListPageData("1");	        	
	        }
	    });
...
function getAppointmentListPageData(pageNo){
	...
	var filterDate=$('#filterDate').val(); 
	 ...
	 var configJson={
   			serviceName:"/getData/getappointmentlistpagedata",			    					
   			callBackFunctionName:"callGetAppointmentListPageData",
   			"inputData":data
   		};
	 $('#preloader').show();
	 callFormFlowXService(configJson);
...
function callGetAppointmentListPageData(configJson, dataJson){
	 //alert(JSON.stringify(dataJson));
	hidePreloader();
	 var buttonHtmlPKBased= $('#buttonHtmlPKBased').html();
	 $('#tbl_appointment_List thead').empty();
	 if(dataJson["data"].length>0){
		...
		//alert("totalRecord===" + totalRecord + " recordPerPage=="+ recordPerPage +" noOfPages===" +noOfPages );
		 ...
		 html+="</tr>";
		 $('#tbl_appointment_List thead').append(html)
		 ...
		 $.each(dataJson["data"], function(indx, rowJson){
			...
			// alert("status_to_show_menu==" + status_to_show_menu);
			 var buttonHtmlPKBased =generateRecordWiseButton(status_to_show_menu);
			 //alert(buttonHtmlPKBased);
			...
			// alert(mybutton);
			 ...
			 $('#tbl_appointment_List tbody').append(html)
			 
		 });
		 
		 
		// $('[id^=btnaction_]').click(showActions);
	
		 //$('.btnLeafMenu').function(openURLFromListPage)
...
function getappointmentkpidata(){
	
	var filterDate=$('#filterDate').val(); 
	var data={"moduleId":$('#selectedModuleId').val().toUpperCase(),"filterDate":filterDate};
	 
	 var configJson={
   			serviceName:"/getData/getappointmentkpidata",			    					
   			callBackFunctionName:"callGetappointmentkpidata",
   			"inputData":data
   		};
		
		callFormFlowXService(configJson);
} 
function callGetappointmentkpidata(configJson, dataJson){
	// alert(JSON.stringify(dataJson));
...
function getPageWiseData(){
	...
	getAppointmentListPageData( nextPageNo);
}
