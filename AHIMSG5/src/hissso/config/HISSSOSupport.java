...
public abstract class HISSSOSupport
{
	...
	public static void onSSOTGTSessionExpiration(HttpSession objSession)
	{
		try
		{
			...
			//System.out.println("Call on Session SSO TGT Expiration  >>>>>>>>>>>>>>>>> +++++++++++++++ ");
			
			String strTicketId = (String) objSession.getAttribute(HISSSOConfig.LOGGEDIN_USER_GRANTING_TICKET_ID);
			
			//System.out.println("Call on Session SSO TGT Expiration  >>>>>>>>>>>>>>>>> +++++++++++++++ "+strTicketId);
			...
			if (strTicketId != null)
			{
				...
				//TicketGrantingTicket objTicketGrantingTicket = (TicketGrantingTicket) objRegistry.getTicket(strTicketId,objSession.getAttribute("User-Agent").toString());
				if (objTicketGrantingTicket != null)
				{
					...
					//{
						...
						System.out.println("HIS-Login-Log::Logout Date & Time on Session Expire-"+voUserLog.getVarUserLoginDate());
	...
	public static void onSSOLogout(HttpSession objSession, HttpServletRequest objRequest)
	{
		try
		{
			String strTicketId = (String) objSession.getAttribute(HISSSOConfig.LOGGEDIN_USER_GRANTING_TICKET_ID);
			if (strTicketId != null)
			{
				HISTicketRegistry objRegistry = HISSSOSupport.getSSORegister(objSession);
				TicketGrantingTicket objTicketGrantingTicket = (TicketGrantingTicket) objRegistry.getTicket(strTicketId);
				//TicketGrantingTicket objTicketGrantingTicket = (TicketGrantingTicket) objRegistry.getTicket(strTicketId , objRequest.getHeader("User-Agent").toLowerCase());
				if (objTicketGrantingTicket != null)
				{
		...
		try
		{
			...
			if (objSession != null)
			{
				...
				else
				{
					...
					//TicketGrantingTicket objGrantingTicket = (TicketGrantingTicket) registry.getTicket(strGrantingTicketId , objRequest.getHeader("User-Agent").toLowerCase());
