...
public class Procedure
{
	...
	public Object execute(Connection _conn) throws HISException
	{
		...
		try
		{
			 ...
			 * (DataSource) ic.lookup("java:comp/env/AHIS"); Connection conn =
			 * ds.getConnection();
			 */
			// create Statement
	...
	{
		// int increment = this.returns?1:0;//if returning increment the idx
		// with one
		try
		{
			...
			// System.out.println("_param.getClass(): " +
			// _param.getValue().getClass());
			...
			switch (_param.type)
			{
				...
				case (Types.VARBINARY):
					...
					/*
					 * this.callStmt.setByte(this.getIdx(_param),
					 * ((Byte)_param.getValue()).byteValue());
					 */
				case (Types.DATE):
	...
	private class Parameter implements Comparable
	{
		...
		public boolean equals(Object obj)
		{
			...
			{ // null Condition & Symmetric
				...
				if (this.idx == param.idx) { return true; }
			}
			return false;
		}
