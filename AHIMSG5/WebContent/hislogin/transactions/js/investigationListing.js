...
function initInvestigationListPage(){
	  ...
	  const d = new Date();
	    ...
	    $('#search').keypress(function(e) {
	        ...
	        if(e.which == 13) {
	        	 ...
	        	 $('#divrowkpis').hide();
	        	 $('#divResetFilter').show();
	        	 $('#tbl_Investigation_List').show();
	            $('#tbl_Investigation_List tbody').empty();	            
	        	getInvestigationListPageData("1");	        	
	        }
	    });
...
function getInvestigationListPageData(pageNo){
	...
	var filterDate=$('#filterDate').val(); 
	 ...
	 var configJson={
   			...
   			callBackFunctionName:"callGetInvestigationListPageData",
   			"inputData":data
   		};
...
function callGetInvestigationListPageData(configJson, dataJson){
	 //alert(JSON.stringify(dataJson));
	 
	
	 $('#tbl_Investigation_List thead').empty();
	 if(dataJson["data"].length>0){
		...
		//alert("totalRecord===" + totalRecord + " recordPerPage=="+ recordPerPage +" noOfPages===" +noOfPages );
		 ...
		 html+="</tr>";
		 $('#tbl_Investigation_List thead').append(html)
		 ...
		 $.each(dataJson["data"], function(indx, rowJson){
			 ...
			 $('#tbl_Investigation_List tbody').append(html)
...
function getInvestigationkpidata(){
	
	var filterDate=$('#filterDate').val(); 
	var data={"moduleId":$('#selectedModuleId').val().toUpperCase(),"filterDate":filterDate};
	 
	 var configJson={
   			serviceName:"/getData/getInvestigationkpidata",			    					
   			callBackFunctionName:"callGetInvestigationkpidata",
   			"inputData":data
   		};
		
		callFormFlowXService(configJson);
} 
function callGetInvestigationkpidata(configJson, dataJson){
	// alert(JSON.stringify(dataJson));
...
function getPageWiseData(){
	...
	getInvestigationListPageData( nextPageNo);
}
