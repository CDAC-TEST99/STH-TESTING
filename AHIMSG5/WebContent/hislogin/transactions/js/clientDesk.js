$(document).ready(function() {
				 ...
				 $( "button" )
			      ...
			      $('#search').keyup(function(){
			    	  ...
			    	  $('[id^=mainContainer_]').hide();
			    	  $('[id^=mainContainerSearch]').show();
			    	  
			    	  $('#rowSearch').empty();
			    	  
			    	  
			    	  $('.leaf').each(function(){
			    		var menuName=$(this).attr("data-menuName").toLowerCase();
			    		if (menuName.includes(searchTerm)) {
			    			var html='<div class="col-lg-2 col-md-4 col-sm-4 col-4" >';
			    			html+=$(this).html();
			    			html+='</div>';
			    			$('#rowSearch').append(html);
			    		}
			    	  })
			      })
...
function callCheckForMenuAsDeskListpage(configJson, dataJson){
	...
	initModuleHTML('Services');
	initModuleHTML('Setup');
	initModuleHTML('Reports');
...
function menuBoxClick(objMenu){
	$('#btnShowVidTutorial').hide();
	$('#btnShowManual').hide();
	//alert(JSON.stringify(objMenu));
     ...
     if(target!=undefined){
        ...
        if (drillDown.length > 0) { // Check if the drill-down container exists
    	 ...
    	 if( $(objMenu).attr("data-isdeskmenu")==0){
	    	 ...
	    	 var vidTutorialUrl=menujsonObj["vidTutorialUrl"]
	    	 var docFileName=menujsonObj["docFileName"]
	    	 var fileURL = "/CGHSGrievance/FormFlowXACTION?hmode=ftpFileDownload&fileName="+docFileName+"&folderName=MenuDocument&isGlobal=1";
	    	 
	    	 //alert("vidTutorialUrl : "+  menujsonObj["vidTutorialUrl"]  +"docFileName : " +  menujsonObj["docFileName"] )
	    	 ...
	    	 if(vidTutorialUrl!=undefined && vidTutorialUrl!=""){
	    		 $('#btnShowVidTutorial').show();
	    		 $('#btnShowVidTutorial').empty().append(
	    		            `<a href="${vidTutorialUrl}" target="_blank" class="video-link" style="text-decoration: none; margin-left: 8px;font-size: larger;"><i class="fa-solid fa-video" style="color: #ffc102;font-size: large;"></i>&nbsp;Watch Tutorial</a>`
	    		        );
	    		 showHideMenuPage(0);
	    		 loadInIframe("1", url);
	    	 }
	    	 if(docFileName!=undefined && docFileName!=""){
	    		 $('#btnShowManual').show();
	    		 $('#btnShowManual').empty().append(
	    		            `<a href="${fileURL}" target="_blank" class="video-link" style="text-decoration: none; margin-left: 8px;font-size: larger;"><i class="fa-solid fa-file" style="color: #ffc102;font-size: large;">&nbsp;</i>User Manual</a>`
	    		        );
	    		 showHideMenuPage(0);
	    		 loadInIframe("1", url);
	    	 }
...
function createModuleMenu(){
	...
	if($('#moduleJson').html()!=""){
        ...
        html += "<div class='col-lg-12 moduleicon modVertical moduleiconActive' data-moduleName='All' id='sideModule_All'>";
        html += "<div class='row'>";
        html += "<div class='col-12 text-center'>"; 
        html += "<span class='' style='display: inline-flex; align-items: center;'>"; // Ensures inline display
        html += "<img src='/HIS/hisglobal/images/e-clinic/module_img/home.png' class='imgmoduleicon' />";
        html += "<span id='notifbdge_All' class='badge bg-danger' style='font-size: 13px; margin-left: 5px; padding: 5px 10px;'></span>";
        html += "</span>";
        html += "</div>";
	    html += "<div class='col-12 mt-1 text-center moduletext'>All</div>";      
	    html += "</div>";
	    html += "</div>";                        
	    html += "</div>"; 
	...
	$.each(moduleJson, function(indx, objModuleJson){
		var html = "<div class='row'>";
        html += "<div class='col-lg-12 moduleicon modVertical '  data-moduleDisplayName='" + objModuleJson["moduleName"] + "' data-moduleName='" + objModuleJson["moduleNameWithoutSpace"] + "' id='sideModule_" + objModuleJson["moduleNameWithoutSpace"] + "'>";
        html += "<div class='row'>";
        html += "<div class='col-12 text-center'>"; 
        html += "<span class='' style='display: inline-flex; align-items: center;'>"; // Ensures inline display
        html += "<img src='" + objModuleJson["moduleImageName"] + "' class='imgmoduleicon' />";
        html += "<span id='notifbdge_"+ objModuleJson["moduleNameWithoutSpace"]+ "' class='badge bg-danger' style='font-size: 13px; margin-left: 5px; padding: 5px 10px;'></span>";
        html += "</span>";
        html += "</div>";
	    ...
	    html += "</div>";                        
	    html += "</div>"; 
		$('#moduleId').append("<option value='"+objModuleJson["moduleNameWithoutSpace"] +"'>"+objModuleJson["moduleName"] +"</option>")
		$('#modules').append(html);
	});
	$('.moduleicon').click(function(){
		var moduleName= $(this).attr("data-moduleName");
		var moduleDisplayName= $(this).attr("data-moduleDisplayName");
		
		$('#divDeskListingContainer').hide();
		$('#divMenuContainer').show();
		filterModuleProcesses(moduleName);
		//alert(moduleName);
		if(moduleName!="All"){
			$(this).addClass('moduleiconActive');
			$('#selectedModuleId').val(moduleName);
			$('.processTypeHeadingModule').text(moduleDisplayName)
		}
		else{
			$('#selectedModuleId').val("");
			$('.processTypeHeadingModule').text("");
	...
	$('#moduleId').change(function(){
		...
		filterModuleProcesses(moduleName);		
		if(moduleName!="All")
			$("[data-moduleName="+moduleName+"]").addClass('moduleiconActive')
	});
  }
}
...
function initModuleHTML(processType){
	//alert("inside initModuleHTML");
	$('#mainContainer_'+processType).empty();
	if($('#moduleJson').html()!=""){
		var moduleJson=JSON.parse($('#moduleJson').html());		
		
		$.each(moduleJson, function(indx, modObj){
			var moduleMainId=modObj["moduleNameWithoutSpace"]+"-"+processType;
			var html='';
			
		
		    
		    var jsonReturn=createBoxMenus(modObj["MenuLevel1"], processType);
		    ...
		    //alert(totalLevel1Menus);
			if(totalLevel1Menus !=0){
				
				/*html+='<div class="row" id="moduleHeading_'+moduleMainId+'">';
		    	html+='<div class="col-12 module-heading" ><span style="float:left;"><img src='+modObj["moduleImageName"]+' class="imgmoduleicon"></span><span style="float:left;margin-top: 24px;" >'+modObj["moduleName"]+'</span></div>';
		    	html+='</div>';	*/	    
		    	//html+='<div class="row" id="module_'+moduleMainId+'">';		    
		    	//html+='</div>';
		    	$('#mainContainer_'+processType).append(menuLevel1HTML);
				//$('#module_'+moduleMainId).append(menuLevel1HTML);
				var menuLevel2HTML= createDrillDownMenu(modObj["MenuLevel1"], processType)
				if(menuLevel2HTML!=''){
					$('#mainContainer_'+processType).append(menuLevel2HTML)
				}
			}			
		});
		//console.log($('#mainContainer_'+processType).html());
		
		
	}
	
}
function createBoxMenus(arrMenu, processType){
	...
	$.each(arrMenuLevel1, function(indx, objmenu){
		if(objmenu["processType"]!= processType)
			return;
		var drilldown="";
		...
		if(objmenu["MenuLevel2"]!=undefined){
			//alert("before>>" + JSON.stringify(objmenu));
			objmenu=checkIfListPageRequired(objmenu);
			iconColorClass="icon-drill";
			//alert("after>>" + JSON.stringify(objmenu));
...
function createDrillDownMenu(arrMenu, processType){
	var html ='';
	if(arrMenu==undefined)
		return html;
	var arrMenuLevel1=arrMenu;
	...
	$.each(arrMenuLevel1, function(indx, objlevel1Menu){
		if(objlevel1Menu["processType"]!= processType)
			return;
