...
<%
	...
	if(objAuthorize!=null && objAuthorize.getMenusHirarchyMap()!=null){
	 ...
	 //System.out.println("st_desk_menu.jsp -> manualSet values="+manualSet);
...
<%! StringBuffer menuNames= new StringBuffer(); 
	...
	StringBuffer manualNames= new StringBuffer();//Added by Anjali
	StringBuffer manualFile=new StringBuffer();//Added by Anjali
	StringBuffer manualJSON=new StringBuffer();//Added by Anjali
	%>
<%!
/*Added by Anjali-28-04-2020*/ 
public String traverseManualMap(Map map,String list){
	...
	for (Map.Entry column : columnMapSet) {
		...
		if (column.getValue() instanceof String) { // only one level in column
			...
			list+=link; */
			manualNames.append("\""+column.getKey().toString().trim()+"\" ,");
			manualFile.append("\""+ value.trim()+"\" ,");
		}
...
public String traverseMap(Map map,String list){
					...
					<% menuNames=menuNames.delete(0, menuNames.length());
					 ...
					 manualNames=manualNames.delete(0,manualNames.length());//Added by Anjali
					 manualFile=manualFile.delete(0,manualFile.length());//Added by Anjali
					 manualJSON=manualJSON.delete(0,manualJSON.length());//Added by Anjali
					%>
 <div id="menuContainer">
			...
			<div id="smoothmenu">
				...
				<!-- Code to Generate Menu Dynamically -->
					<%
						for (Map.Entry set : moduleSet) {
							...
							if (set.getValue() instanceof String) { // no dropdown menu is there
								...
								%>
								<li  id="<%=set.getKey().toString().trim().replaceAll("\\s+","_")%>"><a href="#" onclick="menuSelected('<%=set.getKey().toString() %>',true)"> <span key='<%=set.getKey().toString().toLowerCase().replaceAll(" ", "-")%>' > <%=set.getKey().toString() %> </span> </a> 
					...
					<%
					/*Added by Anjali to create Manual files JSON-28-04-2020*/
						for (Map.Entry set : manualSet) {
							String key = set.getKey().toString();
							//System.out.println("key: " + key);
							if (set.getValue() instanceof String) {
								System.out.println("st_Desk_menu.jsp --> manualSet "+ set.getValue().toString()+ " "+set.getKey().toString());// no dropdown menu is there
								%><%-- <li><a href="<%=set.getValue().toString() %>"> <%=set.getKey().toString() %></a> </li> --%>
								<%
							}else if (set.getValue() instanceof HashMap) {
								%>
								...
								for (Map.Entry columnHeadinigMapSet : columnHeadingSet) {
									...
									<%
										HashMap columnMapFirstLevel=(HashMap) columnHeadinigMapSet.getValue();
										String list= traverseManualMap(columnMapFirstLevel,"");
										%>
										<%-- <%= list %> --%>
					...
					if(manualFile.toString().length()==0)
						manualJSON.append("{ \"manualNames\": ["+manualNames.toString() +"], \"manualFile\" :["+manualFile.toString()+"]}");
						else
						manualJSON.append("{ \"manualNames\": ["+manualNames.toString().substring(0, manualNames.toString().length()-1) +"], \"manualFile\" :["+manualFile.toString().substring(0, manualFile.toString().length()-1)+"]}");
		...
		<input type="hidden" name="manualJSON" id="manualJSON" value='<%= manualJSON%>'><!-- Added by Anjali -->
