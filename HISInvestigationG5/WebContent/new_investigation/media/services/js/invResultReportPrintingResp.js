...
$(document).ready(function() {
  ...
  $('#inputFilterType').change(function() {
    ...
    if (inputTypeBCSL == "1") {
        ...
        $('#inputFilterTypeHelp').html("Search Based On Ben Id.");
        showLoding(false);
    }
...
function showLoding(showBoolean) {
  ...
  if (showBoolean == true) {
    ...
    tippyAllInstanceJson.getDataBtnDiv.setContent('Loading.. Please Wait');
	tippyAllInstanceJson.getDataBtnDiv.enable();
    ...
    $('#container2Row1').removeClass("gradient-border");
	  tippyAllInstanceJson.getDataBtnDiv.disable();
  } else {
    $('#getingData').addClass("d-none");
    $('.rollingCircle').addClass("d-none");
    $('.lodingText').addClass("d-none");
...
function enableSaveBtns(enableFlag){
	if(enableFlag==true){
		$('#saveValidateBtn').attr({"disabled":false});
		  $('#modifyBtn').attr({"disabled":false});
		  
		  tippyAllInstanceJson.saveValBtnDiv.setContent('Click to Validate Selected Requisition');
		  tippyAllInstanceJson.modifyValBtnDiv.setContent('Click to Modify Selected Requisition');
	}else{
		$('#saveValidateBtn').attr({"disabled":true});
		  $('#modifyBtn').attr({"disabled":true});
		  
		  tippyAllInstanceJson.saveValBtnDiv.setContent('Please select atleast one Requisition to Validate');
		  tippyAllInstanceJson.modifyValBtnDiv.setContent('Please select atleast one Requisition to Modify');
	}
}
...
$(document).ready(function(){
	...
	tippyAllInstanceJson.labSelectInputGroup=tippy_labSelectInputGroup[0];
	tippyAllInstanceJson.getDataBtnDiv=tippy_getDataBtnDiv[0];
	
	tippyAllInstanceJson.saveValBtnDiv=tippy_saveValBtn[0];
	tippyAllInstanceJson.modifyValBtnDiv=tippy_modifyValBtn[0];

	//tippyAllInstanceJson.tippy_labSelectInputGroup.enable();
	//tippyAllInstanceJson.tippy_getDataBtnDiv.enable();
	//tippyAllInstanceJson.tippy_labSelectInputGroup.show();
...
$(document).ready(function(){
	$('#collAreaSelectInput').change(function(){
		if(globalSearchParam.collAreaCode==$(this).val()){
			...
			tippyAllInstanceJson.labSelectInputGroup.disable();
			tippyAllInstanceJson.getDataBtnDiv.disable();
		}else if($(this).val()=="-1"){
			...
			tippyAllInstanceJson.labSelectInputGroup.setContent('<strong>Select Any Collection <span style="color: aqua;">Area </span>To Enable This</strong>');
			tippyAllInstanceJson.labSelectInputGroup.enable();

			tippyAllInstanceJson.getDataBtnDiv.setContent('<strong>Select Any Collection <span style="color: aqua;">Area </span>To Enable This</strong>');
			tippyAllInstanceJson.getDataBtnDiv.enable();
...
$(document).ready(function(){
	$('#labSelectInput').change(function(){
		if(globalSearchParam.labCode==$(this).val()){
			...
			$('#getData').attr({"disabled":"disabled"});
			showLoding(false);
			tippyAllInstanceJson.getDataBtnDiv.setContent('<strong>Select Any<span style="color: aqua;"> Lab Name </span>To Enable This</strong>');
			tippyAllInstanceJson.getDataBtnDiv.enable();
		}else{
			$('#getData').removeAttr("disabled");
			tippyAllInstanceJson.getDataBtnDiv.disable();
...
function openModifyResultIFrame(globalSelectedReqnData){
	...
	var requestData = {hmode:"SHOWPATDETAILS", selectedCheckbox:concatSelectedReqnData, chkSendToMachine:concatChkSendToMachine, newEntry:valstusCod};
