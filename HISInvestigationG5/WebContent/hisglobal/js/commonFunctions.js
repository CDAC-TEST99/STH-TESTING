...
function validateAlphaNumericOnly(e,obj)
{
	...
	// control keys
	if ((key==null) || (key==0) || (key==8) ||
		(key==9) || (key==13) || (key==27) || (key==46)) //key 46 for . 
	   return true;
	else if(m)
		return true
...
//////////////////////////////////////////	
 ...
 function removeSpecialCharacter(obj){
	  ...
	  var res  = obj.replace("$", " ", true);
      var res2  = res.replace("~", " ", true);
      
      obj = res2.replace("@", " ", true);
...
//  ******************************************************************************
 ...
 function generateActualCRNO(obj,event){
		...
		eval("processCRNOFromSpecificProcess")();
 ...
 function validateCRNoCHECK(size)
	{
		...
		var crdigit=$('#crDigit').val();
		if(crdigit.trim()==""){
			alert("Please enter CRNO digits!")
			return
		}
		var len=8;
		var final_CR_Digit="";
	...
	var valid=false;
	if(validateMinLength(document.getElementsByName('patCrNo')[0],size) && validateCheckSumBySize(size))
	{
	valid=true;
	}
	else
...
/////////////////////////////////////////////////////function for validating minimum length of a field///////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 function checkSum15(){
 		...
 		if(crNo.substring(5,6)=='1'){
 			return true;
 		}
 		else {
 			//alert(crNo.substring(5,6));
 ...
 function checkTime(eObj,obj)
 {
 	...
 	if(key>=48 && key<=57)
 	{
 		//swal("obj.value.length="+obj.value.length)
