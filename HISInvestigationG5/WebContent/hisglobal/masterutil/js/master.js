...
function OnMouseMove(e) {
	try{
		if(_dragFagFirst){
			...
			dynDivObj.innerHTML="<Table border='1px' cellpadding='1px' cellspacing='1px' style='border-style: solid' border-collapse: collapse; bordercolor='BLACK' rules='none' bgcolor='WHITE'><tr><td class='MASTERNAME' nowrap='nowrap' id='DragTableTDId'> &nbsp;&nbsp;&nbsp;"+
			countCheckedRow
			+" Record(s) Selected </td></tr></table>";
...
function fetchRecords(index,rowNum,prevNext)
{
	...
	var params= document.forms[0].action + "?hmode=LISTPAGE"+str+"&searchColumn="+searchColumn+"&search="+searchVal+"&blockNo="+blockNo+"&prevNext="+prevNext+"&rowNum="+rowNum+"&orderby="+order+"&date="+new Date().getTime();
		
//	alert(" fetchRecords :  "+params);	
...
function searchpage(){
	/*if(document.getElementsByName("search")[0].value==""){
		alert("Please Enter a valid search field.")
...
//alert(" searchpage :  "+params);
	...
	xmlHttp.open("POST",params,true);
	document.getElementById('start').innerHTML = '<table align=center><tr class="ROWFONT"><td height=30 colspan=8  align="right"> <b>Wait Fetching Data from Database...</b></td></tr></table>';
...
function changeColor(event,i,flag)
{
	...
	var retVal;
	var className="ONMOUSEOUT1";
	
	if(parseInt(i)%2==0)
	{
		className="ONMOUSEOUTEVEN";
	}
	else
		className="ONMOUSEOUTODD";
...
function fetchViewData()
	...
	//alert(document.forms[0].cntPage.value);
//    var params= "../../../" + document.forms[0].cnt_page.value+".cnt?hmode="+document.forms[0].hmode.value+"&chk="+document.forms[0].chk.value;
	...
	//alert(" fetchViewData :  "+params);	

    xmlHttp.open("POST",params,true);
	document.getElementById("viewdata").innerHTML ='<table><tr class="ROWFONT"><td height=30>Wait Fetching Data from Database...</td></tr></table>';
	xmlHttp.onreadystatechange = function()
	{
		if(xmlHttp.readyState ==4 )
			{ 
 	...
 	if((index < (parseInt(opener.document.forms[0].chk.length))) && (index < total_rec))
	 	{
			...
			if(opener.document.getElementById("tr"+index).className!="rowfontmatch")
				opener.document.getElementById("tr"+index).className  =	'ONMOUSEOUT1';
			//opener.document.getElementById("tr"+index).className				=	'ONMOUSEOVER1';
			if(opener.document.getElementById("tr"+(parseInt(index)+1)).className!="rowfontmatch")
				opener.document.getElementById("tr"+(parseInt(index)+1)).className	=	'ONMOUSEOVER1';
...
//		var params="../../../"+document.forms[0].cnt_page.value+".cnt?hmode=VIEWDATA&chk="+chk;
		...
		//	alert(" fetchViewNext :  "+params);	
		
		xmlHttp.open("POST",params,true);
		document.getElementById("viewdata").innerHTML ='<table><tr class="ROWFONT"><td height=30><b>Wait Fetching Data from Database...</b></td></tr></table>';
		xmlHttp.onreadystatechange = function()
			{
...
{  alert("VIEWDATA1");
	...
	if(index == 0)
	{
		...
		//alert(index);
		//alert(document.getElementById("tr"+(index)).className);
		//alert(document.getElementById("tr"+(index+1)).className);
		//testId="tr"+index;
		if(opener.document.getElementById("tr"+index).className!="rowfontmatch")
			opener.document.getElementById("tr"+index).className='ONMOUSEOVER1';
		//opener.document.getElementById("tr"+index).className				=	'ONMOUSEOVER1';
...
function userInterface(){
	...
	objContentDiv.innerHTML ='<table><tr class="ROWFONT"><td height=30><b>Wait Report Wizard is being processed...</b></td></tr></table>';
	...
	xmlHttp.onreadystatechange = function(){
		if(xmlHttp.readyState ==4 ){
			if(xmlHttp.status == 200 || xmlHttp.status=='complete'){
				...
				var responseData=response.split("####");
				if (responseData[0] == "ERROR") {
					objContentDiv.innerHTML 	= "<table align=center><tr><td height=30><font color = 'red'><b>" + responseData[1] + "</b></font></td></tr></table>";
				}else{
...
{ 
	...
	var params= document.forms[0].action + "?hmode=REPORTDATA"+str+"&minrow"+minrow
			...
			//alert(" fetchReportsData :  "+params);		
				
	xmlHttp.open("POST",params,true);
	if(document.getElementById('reportdata').innerHTML=="")
		document.getElementById('reportdata').innerHTML ='<table><tr class="ROWFONT"><td height=30><b>Wait Fetching Data from Database...</b></td></tr></table>';
	if(strShowAllData==1)
...
function fetchNextPrev(blockNo)
{
	...
	var params		=	document.forms[0].action + "?hmode=REPORTDATA"+str+"&minrow=0&blockNo="+blockNo
...
/********* Utility to handle onkeypress events automatically on buttons in masters **********/

var ie = document.all;
var tempAdd,tempMod,tempDel,tempView,tempRpt;
...
function firstKeyDown(e)
{
    ...
    for(var x=0;x<objImg.length;x++)
    {
    	var temp=objImg[x].src.split("/")[6];
    	
    	if(temp=="btn-add.png") // ADD button logic
    	{   
    		tempAdd=objImg[x].onclick.valueOf();    		  
    		objImg[x].onkeypress=firstKeyDownAdd;
    	}    	
    	if(temp=="btn-mo.png") // MODIFY button logic
    	{
    		tempMod=objImg[x].onclick.valueOf();    		 
    		objImg[x].onkeypress=firstKeyDownMod;
    	}    	
    	if(temp=="btn-del.png") // DELETE button logic
    	{
    		tempDel=objImg[x].onclick.valueOf();    		 
    		objImg[x].onkeypress=firstKeyDownDel;
    	}    	
    	if(temp=="btn-view.png") // VIEW button logic
    	{
    		tempView=objImg[x].onclick.valueOf();    		  
    		objImg[x].onkeypress=firstKeyDownView;
    	}    	
    	if(temp=="btn-rpt.png")  // REPORT button logic
    	{
    	    tempRpt=objImg[x].onclick.valueOf();    		 
    		objImg[x].onkeypress=firstKeyDownRpt;
    	}
    }		
}
// ADD button
function firstKeyDownAdd(e)
{
	var addVal=tempAdd.toString().split("{")[1].split("}")[0];
	if(e.keyCode=="13")
	{
    	var F=new Function (addVal);
    	return(F());		
	}
}  
// MODIFY button
function firstKeyDownMod(e)
{
	var modVal=tempMod.toString().split("{")[1].split("}")[0];
	if(e.keyCode=="13")
	{
	    var F=new Function (modVal);	
    	return(F());		
	}
} 
// DELETE button
function firstKeyDownDel(e)
{
	var delVal=tempDel.toString().split("{")[1].split("}")[0];
	if(e.keyCode=="13")
	{
	    var F=new Function (delVal);	
	    return(F());		
	}
} 
// VIEW button
function firstKeyDownView(e)
{
	var viewVal=tempView.toString().split("{")[1].split("}")[0];
	if(e.keyCode=="13")
	{
	    var F=new Function (viewVal);	
	    return(F());		
	}
} 
// REPORT button
function firstKeyDownRpt(e)
{
	var rptVal=tempRpt.toString().split("{")[1].split("}")[0];
	if(e.keyCode=="13")
	{
	    var F=new Function (rptVal);	
	    return(F());		
	}
} 
...
function deleteRecordsOnRow(mode,obj2)
{
	
	var allCheckBox=document.getElementsByName("chk");
	for(var nTmpI=0;nTmpI<allCheckBox.length;nTmpI++)
	{
		if(allCheckBox[nTmpI].value.split("$")[1]==obj2.name.split("$")[1])
		{
			allCheckBox[nTmpI].checked=true;
			break;
		}
	...
	var obj = document.getElementsByTagName("img");
	 ...
	 {
	 	if(obj[i].src.indexOf("btn-del.png") >-1)
	 	{
	 		//alert("found"+obj[i].attributes['onclick'].value);
	 		var invokeFuncName=obj[i].attributes['onclick'].value;
	 		//alert(invokeFuncName);
	 		//invokeFuncName=invokeFuncName.split("return ")[0];
	 		//alert(invokeFuncName);
...
/* This function assigns the classNames to Add/Modify/Delete/View/Report Buttons*/

function assignClassToButtons()
{
	
	var obj = document.getElementsByTagName("img");
	 for(var i = obj.length-1 ; i >= 0; i--)
	 {
	 	if(obj[i].src.indexOf("btn-add.png") >-1||obj[i].src.indexOf("btn-mo.png") >-1
	 	||obj[i].src.indexOf("btn-del.png") >-1||obj[i].src.indexOf("btn-view.png") >-1
	 	||obj[i].src.indexOf("btn-rpt.png") >-1||obj[i].src.indexOf("btn-ccl.png") >-1)
	 	{
	 		obj[i].className="masterButtonClass";	 		
	 	}
	 }
	 
}

function addClientMst(mode)
{
	if(document.getElementsByName('combo')[0].value=='0')
	{
		alert("Kindly Select Client Type.");
	}
	else
	{
		document.forms[0].hmode.value=mode;
