...
<his:javascript src="/new_investigation/js/reportsValidation_new.js" />
...
//	var elementCode=thisobj.name.split('#')[3];
	...
	/*mywindow.moveTo(300,300); *//* 	mywindow.moveTo(300,300); */		
}
...
function setmachinebasetest(obj)
{

	var idmachinetest=obj.id;
	
	//alert("idmachinetest"+idmachinetest+"obj.checked"+obj.checked);
	
	
	if(obj.checked)
		{
		//alert("true");
		var newtestcheckbox=idmachinetest;
	
	//	alert(document.getElementsByClassName(newtestcheckbox).length);
		var len=document.getElementsByClassName(newtestcheckbox).length;
	
		for(var a=0;a<len;a++)
			{
			document.getElementsByClassName(newtestcheckbox)[a].checked=true;
			}
		...
		//	alert(document.getElementsByClassName(newtestcheckbox).length);
			...
			for(var a=0;a<len;a++)
				{
				document.getElementsByClassName(newtestcheckbox)[a].checked=false;
				}
...
<body onload="checkEntryType()">
	<html:form action="/invResultValidationTemplateTile">
		...
		<html:hidden name="InvResultValidationFB" property="isSaveToDraft" />
					...
					<tr>
						...
						<td width="25%" class="tdfont">
								...
								value="1"></html:radio>
								 ...
								 <html:radio
								name="InvResultValidationFB" tabindex="1" property="newEntry"
								value="2"></html:radio> <bean:message key="modifyValidate" />&nbsp;
								 <html:radio
								name="InvResultValidationFB" tabindex="1" property="newEntry"
...
<% List<ResultEntryVO> lstPatVO=(List<ResultEntryVO>)session.getAttribute(InvestigationConfig.GROUP_MODIFIED_LIST_PATIENT_RESULT_ENTRY_ESSENTIALS_VO);
					 ...
					 %>
						...
						<!-- <td width="8%" align="left"><b> <font color="#000000" -->
							<td width="6%" align="left" style="display: ;"><b> <font color="#000000"
								size="2" face="Verdana, Arial, Helvetica, sans-serif"> <bean:message
											key="age/gender" /></font></b></td>
							<td width="10%" align="center"><b><font color="#000000"
									size="2" face="Verdana, Arial, Helvetica, sans-serif"> <bean:message
			 ...
			 %><font color="<%=color%>" size="2"
								...
								<td width="4%" align="left">
									<b> <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
										<a onclick="openInvTrackingToCompare('<%=voPat.getPatCRNo()%>','<%=voPat.getTestCode()%>','<%=voPat.getGroupCode()%>','<%=voPat.getRequisitionDNo()%>','<%=voPat.getRequisitionNo()%>');" >
...
<%
										...
										<tr>
											...
											<th width="20%"><font color="red" size="2"
												face="Verdana, Arial, Helvetica, sans-serif"><u><bean:message
															...
															<th width="20%"><font color="red" size="2"
												face="Verdana, Arial, Helvetica, sans-serif"><u>Send Test To Machine</u></font></th>
					...
					for(int iterate=0;iterate<paraSize;iterate++)
					{
						...
						String machinetestid="";
						String machinetestvalue="";
						String ismachinebased="0";
						String reqdno="";
						String testcode="";
						
						if(paraValues.length>=5)
							ismachinebased=paraValues[4];

						
						if(paraValues.length>=6)
							reqdno=paraValues[5];

						if(paraValues.length>=7)
							testcode=paraValues[6];
					
						if(ismachinebased.equals("1"))
						{
							if(!reqdno.equals("") && !testcode.equals(""))
							{
								machinetestid=reqdno+"#"+testcode;
								machinetestvalue=reqdno+"#"+testcode;
							}
								...
								if(rangeTypeFinal.equals("1")){
									 ...
									 if((Float.valueOf(paraEntry) > Float.valueOf(highValue)) || (Float.valueOf(paraEntry) < Float.valueOf(lowValue))){
										 ...
										 <tr>
											...
											<th width="20%"><%=displayRef%><th width="20%"><%if(ismachinebased.equals("1")){%><input name="chkSendToMachinetest" type="checkbox" class="<%=machinetestid%>" id="<%=machinetestid%>" value="<%=machinetestvalue%>" onclick="setmachinebasetest(this)"><%}%></th>
											<!-- 	<th width="20%">ml</th> -->
										</tr>
		...
		<his:ButtonToolBarTag>
				...
				<logic:present
					...
					<logic:equal name="InvResultValidationFB" property="newEntry"
						...
						<img class="button"
							src='<his:path src="/hisglobal/images/btn-save-validate.png"/>'
							id="saveDiv" onkeypress="if(event.keyCode==13)setIndication(); if(event.keyCode==13) onSave();"
							tabindex="1" onclick="setIndication(); onSave();">
						...
						onkeypress="if(event.keyCode==13)setIndication(); if(event.keyCode==13) onSaveValidationToDraft();" tabindex="1"
						onclick="setIndication(); onSaveValidationToDraft();">
