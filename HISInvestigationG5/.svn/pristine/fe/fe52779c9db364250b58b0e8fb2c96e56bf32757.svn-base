...
function AjaxGetPatDetails(globalSearchParam) {
	//alert("inside AjaxGetPatDetails");
		...
		if(!$.isEmptyObject(data)) {
			if(data.isSuccess=="1") {
				// alert("test"+JSON.stringify(data))
		    	  ...
		    	  console.log("data--->>>"+patDetails);
		    	  console.log("data2--->>>"+JSON.stringify(patDetails));
				  ...
				  $("#patDetails-table").removeClass("col-md-12");
				  ...
				  $("#patDetails-table").addClass("col-md-11");
		    	  ...
		    	  $("#patDetails-table").addClass("d-none");
		    	  ...
		    	  if(patDetails.patStatusCode=="2"){
		    		  ...
		    		  $("#patDetails-table").removeClass("d-none");
		    		 // dataEpisodeTablePatDetails(data.patDetails);
		  		  }
		    	  /*----------------------------------------------------*/
...
function dataTableOPDPatDetails(patDetailsOPD){
	...
	if(document.getElementById("patCategoryCode_samplecoll01")!=undefined && document.getElementById("patCategoryCode_samplecoll01")!=null)
		{
		var s = JSON.stringify(patDetailsOPD) ;

		var s1 = JSON.parse(s);

		var x = s1[0];
				var s12 = JSON.stringify(x) ;
		var obj = JSON.parse(s12);

		
		document.getElementById("patCategoryCode_samplecoll01").value=obj.patCategoryCode;
        }
	    
	//Added by Nandini
	var data = patDetailsOPD[0];
	//alert(data)
	//$('[name="strPatHospCode"]').val(data.hcode);
	$('[name="patCRNo"]').val(data.crNumber);
	$('[name="patName"]').val(data.patientName);
	$('[name="patStatus"]').val(data.patStatus);
	$('[name="patGenderCode"]').val(data.patGender);
	$('[name="patAge"]').val(data.patAge);
	$('[name="patGuardianName"]').val(data.patGuardianName);
	$('[name="patCategoryCode"]').val(data.patCategory);
	$('[name="patDOB"]').val(data.patDOB);
	$('[name="patMobileNo"]').val(data.patMobileNo);
	$('[name="patAddress"]').val(data.patAddress);	
	//Ended by Nandini
...
/*
function dataEpisodeTablePatDetails(patDetailsINV){

	if(document.getElementById("patCategoryCode_samplecoll01")!=undefined && document.getElementById("patCategoryCode_samplecoll01")!=null)
		{
		var s = JSON.stringify(patDetailsINV) ;
	...
	//Added by Nandini
	var data = patDetailsINV[0];
	//alert(data)
	
	//alert("departmentname>>"+data.departmentname)
	$('[name="departmentname"]').val(data.departmentname);
	$('[name="unitname"]').val(data.unitname);
	$('[name="visitdate"]').val(data.visitdate);
	//$('[name="lastvisitno"]').val(data.lastvisitno);
	/// lastvisitno;
	//Ended by Nandini
...
function dataTableIPDPatDetails(patDetailsIPD){
	...
	if(document.getElementById("patCategoryCode_samplecoll01")!=undefined && document.getElementById("patCategoryCode_samplecoll01")!=null)
	{
	var s = JSON.stringify(patDetailsIPD) ;
