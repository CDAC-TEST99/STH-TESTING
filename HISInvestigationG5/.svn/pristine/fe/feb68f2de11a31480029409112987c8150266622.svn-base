...
if (orderArray[1].equals("x.gstr_test_name")) {
    
    sb.append(" ORDER BY hivt_num_element_sequence , " + orderby);
} else {
    ...
    sb.append(" ORDER BY " + orderby);
			...
			//	sb.append(" order by  hivt_num_element_sequence ," + orderby);
				...
				///sb.append(" order by  hivt_num_element_sequencefsd ," + vo.getOrderby());
			}
			
			sb.append(") A ) B WHERE B.SNO <=" + vo.getMax_rownum()+ "  AND B.SNO > " + vo.getMinrow() + " ");
			System.out.println("dao.getQryResult(sb.toString()>>"+dao.getQryResult(sb.toString()));
...
//	 getArrayList() methods will give the data for view mode in form of List.
	...
	public static void getReports(GenericVO vo, String query, int maxRow, int minRow,String strShowAllData){
		...
		try{
			...
			if (vo.getSearch() != null && !vo.getSearch().equals("null") && !vo.getSearch().equals(""))
				sb.append("  and exists (select 'x' from dual where UPPER(" + vo.getSearchColumn()	+ ") LIKE UPPER('" + vo.getSearch().replace("$", "%") + "'||'%')) ");
	...
	*/
	...
	public static void insertReportConfig(GenericVO _genericVO) {
		...
		try {
			dao = new HisDAO("Master Template","GenericDAO");    
			strDeleteQuery="DELETE FROM GBLT_MASTER_REPORT_CONFIG WHERE GSTR_MASTER_NAME='"+_genericVO.getStrMasterName().split("#")[0]+"' AND GNUM_HOSPITAL_CODE="+InvestigationConfig.HOSPITAL_CODE;
			...
			dao.setQryValue(nQueryIndex,11,InvestigationConfig.HOSPITAL_CODE);  
	...
	public static void fetchReportConfig(GenericVO _genericVO) {
		...
		try{
			...
			strQuery.append("' AND G.GNUM_HOSPITAL_CODE= "+InvestigationConfig.HOSPITAL_CODE); 
