...
public class InvTrackingReportDAO extends DataAccessObject implements InvTrackingReportDAOI {
	...
	public InvTrackingReportVO AjaxGetPatDetails(InvTrackingReportVO vo, UserVO userVO) {
		...
		String queryKey5="SELECT_INV_TRACKING_PAT_DETAILS_ONCRNO_HIVT_REQUISITION_DTL";//nandini
		...
		try
		{
			if(vo.getSearchType().equals("1")) {
				...
				//query5= HelperMethodsDAO.getQuery(filename, queryKey5);
		...
		{	
			...
			if(rs!=null) {
				...
				if(rs.next()) { 
				  ...
				  else { rs.beforeFirst(); while(rs.next()) {
			...
			/*
			 * rs = HelperMethodsDAO.executeQuery(conn, query5, populateMap);
			 * System.out.println("rsfdf>4>>"+rs.getFetchSize()); if(!rs.next()) { } else {
			 * rs.beforeFirst(); while(rs.next()) { HelperMethods.populateVOfrmRS(vo2, rs);
			 * } }
			 */
			...
			if(vo2.getPatStatusCode()!=null && vo2.getPatStatusCode().equals("2")){
				sq.reset();
	...
	public List<InvTrackingReportVO> AjaxGetPatSampleReqnList(InvTrackingReportVO vo, UserVO userVO) {
		...
		String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRACKING_REPORTSDAO;
		String seatId = userVO.getSeatId();
		...
		{ 
			if(vo.getSearchType().equals("1")) {
				...
				if(vo.getDataFromArchival().equals("0")) {
					...
					System.out.println("vo.getForTestOrGroupOrAll()>>"+vo.getForTestOrGroupOrAll());
					populateMap.put(sq.next(), vo.getTestCode());
					System.out.println("vo.getTestCode()>>"+vo.getTestCode());
					populateMap.put(sq.next(), vo.getGroupCode());
					System.out.println("vo.getGroupCode()>>"+vo.getGroupCode());
					}
				}
