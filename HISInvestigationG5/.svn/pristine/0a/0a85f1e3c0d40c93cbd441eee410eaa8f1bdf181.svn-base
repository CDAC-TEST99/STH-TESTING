	...
	public static JsonObject AjaxGetPatDetails(InvTrackingReportFB fb, HttpServletRequest request) {
		...
		try {
			...
			if(userVO.getHospitalCode()==null) {
				userVO.setHospitalCode(fb.getHospitalCode()==null || fb.getHospitalCode().equals("")?fb.getCrNo().substring(0, 5):fb.getHospitalCode());
				
			
			}
			
			/*
			 * vo.setDepartmentname(fb.getDepartmentname());
			 * vo.setUnitname(fb.getUnitname()); vo.setVisitdate(fb.getVisitdate());
			 */
			...
			System.out.println("patstatus"+vo2.getPatStatusCode());
			...
			objectRow.addProperty("unitname", vo2.getUnitname()==null?"NA":vo2.getUnitname());
			objectRow.addProperty("visitdate", vo2.getVisitdate()==null?"NA":vo2.getVisitdate());
			//lastvisitno
			objectRow.addProperty("lastvisitno", vo2.getVisitdate()==null?"":vo2.getVisitdate());
			
			System.out.println("vo.getDepartmentname>>" + vo.getDepartmentname());
			...
			for(int i=0;i<arrSelectedPDFName.length;i++)
			 {
				...
				filename=filename;
					 ...
					 response.setHeader("content-disposition", "inline; filename="+filename);	
						...
						System.out.println("Mongo filename=="+filename);
	...
	public static void AjaxGetPDFReportFTP(InvTrackingReportFB fb,HttpServletRequest request,HttpServletResponse response)
	{
		...
		try{
			...
			String[] arrSelectedPDFName=selectedPDFName.split("@@");
				...
				{
					  String strPdfName=(String)itrPdfName.next();
					  String[] parts = strPdfName.split("_");

				        // The first part is the crNo
				        String crNo = parts[0];
					  //String crNo=strPdfName.substring(0,7);
				...
				//		String year= crNo.substring(5,7); //MergeAllPdfNewInv.getYear(crNo); 
					 ...
					 //   String count=MergeAllPdfNewInv.getcount(crNo);
					   ...
					   // String strPdfPath = patientcreatefileftpaddress+"/"+hospCode+"/"+ "20"+year+"/"+insideyear+"/"+count+"/"+crno+"/"+strPdfName;
					     
					    String strPdfPath = patientcreatefileftpaddress+"/"+hospCode+"/"+crno+"/"+strPdfName;
				...
				filename=filename+".pdf";
					 ...
					 response.setHeader("content-disposition", "inline; filename="+filename);	
						OutputStream output = response.getOutputStream();
						//System.out.println("concatallPDFsconcatallPDFsconcatallPDFsconcatallPDFs chandannn");
						System.out.println("ftp filename=="+filename);
