...
function processCRNOFromSpecificProcess(){
	var patCrNo=$("[name=patCrNo]").val()
	
	getPateintandTestDetails();
	getEpisodeDetails();	
	
/*
 * $('#episodeDetailTable tbody').html(""); if($('[name="patCrNo"]').val()!=""){
 * var data ={ patCrNo:$('[name="patCrNo"]').val(),
 * patEpisodeCode:$('[name="patEpisodeCode"]').val(),
 * patVisitNo:$('[name="patVisitNo"]').val(), hmode:"GET_EPISODE_LIST" }
 * 
 * 
 * $.ajax({url:'/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt',
 * dataType: "json", data:data, type: "POST", success:function(json){
 * //alert(JSON.stringify(json)); $('#episodeDetailTable tbody').html(""); var
 * flagDataFound=false; var pk=""; var count=0; $.each(json,function(indx,
 * objjson){
 * 
 * count++; flagDataFound=true; html=""; html+="<tr>";
 * pk=objjson["episodeCode"]+"_"+objjson["visitNo"]
 * 
 * html+="<td><input type='radio' name='selectedEpisodeDtl' value='"+pk+"'
 * onclick='populateEpisode(this)'></td>"; html+="<td>"+objjson["departmentName"]+"</td>";
 * html+="<td>"+objjson["UnitName"]+"</td>"; html+="<td>"+objjson["visitDate"]+"</td>";
 * html+="</tr>";
 * 
 * 
 * $('#episodeDetailTable tbody').append(html);
 * 
 * 
 * 
 * 
 ...
 * });
 * 
 * if(flagDataFound==false){ html="<tr>"; html+="<td colspan='4' class='text-center text-danger' style='font-weight: bold;'>No
 * Episode details Found</td>"; html+="</tr>"; $('#tablePatientList
 * tbody').append(html); } else{ if(count==1){
 * document.getElementsByName("selectedEpisodeDtl")[0].checked=true;
 * populateEpisode(document.getElementsByName("selectedEpisodeDtl")[0]); } } }
 * }); }
 */
...
function processCRNOFromSpecificProcessOnGO(){
	
	/*if(event.keyCode!=13){ 
		return validateNumericOnly(obj,event);	
	}*/
	
	var crClientId=$('#crClientId').val();
	var crYear=$('#crYear').val();
	var crdigit=$('#crDigit').val();
	var len=8;
	var final_CR_Digit="";
	if(crdigit.length<len){
		var zeroRequired= len-crdigit.length;
		for(i=0;i<zeroRequired;i++ ){
			final_CR_Digit+="0"
		}
	}
	final_CR_Digit+=crdigit;
	$('#crDigit').val(final_CR_Digit);
	$("[name=patCrNo]").val(crClientId+"/"+crYear+"/"+final_CR_Digit);
	////alert("in"+$("[name=patCrNo]").val());
	
	if($('#crNoInput').length>0){
		$('#crNoInput').val($("[name=patCrNo]").val());
	}
	//submitFormOnValidate(validateCRNoCHECK(15),'GETPATDTL');

	//var patCrNo=$("[name=patCrNo]").val()
	
	$('#inputCrDivId').addClass("d-none");
	getPateintandTestDetails();
	getEpisodeDetails();	
}
				...
				$("[name=srrChk]:checked").each(function(){
					...
					$.each(jsonArr,function(indx,jsonobj){
						if(jsonobj["gnum_test_code"]==arr[1]){
							...
							jsonobj["patCRNo"]= $('[name="patCrNum"]').val();
...
$(document).ready(function() {
	...
	if($('[name="patCrNo"]').val()!=""){
		...
		/*
		 * getEpisodeDetails(patCrNo); getPateintandTestDetails(patCrNo);
		 */
		document.getElementById("crDigit").addEventListener("keypress", myFunction);
         function myfunction()
         {
		generateActualCRNO1(obj,event);
		
         }
	}
	
  $('#getData').click(function() {
	//   alert("inside getData click");
	  $('[name="patCRNo"]').val($('#crNoInput').val());
	  var patCrNo=$('[name=patCrNo]').val();
	  getEpisodeDetails(patCrNo);
	  getPateintandTestDetails();   
  });
	...
	/* function getPateintandTestDetails(patCrNo){ */
function getPateintandTestDetails(){
	alert("hii90");
	$('.patDetails').removeClass("d-none");
	  var _mode = "AJX_TEST_DTL";
	  var patCrNo=$('[name=patCrNum]').val();
	  var ticket=$('#varSSOTicketGrantingTicket').val()
	  //alert('patCrNo '+$('[name=patCrNum]').val())
	/// alert('ticket'+ticket)
	  $.ajax({
        type: "POST",
     //   url: "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode+"&ticket="+ticket,
        //url: "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode+"&isbillingreq="+$('#isbillingreq').val()+"&ticket="+ticket,
        url: "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode+"&isbillingreq="+$('#isbillingreq').val()+ "&patCRNo="+$('[name=patCrNum]').val()+"&ticket="+ticket,
...
/*
 * alert('shortnameenew'+$('#varclientshort').val());
 * alert('clientcode'+$('#varclientcode').val());
 * alert('clientname'+$('#varclientname').val());
 * alert('clientstrtyr'+$('#varclientstartyear').val());
 */
 	...
 	/*
	 * var crno = $('#crNoInput').val(); var crnonew = $('#crNoInput').val();
	 */


     
   
 	var clientShortName = $('#VarClientShort').val();
 	var client_code = $('#VarClientId').val();
 	var client_name = $('#VarClientName').val();
 	var client_start_year = $('#VarClientStartYear').val();
 	 

 
 	// validateCRNoCHECK(15);
  
    // document.getElementById("newOfflineTestsListId").classList.remove("d-none");
    // console.log("fthfyt");
       /* if (crno.length == crnonew.length) { */
            showLoding(true);
         // document.getElementById("newOfflineTestsListId").classList.remove("d-none");
        	$('.patDetails').removeClass("d-none");
        	
        	 var patCrNo=$('[name=patCrNo]').val();
        	// var patCrNo=$('[name=ptcr]').val();
        	 
        	 
      
        	/*patCrNo="'"+patCrNo+"'"*/
        
        	var globalSearchParam={ crNo:patCrNo, billNo:0,searchType:"1"};
        		...
        		$.when(globalAjaxGetPatDtlObj ).then(function( data, textStatus, jqXHR ){
        		 ...
        		 //alert("Loading...");
            		...
            		$("#patDetails-table").addClass("col-md-12");
       	    	 });
        	//	$('#container2').hide();
			...
			/*
			 * } else{ alert("Cr. No. Should Be Of "+crnonew.length+" Digits");
			 * $('#crNoInput').focus(); return null; }
			 */
        	//	alert("opd");
             showLoding(false);
             ShowCheckboxTestDtl();  
...
function showLabTestDtl()
{
	  ...
	  var urlNew = "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode;
...
$(document).ready(function(){
 ...
 var str = null;
/*
 * function billedUnbilledfun(patStatus){ // alert("hello "+patStatus) var crNo =
 * $('#crNoInput').val(); var patStatus = patStatus; var _mode =
 * "AjaxGetDetails"; var sampleAreaCode=
 * document.getElementsByName("sampleAreaCode")[0].value;
 * 
 * var url =
 * "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode=" +
 * _mode + "&crNo=" + crNo +"&patStatusCode=" +patStatus+"&sampleAreaCode="
 * +sampleAreaCode; // alert(url); var ajaxObjectArr1 ={};
 * ajaxObjectArr1.AjaxGetPatDetails=$.getJSON(url, function(data) {
 * if(!$.isEmptyObject(data)) { //alert("hi"); if(data) { //alert(data);
 * $('#billedSpan').empty(); $('#UnbilledSpan').empty(); var bill=
 * data.BilledDtl.length; var unbill = data.UnbilledDtl.length;
 * $('#billedSpan').append(bill); $('#UnbilledSpan').append(unbill);
 * callUnBilledSection(data.UnbilledDtl);
 * callbilledSection(data.BilledDtl,data.UOMCombo,data.machineCombo,data.ContainerCombo);
 * //alert(data.UnbilledDtl.length); //alert(data.BilledDtl.length);
 *  } else { var bill= data.BilledDtl.length; var unbill =
 * data.UnbilledDtl.length; $('#billedSpan').empty();
 * $('#UnbilledSpan').empty(); $('#billedSpan').append(bill);
 * $('#UnbilledSpan').append(unbill);
 *  } }
 *  }) .done(function(data) { console.log("%cAjaxGetPatDetails success |
 * ResponseData Is Below", "color:green;"); console.log(data); })
 * .fail(function( jqxhr, textStatus, error ) { console.log("%cAjaxGetPatDetails
 * Failed | ResponseError Is Below", "color:red;"); var err = textStatus + ", " +
...
/*
 * function billedUnbilledfun_ipd(patStatus,wardcode){ // alert("hello
 * "+patStatus) var crNo = $('#crNoInput').val(); var patStatus = patStatus; var
 * _mode = "AjaxGetDetails";
 * document.getElementsByName("sampleAreaCode")[0].value=wardcode; var
 * sampleAreaCode= wardcode;
 * 
 * var url =
 * "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode=" +
 * _mode + "&crNo=" + crNo +"&patStatusCode=" +patStatus+"&sampleAreaCode="
 ...
 function callUnBilledSection(UnbilledDtl){
	...
	"order": [[ 4, "desc" ]],
       ...
       api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
    	...
    	// alert("hel");
    		/*
			 * var currentOrder = ""; currentOrder=table.order()[6];
			 * alert(currentOrder); if(currentOrder==undefined)
			 * currentOrder=table.order()[0];
			 * 
			 */        /*
						 * "order": [[ 12, "desc" ], [ 23, "desc" ],[ 15, "desc" ] ,[
						 * 16, "desc" ],[ 17, "desc" ],[ 18, "desc" ], [ 19,
						 * "desc" ],[ 20, "desc" ],[ 10, "desc" ],],
						 */
    		        var currentOrder = "";
...
// strStatus = '<td id="testName_'+chk+'">'+billedDtl[i].testName+'<input
// type="hidden" id="testCodeId_'+chk+'" value="'+billedDtl[i].testCode+'"><img
// src="media/images/icon1.png" width="45px;" height="30px;" ><input
// type="hidden" id="isConfidentialId_'+chk+'"
// value="'+billedDtl[i].isConfidential+'"></td>';
				...
				// strform = '<td ><input class="'+billedDtl[i].requisitionNo+'
				// rowCheckBoxes" type="checkbox" name="checkboxName_'+chk+'"
				// id="checkboxId_'+chk+'" "><input type="hidden" id="totalId1"
				// value="'+len+'"><input type="hidden"
				// id="priorityAllCodeId_'+chk+'"
				// value="'+billedDtl[i].priorityAllCode+'"><input type="hidden"
				// id="patInstructId_'+chk+'"
				// value="'+billedDtl[i].patInstruct+'"> </td>';
					strform ='<td ><input value="'+billedDtl[i].requisitionDNo+'" class="'+billedDtl[i].requisitionNo+' rowCheckBoxes" type="checkbox" name="checkboxName_'+chk+'" id="checkboxId_'+chk+'" "><input type="hidden" id="totalId1" value="'+len+'"><input type="hidden" id="priorityAllCodeId_'+chk+'" value="'+billedDtl[i].priorityAllCode+'"><input type="hidden" id="patInstructId_'+chk+'" value="'+billedDtl[i].patInstruct+'"> </td>';
				...
				if(billedDtl[i].priorityAll=="Urgent")
					colorr="red";
				// <img src="/HISInvestigationG5/hisglobal/images/button_rf.png"
				// onclick="ShowRequistionForm()">
		...
		/*
		 * if(is_ins=="1" || is_ins=="2") { var msg=""; if(is_ins=="1")
		 * msg="Fasting Required:Yes("+is_ins_fastingtime+"
		 * "+is_ins_fastingtype+")"; else msg="Full Bladder
		 * Required:Yes("+is_ins_bladder+")";
		 * 
		 * 
		 * 
		 */
    				...
    				$('.rowCheckBoxes').off("click").on("click", function (e){
    					...
    					var is_ins_id="#"+ins_reqno+"is_inst";
    						...
    						if(is_ins=="1" || is_ins=="2")
    							...
    							var select = this; // save select element to
													// variable
    						     return swal({
    						...
    						// alert("hello");
    						/*
							 * var txt; var r = confirm("This Test has some
							 * Instructions which needs to be followed in a
							 * mandatory basis.\nFasting
							 * Required:Yes("+is_ins_fastingtime+"
							 * "+is_ins_fastingtype+")"); if (r == true) { txt =
							 * "You pressed OK!"; } else { isflg="0";
							 * this.checked=false; txt = "You pressed Cancel!"; }
							 */
    							}
...
// alert(totallen);
	...
	// alert(patientDtl)
	 	// alert("sampleNoConfigurationVal==" + sampleNoConfigurationVal);
...
function savemethodHit(listFinal,crNo,sampleNoVal,sampleNameVal,testNameVal){
	...
	$ .ajax({
			type : "POST",
			url :"/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode=AjaxSaveDetails",
			 async :false,
...
{ var list123 = "";
	...
	var autoGen = "";
// alert("tempSampleNo==" + tempSampleNo);
...
function saveBtnsAnimations(enableFlag, animateFlag){
	...
	// tippyLoadingUtlInstance.modifyValBtnDiv.setContent('Click to Modify
	...
	// alert("sxsxs");
		...
		// tippyLoadingUtlInstance.modifyValBtnDiv.setContent('Please select
		// atleast one Requisition to Modify');
		//
		enableRowCheckBoxes(true);
	...
	// tippyLoadingUtlInstance.modifyValBtnDiv.setContent('Please wait while
	// current Requisition is Validated');
...
$(document).ready(function() {
	$(".fancyBoxIFrame").fancybox({
		...
		iframe : {
			// Iframe template
			// tpl: '<iframe id="fancybox-frame{rnd}" name="fancybox-frame{rnd}"
			// class="fancybox-iframe" allowfullscreen allow="autoplay;
			// fullscreen" src=""></iframe>',
			preload : true,
			css : {
// 'width':$fancyboxWidth,
...
// alert("2+++")
	...
	var lowrange = document.getElementById("strTestHighValueId"+index).value;
	 ...
	 if(!isNaN(parseFloat(lowrange)) || !isNaN(parseFloat(lowrange)) )	// if(!refrange.includes("-
																		// - -
																		// -"))
																		// /^\d+$/
		{
...
function enableFields(chkObj,index,testcode){
	  ...
	  if(chkObj.checked){
		  ...
		  $("[id$='"+testcode+"']").each(function(){
			  if(document.getElementById("strTestResultId"+index+"#"+testcode)!=undefined){
		 ...
		 // alert($(chkObj).closest('tr').length);
		// $(chkObj).closest('tr').addClass('bg-success text-white')
		  $("[id$='tr"+testcode+"']").addClass('bg-success-light');
		  ...
		  // alert($("[id$='tr"+testcode+"']").length);
		  $("[id$='tr"+testcode+"']").removeClass('bg-success-light');
	  ...
	  function getPatDtl(){
	  	...
	  	// alert(234);
	  	  ...
	  	  if (document.getElementById("patCRNoId").value=="" && document.getElementById("patMobNoId").value=="")
	  		  ...
	  		  if (document.getElementById("patCRNoId").value!="" && document.getElementById("patCRNoId").value.length !=10){
	  		  ...
	  		  var urlNew = "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode+"&cr_no="+crNo+"&mob_no="+mobNo;
	  		  var objXHR = {url: urlNew, sync:true, postData: "", handleAs: "text",
	  	  	  load: function(data) 
	  	  		{
	  	  		res = data;
	  		...
	  		for(var i = 0; i < nodes1.length; i++){
	  		     nodes1[i].disabled = false;
	  			  	...
	  			  	document.forms[0].patGenderCode.value=val[1]; // set
																	// Gender.
	  			  	document.forms[0].strPatAge.value=val[2];  // Set Age.
	  			  	document.forms[0].strPatAgeUnit.value=val[3]; // Set Age
																	// Unit.
	  			  	document.forms[0].patGuardianName.value=val[4];  // Set
																		// Father/Spouse
																		// Name.
	  			  	document.forms[0].patMobileNo.value=val[5];  // Set
																	// Mobile
																	// number.
	  			  	document.forms[0].patAdd.value=val[6];  // Set Address.
	  			  	
	  			  	document.forms[0].patCRNo.value=val[7]; // Set CR Number In
															// case of search by
															// mobile number.
				...
				for(var i = 0; i < nodes1.length; i++){
				     nodes1[i].disabled = false;
					  	...
					  	document.forms[0].patGenderCode.value=val[1]; // set
																		// Gender.
					  	document.forms[0].strPatAge.value=val[2];  // Set Age.
					  	document.forms[0].strPatAgeUnit.value=val[3]; // Set
																		// Age
																		// Unit.
					  	document.forms[0].patGuardianName.value=val[4];  // Set
																			// Father/Spouse
					  	...
					  	document.forms[0].patCRNo.value=val[7]; // Set CR Number
																// In case of
																// search by
																// mobile
																// number.
			  			...
			  			// document.getElementById("patInfoDivId").innerHTML =
						// res;
					  	var nodes2 = document.getElementById("patDtlId").getElementsByTagName('*');
						for(var i = 0; i < nodes2.length; i++){
						     nodes2[i].disabled = true;
	...
	function validateRangeVal(obj,index){
	  ...
	  function generateComposition()
	  {
	  		localStorage.setItem("isPrintCalled","true");
	  	/*
		 * var options = { }; var pdf = new jsPDF('p', 'pt', 'a2');
		 * pdf.addHTML($("#PDFBODY"), 15, 15, options, function() {
		 * pdf.save('pageContent.pdf'); });
		 */
	  	  /* window.print(); */
...
/* function getEpisodeDetails(patCrNo){ */
function getEpisodeDetails(){
	...
	$('.patDetails').removeClass("d-none");
	$('#episodeDetailTable tbody').html("");
	
	if($('[name="patCrNo"]').val()!=""){
		var data ={
			patCrNo:$('[name="patCrNo"]').val(),
			patEpisodeCode:$('[name="patEpisodeCode"]').val(),
			patVisitNo:$('[name="patVisitNo"]').val(),
			hmode:"GET_EPISODE_LIST",
	...
	$.ajax({url:'/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt',
		...
		success:function(json){
			 //alert('episodedtl'+JSON.stringify(json));
			$('#episodeDetailTable tbody').html("");
			...
			$.each(json,function(indx, objjson){
			  ...
			  html+="<tr>";
			  pk=objjson["episodeCode"]+"_"+objjson["visitNo"]
			  html+="<td><input type='radio' name='selectedEpisodeDtl' value='"+pk+"' onclick='populateEpisode(this)'></td>";
			  html+="<td>"+objjson["departmentName"]+"</td>";
			  html+="<td>"+objjson["UnitName"]+"</td>";
			...
			if(flagDataFound==false){
				html="<tr>";
				  html+="<td colspan='4' class='text-center text-danger' style='font-weight: bold;'>No Episode details Found</td>";
				html+="</tr>";  
...
function populateEpisode(objRadio){
	var arr= $(objRadio).val().split("_");
	$('[name="patEpisodeCode"]').val(arr[0]);
	$('[name="patVisitNo"]').val(arr[1]);
	document.getElementById("newOfflineTestsListId").classList.remove("d-none");
}

/*
 * function ShowEpisodeTestDtl(){
 * document.getElementById("newOfflineTestsListId").classList.remove("d-none");
 * var _mode = "EPI_AJX_TEST_DTL"; $.ajax({ type: "POST", url:
 * "/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt?hmode="+_mode
 * +"&episode_code="+ episodecode +"&crNo=" + crNo + "&visitno=" + visitno ,
 * contentType: "html", async: false, success: function (data) {
 * document.getElementById("divtestdtl").innerHTML = data; } });
 *  }
 */


function ShowCheckboxTestDtl(){
	//alert("inside 34 ShowCheckboxTestDtl")

	if($('[name="patCrNum"]').val()!=""){
		var data ={
			patCRNo:$('[name="patCrNum"]').val(),
			patEpisodeCode:$('[name="patEpisodeCode"]').val(),
			patVisitNo:$('[name="patVisitNo"]').val(),
			hmode:"SBX_AJX_TEST_DTL"
		}
			$.ajax({url:'/HISInvestigationG5/new_investigation/NEWOfflineResultEntry.cnt',
			dataType: "json",
			data:data,
			...
			$('#onlineReqJson').val(JSON.stringify(json));	
			var testcodeArr = json.map(function(obj){  return obj['gnum_test_code'] })
				  $.fn.reverse = [].reverse;			
				 $("input[name=srrChk]").reverse().each( function () {
				       
				       if(testcodeArr.includes( $(this).val().split('^')[1] )){
				    	   $(this).click();  
				    	   }    
				       });		 
			}
