...
function openInvTrackingToCompare(crNo,testCode,groupCode,requisitionDNo,requisitionNo){
	crNo='<%=(String) session.getAttribute("patcrno")%>';
  //alert("hi")
  var isGroup="0";
...
//window.open(url,'_blank','scrollbars=1,directories=no, status=no,width=700, height=500,top=200,left=500');
...
@keyframes homeCycle { 0% {
	background-color: #3366CC;}25%{background-color:#003399;}50%{background-color:#6699FF
						...
						<td class="tdfonthead" width="5%">
							...
							<div align="left">
									<b> <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
										<a onclick="openInvTrackingToCompare('<%=voPatInv.getPatCrNo()%>','<%=voPatInv.getTestCode()%>','<%=voPatInv.getGroupcode()%>','<%=voPatInv.getRequisitionDNo()%>','<%=voPatInv.getRequisitionNo()%>');" >
...
<div id="dialogg" style="display: none;">
    <div>
        <iframe style="margin-top: 20px" width="2000" height="1960"  frameborder="0" id="frame"></iframe>
    </div>
</div>
