...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    ajaxreferraltestdata(searchValue);
});

//alert("radhe>>"+$('[name="hiddenCrNO"]').val())
if($('[name="hiddenCrNO"]').val()!= null && $('[name="hiddenCrNO"]').val()!= ""){
	
	ajaxreferraltestdata($('[name="hiddenCrNO"]').val());
...
if ($('#firstCombo').val() === 'CGHS') {
   ...
   //  $('.modal-body').hide();
 //  $('#myModal2').hide();
	$('#secondComboArea').css('display', 'none'); 
	$('#thirdComboArea').css('display', 'none'); 
	...
	$('#secondComboArea').css('display', 'block'); // Show the div
	$('#thirdComboArea').css('display', 'block'); 
...
$('#secondCombo').on('change', function() {
	...
	if ($(this).val() == 'government hospital') {
		 $('#thirdComboArea').hide();
	   }else {
	   	$('#thirdComboArea').show();
	       }
...
if(!child.opener)
 ...
 url= url+"?varSSOTicketGrantingTicket="+ gt;
 //alert(url);
	 ...
	 document.getElementById("cont1").style.display="none";
		...
		document.getElementById("clearId").style.display="";
					...
					document.getElementById("cont6").style.display="";
					 document.getElementById("cont8").style.display="";
...
var selectElement = document.getElementById("thirdCombo");
var selectedHospitalName = selectElement.options[selectElement.selectedIndex].value;
/* alert("selectedHospitalName>"+selectedHospitalName); */
// Set the selected hospital name in the hidden input field
document.getElementById("selectedHospitalName").value = selectedHospitalName;

	document.forms[0].submit();

	document.getElementById("cont3card1").style.display = "none";
	var element = document.getElementById('firstCombo');
	if (element) {
...
function saveref() {
   // alert("saveref>>");
    //alert( sessionStorage.getItem("lab_code"));
    var hiddenCrNO = document.getElementById("hiddenCrNO").value;
 
  ///  var labCode = document.getElementById("labCode").value;

    // refIdCheck
   
    document.getElementsByName('hmode')[0].value = "SAVEREFPROCESS";
...
function maktestinrowtable(testdata)
{
      // alert(testdata);      
       ...
       if(checktestalreadyraised(tests[2]))
       {
         ...
         if(tests[29] != null && tests[29] != 'undefined' && tests[29].includes('^'))
           {      
        //     alert("match");  
 ...
 */
 ...
 if(tests[5]=="S") // Sample or Slide Based
	...
	// cols += "<td>"+"<div align='left' style='display:none;height:18 ;border-left : 4px solid "+colorcodee+" ' class='"+grptyp1+"' >"+testr1+"</div></td>";
...
function ajaxreferraltestdata(patCrNo) {
	
    //alert("ajaxreferraltestdata>"+patCrNo);
    ///alert("saveref>>");
   // alert( sessionStorage.getItem("lab_code"));
     
  var _mode = "AJX_GETREFERRALTESTDATA";
    $.ajax({
        url: "/HISInvestigationG5/new_investigation/onlineRequisitionRaisingreq.cnt?hmode="+_mode+"&patCrNo="+patCrNo, sync:true,
        success: function (data) {
            // alert("data>>"+data);
            if (data == "1") {
                swal('', 'Patient Account Balance Going To Be Insufficient. Please Deposit Part Payment!!', 'warning');
                document.getElementById('automplete-testsearch').value = "";
            } else {
                populateTable(data);
            }
...
function ajaxhospitalname() {
    
    var hospdata = "";
    var flg = false;
    var _mode = "AJX_HOSPITALNAME";
  
    var objXHR = {
        url: "/HISInvestigationG5/new_investigation/onlineRequisitionRaisingreq.cnt?hmode=" + _mode,
        sync: true,
        load: function(data) {
           // alert("data>>" + data);
            hospdata = data;
           /// alert("hospdata>>" + hospdata);
            
       
            var hospitals = hospdata.split("\n");  
        	...
        	// alert("tocheck"+tocheck);
        	 ...
        	 if(tocheck[0]=="1")
        	 {
            	// alert("enter");
        	 			 ...
        	 			 /// alert("ref>>");       	 
        	 			$('#hiddenCrNO').val($('[name="patCrNo"]').val());
        	 			// alert("ref>>>"+$('[name="patCrNo"]').val());
                       //  alert("ref4"+$('#hiddenCrNO').val());
...
<input type="hidden" id="selectedHospitalName" name="selectedHospitalName">
<input type="hidden" id="hiddenCont3card1" name="hiddenCont3card1">
<input type="hidden" id="hiddenCont8card1" name="hiddenCont8card1">
<!-- <input type="checkbox" class="checkbox" name="checkboxGroup"> -->
...
<div class="container-fluid mt-2 mb-3" id="cont2">
        <div class="row p-2 align-items-center shadow rounded bg-white no-guttersNEW" id="cont3card1">
            <div class="col-md-3">
                Test Type
                <select id="firstCombo" class="custom-select">
                    <option value="CGHS">CGHS</option>
                    <option value="External">External</option>
                </select>
            </div>
            <div class="col-md-4" id='secondComboArea' >
                 Hospital Type
                <select id="secondCombo" class="custom-select">
                    <option value="empanelled ">Empanelled</option>
                    <option value="government hospital">Government Hospital/CGHS</option>               
                </select>
                
            </div>
            <div class="col-md-4" id='thirdComboArea'>
            Hospital Name
                <select id="thirdCombo" class="custom-select">
    ...
    <div id="cont8" class=" container-fluid mt-2 mb-3" >
							
<div id="cont8card1" class="row p-2 align-items-center shadow rounded bg-white no-guttersNEW">
    <div class="col-md-3 d-flex justify-content-center prevReqd-col">
        &emsp;
        <button style="background-color:#34495E; color:white;font-size:12px" type="button" class="btn btn-hover1 btn-sm">
            <a style="color:white" id="prevreqshww" class="iframee"><b>Previous Requisition</b></a>
        </button>
    </div>
    <div class="col-md-3 d-flex justify-content-center">
        <input type="text" id="searchInput" style="display:none;" class="form-control" placeholder="Search...">
    </div>
</div>
...
<div  id="cont4card1" class=" row p-2 align-items-center shadow rounded bg-white no-guttersNEW">
	...
	<button id="estimatebtn" style="background-color:green;color:white;font-size:12;display: none;" type="button" class="btn btn-sm" role="button"  >
...
<div class=" container-fluid mt-2 mb-3" id="cont5">
	...
	<div class=" row p-2 shadow rounded bg-white no-guttersNEW">
			...
			<div  class="col-12  col-md-10 rounded"><!-- col-md-10	nandini -->
				
				<div style="width:120%; background: #15658B;"   id="cont5card1" class="p-2  rounded accordion">
			...
			<div id="cont6" class="col-12 col-md-2 pl-2 rounded" style="margin-top: 2%;">               
...
<!-- <span class="badge badge-pill badge-info">Malaria</span>
		...
		<span class="badge badge-pill badge-success" style="font-weight:200;font-size:12">Q fever <span class="fdl-remove" onclick="$(this).parent().remove();"><a>x</a></span> </span>
		 ...
		 <span class="badge badge-primary">Primary</span>	 -->
			...
			<div  class="col-12   rounded"><!-- col-md-10	nandini -->
			
				<div style="width:100%; background: #15658B;"    id="cont6card1" class="p-2  rounded accordion">
				</div> 
				
				<div class=""  style="max-height: 350px !important; overflow-y:auto;">
					<table id="testtablenew" class="table  table-xs table-condensed table-hover table-responsive-xs order-list table-striped"  style="width:100%;font-size: small;" >
						<!-- <thead class="bg-white  rounded border-bottom border-primary" style=" position: -webkit-sticky; position: sticky; top: 0;">
							<tr>
								<th class="th-sm"width=25%>Test</th>
								<th class="th-sm" width=15%>Laboratory</th>
	 ...
	 <div class="row mt-4">
			<div class="col-md-12" style="font-size:1.1rem;">
				<div class="d-flex justify-content-center">
					<button type="button" class="btn btn-success " style="color:white" onclick="saveref()">
						Save <i class="fa fa-floppy-o" aria-hidden="true"></i>
					</button>
					&ensp;
					<button type="button" class="btn  btn-danger " style="color:white" onclick="cancel()">
...
<!-- <div class="modal fade modal-tall" id="myModalHorizontal" tabindex="-1"
    	  			...
    	  			while(itrAutogrp.hasNext()) { 
    					...
    					//  String test=testlab.split("#")[0];
    	  				...
    	  				if(voPatEpisodegrp.getTestCode()!=null && voPatEpisodegrp.getTestGroupCode()!=null && voPatEpisodegrp.getLabCode()!=null && voPatEpisodegrp.getTestGroupName()!=null )
    	  				{
    	  					...
    	  					//System.out.println("Test Name : "+ voPatEpisodegrp.getTestName());
    	  				%>
 ...
 <% } %>
	...
	<!-- Modal -->
       ...
       <input type="hidden" name="hiddenCrNO" id="hiddenCrNO" value="<%= session.getAttribute("patcrno") %>" />
      
       <input type="hidden" id="hospitalTypeValue" name="hospitalTypeValue">
         <input type="hidden" id="hospitalNameValue" name="hospitalNameValue">   
</html:form>
