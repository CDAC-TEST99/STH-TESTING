...
public class JakartaFTPWrapper extends FTPClient {
 ...
 you can use either code which is written above above or below code as ftp port 20 is used for the data transfer and port 21 is used for command and controlls 
     ftp.connect(InetAddress.getByName(host),14147); 
	 ...
	 public boolean connectAndLogin(final String host, final String userName, final String password) throws IOException, UnknownHostException, FTPConnectionClosedException {
	        boolean success = false;
	        this.connect(host,PropertiesHelper.getFTPPORT());
	        final int reply = this.getReplyCode();
	        if (FTPReply.isPositiveCompletion(reply)) {
