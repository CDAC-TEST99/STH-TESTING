	...
	public void doFilter(ServletRequest request, ServletResponse response,FilterChain chain) throws IOException, ServletException 
	{
		...
		{			
			if(HisUtil.getParameterFromHisPathXML("PERFORMANCE_FILTER")!=null && HisUtil.getParameterFromHisPathXML("PERFORMANCE_FILTER").equalsIgnoreCase("ON"))
			{
	            		...
	            		if (strURI.contains(HISSSOClientConfig.SSO_ST_SERVICE_URL)  || strURI.contains("HISDRDESK_MC/services/restful/patdata/digi") )
	        			{
	        			...
	        			else
	        			{
	        				...
	        				else
	        				{
	        					...
	        					if(strURI.contains("/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt") && hmode != null && hmode.equals("NEW1"))
	        					{
	        						chain.doFilter(request, response);
	        						return;
	        					}
