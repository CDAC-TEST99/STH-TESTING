...
function saveBtnsAnimations(enableFlag, animateFlag) {
	var savebtn = $('#valStatusSelectInput').val();
	//alert('savebtn '+savebtn)
if(savebtn!= "5")										//value changed from 4 to 5 by ashutoshk to allow  Modification for tests whose reports have been viewed
{
...
function sendToMachineCheckBox(data, type, row) {
  ...
  if (isMachineBasedTest == "1") {
   ...
   // alert("sndToMachineChkVal"+JSON.stringify(sndToMachineChkVal));
