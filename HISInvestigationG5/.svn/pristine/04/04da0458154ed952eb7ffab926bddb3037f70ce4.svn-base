...
public class InvestigationEssentialsamplecollDAO extends DataAccessObject implements InvestigationEssentialDAOi
{


	public InvestigationEssentialsamplecollDAO(JDBCTransactionContext _tx) { super(_tx); }
		...
		try
		{
				...
				if(vo.getSampleAreaCode()!=null)
				{
					...
					if(wardtype.equals("2"))
					{
					 ...
					 if(wardtype!=null && wardtype.equals("2"))
					 {
...
String condition4="";
				...
				if(vo.getWardCode()==null ||   vo.getWardCode().equals(""))
				{

//condition4=" and gnum_lab_code in ( SELECT gnum_column_value FROM gblt_role_seat_table_dtl p  where p.gbl_isvalid=1  AND  p.gnum_module_id = 15  AND p.gstr_table_name = 'HIVT_LABORATORY_MST'  AND p.gstr_column_name = 'GNUM_LAB_CODE' AND p.gnum_seatid = "+userVO.getUserSeatId()+"  AND p.gnum_hospital_code = "+userVO.getHospitalCode()+" )  AND GNUM_LAB_CODE IN ( SELECT GNUM_LAB_CODE FROM HIVT_LAB_COLLECTION_AREA_MST WHERE GNUM_ISVALID = 1 AND gnum_hospital_Code  = "+userVO.getHospitalCode()+" AND GNUM_AREA_CODE = "+vo.getSampleAreaCode()+" ) order by hivt_entry_date desc,hgnum_group_code";
					condition4=" and gnum_lab_code in (SELECT  gstr_column_value::numeric FROM usm.ummt_user_dataset_mst where gnum_isvalid=1 and gnum_dataset_id=124 /*Lab dataset INV*/  and gnum_module_id=15  and gnum_userid="+userVO.getUserSeatId()+"::numeric and gnum_hospital_code ="+userVO.getHospitalCode()+"::numeric )  AND GNUM_LAB_CODE IN ( SELECT GNUM_LAB_CODE FROM HIVT_LAB_COLLECTION_AREA_MST WHERE GNUM_ISVALID = 1 AND gnum_hospital_Code  = "+userVO.getHospitalCode()+" AND GNUM_AREA_CODE = "+vo.getSampleAreaCode()+" ) order by hivt_entry_date desc,hgnum_group_code";
				}
				else
				{

		//
					condition4=" and gnum_lab_code in (SELECT  gstr_column_value::numeric FROM usm.ummt_user_dataset_mst where gnum_isvalid=1 and gnum_dataset_id=124 /*Lab dataset INV*/  and gnum_module_id=15  and gnum_userid="+userVO.getUserSeatId()+"::numeric and gnum_hospital_code ="+userVO.getHospitalCode()+"::numeric ) and  EXISTS ( SELECT 1 FROM HIVT_COLLECTION_AREA_MST z WHERE   z.gnum_hospital_code="+userVO.getHospitalCode()+" and gnum_isvalid=1) order by hivt_entry_date desc,hgnum_group_code";			
				}
