...
public class TestMandRefMstDAO extends DataAccessObject implements TestMandRefMstDAOi
{
	...
	public void createTestMandRefold(TestMandRefMasterVO testmandref_VO, UserVO _UserVO)
	{
		String query = "";
	...
	public void createTestMandRef(TestMandRefMasterVO testmandref_VO, UserVO _UserVO) {
	    ...
	    try {
	    	System.out.println(" save ref range");
	        query = HelperMethodsDAO.getQuery(filename, queryKey);
	    } catch (Exception e) {
	        throw new HisApplicationExecutionException("Error loading query from properties file (filename: " + filename + ", queryKey: " + queryKey + "): " + e.getMessage());
	    }
	    ...
	    try {
	        ...
	        throw new HisApplicationExecutionException("Error while populating MAP for INSERT query: " + e.getMessage());
	    }

	    try {
	        HelperMethodsDAO.excecuteUpdate(super.getTransactionContext().getConnection(), query, populateMAP);
	    } catch (Exception e) {
	        // Log more details like the query, parameters, and the full stack trace
	        System.err.println("Error executing query: " + query);
	        System.err.println("Parameters: " + populateMAP.toString());
	        e.printStackTrace(); // Log the stack trace for debugging
	        
	        throw new HisDataAccessException("Error executing query using HelperMethodsDAO.excecuteUpdate: " + e.getMessage());
	    }
	}
	...
	public void updateTestMandRef(TestMandRefMasterVO testmandref_VO, UserVO _UserVO)
	{
		...
		Map populateMAP = new HashMap();
		Sequence sq = new Sequence();
		System.out.print("updateTestMandRef>>global");
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
