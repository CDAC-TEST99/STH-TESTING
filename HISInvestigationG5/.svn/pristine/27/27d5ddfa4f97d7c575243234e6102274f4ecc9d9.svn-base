...
public class SampleCollectionCumAcceptanceDAO extends DataAccessObject {
	...
	public SampleCollectionCumAcceptanceDAO(JDBCTransactionContext _tx)
	{
		...
		public  List<SampleCollectionCumAcceptanceVO> getSampleCollectionArea(UserVO _UserVO)
		{
			...
			populateMAP.put(sq.next(), _UserVO.getHospitalCode());
			System.out.println("_UserVO.getUserSeatId()>>>"+_UserVO.getSeatId());
		...
		//	cal.add(Calendar.DATE, -7);
			...
			if(objSampleCollectionVO.getFromDate() == null)
				objSampleCollectionVO.setFromDate(dateBefore7Days);
			
			objSampleCollectionVO.getEntryDate();
			
			String condition1=" and HRGNUM_PUK= "+objSampleCollectionVO.getPatCRNo();
			System.out.println("getPatCRNo()>>mai hu jeeyan>"+objSampleCollectionVO.getPatCRNo());
			//String condition2=" AND (TRUNC(hivdt_entry_date) >= TRUNC(TO_DATE(\'"+objSampleCollectionVO.getEntryDate()+"\','DD-Mon-YYYY'))) "
				       //  	+ " AND (TRUNC(hivdt_entry_date) <= TRUNC(TO_DATE(\'"+objSampleCollectionVO.getEntryDate()+"\','DD-Mon-YYYY')))";
	
			//String condition3 = "AND TRUNC(HGNUM_VISIT_DATE) = TRUNC(TO_DATE(\'"+objSampleCollectionVO.getFromDate()+"\','DD-Mon-YYYY')) ";
		...
		//	String condition2=" AND (TRUNC(HIVDT_REQUISITION_DATE) >= TRUNC(TO_DATE(\'"+dateBefore7Days+"\','DD-Mon-YYYY')))  AND (TRUNC(HIVDT_REQUISITION_DATE) <= TRUNC(TO_DATE(SYSDATE,'DD-Mon-YYYY')))";
			String condition2=" AND (TRUNC(HIVDT_REQUISITION_DATE) >= TRUNC(TO_DATE(\'"+dateBefore7Days+"\','DD-Mon-YYYY')))";  
		...
		/////	populateMAP.put(sq.next(), objSampleCollectionVO.getPatCRNo());
			...
			try
			{
				...
				query = "select patCategoryCode ,  patCRNo , patname , patage , patstatus , patdeptname , patunitname ,patOrderByDoc, patCategory, patVisitDate,  patGenderCode , " + 
						"string_agg(patCRNo||'#'||requisitionNo||'#'||labCode||'#null#'||'!!!'||'#'||testCode , ',')  as  requisitionNo\r\n" + 
						" from ( " + query ;
			...
			try
			{
				...
				query += " ) group by patCategoryCode ,  patCRNo , patname , patage , patstatus , patdeptname , patunitname ,patOrderByDoc ,patCategory, patVisitDate, patGenderCode  " ;
		...
		public void updateRequisitionDtl(SampleCollectionCumAcceptanceVO voSample,UserVO userVO)
		{
			try
			{
				...
				strProc.addInParameter(38,Types.VARCHAR,"101");
				strProc.addInParameter(39,Types.VARCHAR,null);
		...
		public void saveSampleDetail(SampleCollectionCumAcceptanceVO inv_SampleCollectionVO, UserVO _userVO) {
           ...
           String seatId = _userVO.getUserId();
