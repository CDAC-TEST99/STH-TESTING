...
public class InvResultValidationDAO
  extends DataAccessObject
{
  public InvResultValidationDAO(JDBCTransactionContext _tx) { super(_tx); }
  ...
  public List<ResultEntryVO> setPatientResultValidationEssentials(ResultEntryVO InvResultEntryVO, UserVO _UserVO) {
    ...
    if (InvResultEntryVO.getNewEntry() != null && InvResultEntryVO.getNewEntry().equals("2")) {
      ...
      reqDtlValue = " and hivnum_reqdtl_status in (28) ";
    } else {
    ...
    try {
      ...
      if (rs.next())
      {
        ...
        for (int i = 0; i < valueObjects.length; i++)
        {
          ...
          {
            if (((ResultEntryVO)lstInvResultValidationVO.get(i)).getReportAvailableAfter().equals("2"))
            {
              
              finalLststInvResultValidationVO.add((ResultEntryVO)lstInvResultValidationVO.get(i));
    ...
    return finalLststInvResultValidationVO;
  }
  ...
  public void insertResultValidationDtlInJobWorkorderData(ResultEntryVO voResultEntry, UserVO _UserVO) {
    ...
    try {
      ...
      if (voResultEntry.getPatVisitNo() != null && 
        voResultEntry.getPatVisitNo().equals("null"))
  ...
  public List<ResultEntryVO> getdata( String hospcode) {
	    ...
	    try{   
	      ...
	      if (rs.next())
	      {
	        ...
	        //   if(lstInvResultValidationVO.get(i).getRequisitionDNo().equals("961011000119101110001001"))
	            finalLststInvResultValidationVO.add((ResultEntryVO)lstInvResultValidationVO.get(i));
  ...
  public String checkisparmeterexist(ResultEntryVO resultEntryVO,String testcode,String parametercode,UserVO _UserVO) {
	    String query = "";
  ...
  public void insertResultEntryDtl_textareaeditor(ResultEntryVO voResultEntry, UserVO _UserVO,String testcode,String paracode,String defaultval) {
	    ...
	    try {
	      String resultEntryvalue = defaultval==null?"--":defaultval ;
	      resultEntryvalue = resultEntryvalue.replace("&lt;", "&amp;lt;");
	      resultEntryvalue = resultEntryvalue.replace("&gt;", "&amp;gt;");


	      
	      String value = resultEntryvalue.equals("") ? "--" : resultEntryvalue;
	      ...
	      populateMAP.put(sq.next(), _UserVO.getHospitalCode());
	      populateMAP.put(sq.next(), _UserVO.getSeatId());
	      populateMAP.put(sq.next(), value==null?"--":value);
	      populateMAP.put(sq.next(), paracode);
	      populateMAP.put(sq.next(), "1");
