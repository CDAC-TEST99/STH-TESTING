...
import new_investigation.masters.controller.data.LocalTestMandRefMstDATA;
...
public class LocalTestMandRefMstUTIL implements MasterInterface
{
...
/*
	...
	public static boolean saveLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
...
*/
		...
		if(testmandref_fb.getNumberOfRow().equals(""))
			if(testmandref_fb.getNumberOfRowPresent()!=null && testmandref_fb.getNumberOfRowPresent().equals(""))
			rowvalue=1;
		...
		int i=0;
		List<LocalTestMandRefMasterVO> lstLocalTestMandRefVO=new ArrayList<LocalTestMandRefMasterVO>();

		if(testmandref_fb.getNumberOfRow()!=null && testmandref_fb.getNumberOfRowPresent()!=null && !testmandref_fb.getNumberOfRow().equals("") && !testmandref_fb.getNumberOfRowPresent().equals("") && testmandref_fb.getFromGlobal().equals("0"))
		{
		...
		rowvalue=i+(Integer.parseInt(testmandref_fb.getNumberOfRow())-Integer.parseInt(testmandref_fb.getNumberOfRowPresent()));
			...
			for(;i<rowvalue;i++)
			{
				LocalTestMandRefMasterVO testmandref_VO = new LocalTestMandRefMasterVO();
				...
				lstLocalTestMandRefVO.add(testmandref_VO);
			}
			
			LocalTestMandRefMstDATA.saveLocalTestMandRef(lstLocalTestMandRefVO, userVO);
	...
	public static boolean fetchCheckListLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			mp=LocalTestMandRefMstDATA.fetchCheckListLocalTestMandRef(testmandref_VO, userVO);
	...
	public static boolean fetchLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		LocalTestMandRefMasterVO testmandref_VO = new LocalTestMandRefMasterVO();
		try
		{
			...
			mp=LocalTestMandRefMstDATA.fetchLocalTestMandRef(testmandref_VO, userVO);
			WebUTIL.setMapInSession(mp, _request);
	...
	public static boolean savemodifyLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			lstOld=LocalTestMandRefMstDATA.fetchOldLocal(testmandref_fb.getTestCode(),testmandref_fb.getParameterCode(), userVO);
			...
			{
				if(testmandref_fb.getNumberOfRowPresent()!=null && !testmandref_fb.getNumberOfRowPresent().isEmpty())
					rowvalue=Integer.parseInt(testmandref_fb.getNumberOfRowPresent());
				else
					rowvalue=1;
			}
			...
			if(testmandref_fb.getSeqNo()!=null && testmandref_fb.getSeqNo().length>0)
			...
			if(testmandref_fb.getSeqNo()!=null)
...
/*	for(int i=0; i < list1.size(); i++){
			...
			*/
			...
			List<LocalTestMandRefMasterVO> new_lstLocalTestMandRefVO=new ArrayList<LocalTestMandRefMasterVO>();
			List<LocalTestMandRefMasterVO> modify_lstLocalTestMandRefVO=new ArrayList<LocalTestMandRefMasterVO>();
			...
			for(i=0;i<oldrow;i++)
			{
				LocalTestMandRefMasterVO testmandref_VO = new LocalTestMandRefMasterVO();
				  ...
				  /*modify */
						...
						modify_lstLocalTestMandRefVO.add(testmandref_VO);
			...
			for(i=oldrow;i<rowvalue;i++)
			{
				LocalTestMandRefMasterVO testmandref_VO = new LocalTestMandRefMasterVO();
				  ...
				  /*add */  
					...
					new_lstLocalTestMandRefVO.add(testmandref_VO);
			...
			if(new_lstLocalTestMandRefVO.size()>0)
				System.out.print("new_lstLocalTestMandRefVO.size(1)"+new_lstLocalTestMandRefVO.size());
			LocalTestMandRefMstDATA.saveLocalTestMandRef(new_lstLocalTestMandRefVO, userVO);
			
			if(deleteSeqList.size()>0)
			LocalTestMandRefMstDATA.deleteLocalTestMandRef(deleteSeqList,testmandref_fb.getTestCode(),testmandref_fb.getParameterCode(), userVO);
			
			if(modify_lstLocalTestMandRefVO.size()>0)
				System.out.print("new_lstLocalTestMandRefVO.size(new_lstLocalTestMandRefVO)"+new_lstLocalTestMandRefVO.size());
			System.out.print("Save Range ");
			LocalTestMandRefMstDATA.savemodifyLocalTestMandRef(modify_lstLocalTestMandRefVO, userVO);

			objStatus.add(Status.DONE, "Data Save Successfully", "");
	...
	public static boolean fetchdisplaydataLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			testmandref_listVO=LocalTestMandRefMstDATA.fetchdisplaydataLocalTestMandRef(testmandref_VO, userVO);
	...
	public static boolean reFetchCheckListLocalTestMandRef(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
	...
	public static boolean fetchParameterCombo(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			mp=LocalTestMandRefMstDATA.fetchParameterCombo(testmandref_VO, userVO);
			WebUTIL.setMapInSession(mp, _request);
	...
	public static boolean fetchCombo(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		HttpSession session= _request.getSession();
		LocalTestMandRefMasterVO testmandref_VO = new LocalTestMandRefMasterVO();
		try
		{
			...
			mp=LocalTestMandRefMstDATA.fetchCombo(testmandref_VO, userVO);
			WebUTIL.setMapInSession(mp, _request);
	...
	public static StringBuffer getLabTest(LocalTestMandRefMstFB objFB, HttpServletRequest request, String labCode,String testCode)
	{
		...
		try
		{
			...
			lstUnit = LocalTestMandRefMstDATA.getLabSample(testmandref_VO,userVO);
	...
	public static boolean getValues(LocalTestMandRefMstFB testmandref_fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
