...
//masterObj.getViewHeader()
	...
	public static void LISTDATA(HttpServletRequest request, TransInterface masterObj, String cnt_page){
		...
		try {
			...
			if(masterObj.getMenuOption().equalsIgnoreCase("Tiles")) {
				request.setAttribute("buttonConfigData1",hlp.getButtonLayOut1(masterObj.getUserDefinedButtons(), masterObj.getMenuOption(), masterObj.getButtonConfiguration(), masterObj.getMinPanelButton(), masterObj.getRecord_per_page(), vo, masterObj.getDbButtons()));
				request.setAttribute("buttonConfigData2",hlp.getButtonLayOut2(masterObj.getUserDefinedButtons(), masterObj.getMenuOption(), masterObj.getButtonConfiguration(), masterObj.getMinPanelButton(), masterObj.getRecord_per_page(), vo, masterObj.getDbButtons()));
			}			
	...
	public static void BUTTON(HttpServletRequest request, HttpServletResponse response, TransInterface masterObj){		
		...
		try{
			...
			if(request.getParameter("mode").equals("1")){				
				out.println(hlp.getButtonLayOut1(masterObj.getUserDefinedButtons(), masterObj.getMenuOption(), masterObj.getButtonConfiguration(), masterObj.getMinPanelButton(), masterObj.getRecord_per_page(), vo, masterObj.getDbButtons()));
			} else if(request.getParameter("mode").equals("2")){				
				out.println(hlp.getButtonLayOut2(masterObj.getUserDefinedButtons(), masterObj.getMenuOption(), masterObj.getButtonConfiguration(), masterObj.getMinPanelButton(), masterObj.getRecord_per_page(), vo, masterObj.getDbButtons()));				
			}
	...
	public static void LISTPAGE(HttpServletRequest request, HttpServletResponse response, TransInterface masterObj){
		...
		try {		
			...
			bo = new GenericBO();
			bo.LISTPAGE(vo, masterObj.getRecord_per_page(), masterObj.getPage_per_block(), "LISTPAGE", masterObj.getMainQuery(vo.getCombo()),masterObj.getOrderBy());			
			String str = "";
			...
			HisException eObj = new HisException(masterObj.getMasterName(), "GenericData--1>LISTPAGE()", vo.getStrMsgString());
			out.println("ERROR####Application Error [ERROR ID : " + eObj.getErrorID() + "], Contact System Administrator! ");
			eObj = null;
		} finally {
			htSession = null;
	...
	public static void DEFAULT(HttpServletRequest request, HttpServletResponse response, TransInterface masterObj) {
		...
		try{
			...
			bo = new GenericBO();
			bo.LISTPAGE(vo, masterObj.getRecord_per_page(), masterObj.getPage_per_block(), "DEFAULT", masterObj.getMainQuery(vo.getCombo()),masterObj.getOrderBy());			
			String str = "";
	...
	public static void DELETE(HttpServletRequest request, HttpServletResponse response, TransInterface masterObj) {				
		...
		try {
			...
			vo.setSession(request.getSession());
			String p = hlp.generateData(vo, masterObj.getColumnHeader(), masterObj.getPage_per_block(), 
					masterObj.getMasterName(), masterObj.getSearchField(), masterObj.getRecord_per_page(), 
					masterObj.getComboHeader(), masterObj.getComboQuery(), strData, 
					TransInterface.nextprevcolor, masterObj.getNo_of_combo(), masterObj.getHeaderNote(), 
					masterObj.getFooterNote(), masterObj.getRowStatus(), masterObj.getButtons(), 
