...
public class GenericData {
	...
	public static void LISTPAGE(HttpServletRequest request,
		...
		try {
			...
			HisException eObj = new HisException(masterObj.getMasterName(),
					"GenericData--2>LISTPAGE()", vo.getStrMsgString());
	...
	public static void REPORTDATA(HttpServletRequest request,
			HttpServletResponse response, MasterInterface masterObj) {
		GenericVO vo = null;
		GenericBO bo = null;
		GenericHLP hlp = null;
		...
		PrintWriter out = null;
		String hospCode=InvestigationConfig.HOSPITAL_CODE ;
		try {
			...
			if(vo.getStrRepeat()!=null && vo.getStrRepeat().equals("yes")){
				...
				bo.getReports(vo, masterObj.getMainQuery(vo.getCombo()), masterObj
							.getRecord_per_page(), masterObj.getPage_per_block(), request.getParameter("strShowAllData"));
				
			if (vo.getStrMsgType() != "1") {
				hlp = new GenericHLP();
				out.print(hlp.getReports(vo, masterObj.getComboHeader(),
						masterObj.getRecord_per_page(), masterObj
								.getPage_per_block(), masterObj
								.getColumnHeader(),request.getParameter("strShowAllData"),masterObj.getColsWidth(),vo.getStrReportGroupBy()));
