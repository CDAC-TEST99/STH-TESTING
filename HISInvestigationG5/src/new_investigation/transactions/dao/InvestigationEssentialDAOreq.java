...
public class InvestigationEssentialDAOreq extends DataAccessObject implements InvestigationEssentialDAOi {
	public InvestigationEssentialDAOreq(JDBCTransactionContext _tx) {
		super(_tx);
	}

	public Inv_RequisitionRaisingPatientVO getInvRaisingPatDetails(Inv_RequisitionRaisingPatientVO patVO,
			UserVO _UserVO) {
		Inv_RequisitionRaisingPatientVO invReqRaisingVO;
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
	...
	// getReferralPatDetails

	public Inv_RequisitionRaisingPatientVO getReferralPatDetailsold(Inv_RequisitionRaisingPatientVO patVO,
			UserVO _UserVO) {
		Inv_RequisitionRaisingPatientVO invReqRaisingVO;
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		String queryKey = "SELECT.REFERRAL.PATIENT_DTLS";
		Sequence sq = new Sequence();
	...
	public List<Map<String, String>> getReferralPatDetails(Inv_RequisitionRaisingPatientVO patVO, UserVO _UserVO) {
		...
		System.out.println("AJX_GETREFERRALTESTDATA DAO>");
		...
		try {
			...
			throw new HisDataAccessException("Error fetching query from property file: " + e);
		}
		...
		try {
			...
			while (rs.next()) {
				...
				row.put("refValidity", rs.getString("ref_validity"));
				row.put("hospName", rs.getString("hosp_name"));
				row.put("unitName", rs.getString("hgstr_unitname"));
				row.put("deptName", rs.getString("gstr_dept_name"));
				row.put("componentName", rs.getString("hstr_component_name"));
				row.put("qtyAllowed", rs.getString("hnum_no_of_qty_allowed"));
				row.put("qtyConsumed", rs.getString("hnum_no_of_qty_consumed"));
				row.put("episodecode", rs.getString("hrgnum_episode_code"));
				row.put("visitno", rs.getString("hrgnum_visit_no"));
				row.put("fromHospCode", rs.getString("gnum_hospital_code"));
				row.put("cghsCode", rs.getString("referral_cghs_code"));
				row.put("entryDate", rs.getString("gdt_entry_date"));
				row.put("validUpto", rs.getString("hdt_valid_upto"));
				//erd.hstr_ref_id ,erd.hstr_ref_seq_num 
				row.put("referralid", rs.getString("hstr_ref_id"));
				row.put("seqNo", rs.getString("hstr_ref_seq_num"));
				row.put("labCode", rs.getString("lab_code"));
			...
			throw new HisDataAccessException("Error executing query: " + e);
	...
	public List<Map<String, String>> gethospitalname(Inv_RequisitionRaisingPatientVO patVO, UserVO _UserVO) {
		...
		ResultSet rs = null;
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		String queryKey = "SELECT.HOSPITAL_NAME_COMBO";
		Sequence sq = new Sequence();
	...
	//	populateMAP.put(sq.next(), _UserVO.getHospitalCode());
		...
		try {
			...
			while (rs.next()) {
				///ad_city_id as cityId,empled_hospital_id as empled_hospId, hosp_name as hospitalName
				Map<String, String> row = new HashMap<>();
				row.put("ad_city_id", rs.getString("cityId"));
				row.put("empled_hospital_id", rs.getString("empled_hospId"));
				row.put("hosp_name", rs.getString("hospitalName"));
				tableRows.add(row);
			}
		} catch (Exception e) {
			throw new HisDataAccessException("Error executing query: " + e);
	...
	public List<LabTestVO> searchLabWiseTestDtls(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition3 = "and EXISTS (SELECT 1 FROM HIVT_TEST_GROUP_INFO_MST where hgnum_group_code ="
				+ searchVO.getSearchTestGroupName()
				+ " AND gnum_lab_code=a.gnum_labcode AND gnum_test_code=a.gnum_test_code and gnum_hospital_Code=a.gnum_hospital_Code and gnum_is_valid=1)";
		...
		if (searchVO.getPatAdmNo() == null || !searchVO.getPatAdmNo().equals("-1")) {
			deskcalllabhide = " and a.gnum_labcode not in (select nvl(n.gnum_lab_code,'0') from hivt_laboratory_mst n where n.gnum_lab_code=a.gnum_labcode and n.gnum_isvalid=1 and n.gnum_hospital_code="
					+ _UserVO.getHospitalCode() + "  and n.gnum_islabhidefromdesk=1)";
		}
		...
		try {
			...
			if (searchVO.getTestSearchKeyword().equals("") && searchVO.getSearchTestName() != null
					&& !searchVO.getSearchTestName().equals(""))
	...
	public List<LabTestVO> searchBookMark(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition4 = "and EXISTS (SELECT 1 from hivt_test_mst where GNUM_TEST_CODE =  a.GNUM_TEST_CODE AND UPPER(GNUM_TEST_CODE) LIKE '%'||UPPER(TRIM('"
				+ searchVO.getSearchTestName() + "'))||'%')";
		String condition5 = "   AND EXISTS (SELECT 1 FROM hivt_test_mst WHERE gnum_test_code = a.gnum_test_code AND  gnum_test_code in("
				+ searchVO.getTestCode()
				+ "0)) and exists (select 1 from hivt_laboratory_mst where gnum_lab_code=a.gnum_labcode and   gnum_lab_code in("
				+ searchVO.getLabCode() + "0) ) ";
	...
	public List<LabTestVO> searchTestGroup(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition1 = "and EXISTS (SELECT 1 FROM HIVT_TEST_GROUP_INFO_MST where hgnum_group_code ="
				+ searchVO.getSearchTestGroupName() + " AND gnum_lab_code=" + searchVO.getSearchLabName()
				+ " AND gnum_hospital_Code=a.gnum_hospital_Code and gnum_is_valid=1)";
		String condition2 = "and EXISTS (SELECT 1 FROM HIVT_TEST_GROUP_INFO_MST where hgnum_group_code ="
				+ searchVO.getSearchTestGroupName()
				+ " AND gnum_hospital_Code=a.gnum_hospital_Code and gnum_is_valid=1)";
		...
		try {
			if (searchVO.getSearchTestGroupName() != null && !searchVO.getSearchTestGroupName().equals("")
					&& searchVO.getSearchLabName() != null && !searchVO.getSearchLabName().equals(""))
				query = String.valueOf(query) + condition1;
			if (searchVO.getSearchTestGroupName() != null && !searchVO.getSearchTestGroupName().equals(""))
				query = String.valueOf(query) + condition2;
			query = String.valueOf(query) + orderBy;
	...
	public void insertRequisitionDtl(InvestigationRequistionVO voLabTest, UserVO _userVO, String priorityAll,
			RequistionHeaderVO voReqHeader) {
		String sampleNo = "";
		String errorMsg = "";
		...
		System.out.print("voLabTest.getHospName()>6>>" + voLabTest.getHospName());
		try {
			...
			System.out.print("voLabTest.getHospName()>>1>" + voLabTest.getHospName());
			...
			Procedure strProc = new Procedure("pkg_inv_dml.proc_insert_hivt_requisition_dtl_req_new");
			strProc.addInParameter(1, 12, "3");
			strProc.addInParameter(2, 12, _userVO.getHospitalCode());
			strProc.addInParameter(3, 12, voLabTest.getStrRequsitionDNo());
			strProc.addInParameter(4, 12,
					(voLabTest.getStrIsAppointment() == null) ? "0" : voLabTest.getStrIsAppointment());
			strProc.addInParameter(5, 12, voLabTest.getStrLabCode());
			strProc.addInParameter(6, 12, voLabTest.getStrTestCode().toString());
			strProc.addInParameter(7, 12,
					(voLabTest.getStrIsConfidential() == null) ? "0" : voLabTest.getStrIsConfidential().toString());
			...
			strProc.addInParameter(96, 12, voReqHeader.getIs_dead() == null ? "" : voReqHeader.getIs_dead());
			strProc.addInParameter(97, 12, voReqHeader.getIs_mlc() == null ? "" : voReqHeader.getIs_mlc());
			strProc.addInParameter(98, 12, voReqHeader.getIs_newborn() == null ? "" : voReqHeader.getIs_newborn());
			strProc.addInParameter(99, 12, voReqHeader.getIs_pregnant() == null ? "" : voReqHeader.getIs_pregnant());
			strProc.addInParameter(100, 12,
					voReqHeader.getIs_vip() == null || voReqHeader.getIs_vip().equals("null") ? ""
							: voReqHeader.getIs_vip());
			/// voLabTest.getHospName()

			String chiefcode = "";
			String chiefname = "";

			if (voReqHeader.getChief_complaints_code() != null && voReqHeader.getChief_complaints_name() != null) {
				chiefcode = voReqHeader.getChief_complaints_code()[0];

				chiefname = voReqHeader.getChief_complaints_name()[0];
			...
			if (voReqHeader.getDiagnosis_code() != null && voReqHeader.getDiagnosis_name() != null) {

				diagnosiscode = voReqHeader.getDiagnosis_code()[0];

				diagnsisname = voReqHeader.getDiagnosis_name()[0];

			}

			strProc.addInParameter(101, 12, chiefcode);
			strProc.addInParameter(102, 12, chiefname);
			strProc.addInParameter(103, 12, diagnosiscode);
			strProc.addInParameter(104, 12, diagnsisname);
			strProc.addInParameter(105, 12,
					voReqHeader.getRequisitionwiseno() == null ? "" : voReqHeader.getRequisitionwiseno());
			strProc.addInParameter(106, 12,
					voReqHeader.getInstruction_testwise() == null ? "" : voReqHeader.getInstruction_testwise());
			strProc.addInParameter(107, 12, voReqHeader.getIs_unknown() == null ? "" : voReqHeader.getIs_unknown());
			strProc.addInParameter(108, 12, voLabTest.getHospName());
			if (voLabTest.getHospName() != null) {
				strProc.addInParameter(109, 12, "Empanelled");
				strProc.addInParameter(110, 12, "External");

			} else {

				strProc.addInParameter(109, 12, "Government Hospital");
				strProc.addInParameter(110, 12, "CGHS");

			}

			strProc.addInOutParameter(111, 12, "");
	...
	public List<String> saveReferralDetailsData( List<Inv_RequisitionRaisingPatientVO> patVOList, InvestigationRequistionVO voReq,UserVO _userVO) throws Exception {
		List<String> patCRNoList = new ArrayList<>();
		String error = "";
		String seatId = _userVO.getSeatId();
		...
		try {
			 ...
			 for( Inv_RequisitionRaisingPatientVO  patVO:patVOList) {
				 ...
				 if(i < 10)
					...
					String requisitionDNo = voReq.getStrReqNo() + iVal;
			           
			        
			           voReq.setStrRequsitionDNo(requisitionDNo);
				 

					Procedure strProc = new Procedure("pkg_inv_dml.proc_insert_hivt_reqisition_referral");
					strProc.addInParameter(1, 12, "1");
					
					strProc.addInParameter(2, 12, patVO.getHospName());
					strProc.addInParameter(3, 12, patVO.getTestName());
				  
					strProc.addInParameter(4, 12, patVO.getCghsTestCode());
					strProc.addInParameter(5, 12, patVO.getPatCRNo());
					System.out.println("patVO.getRefId()>>"+patVO.getRefId());
					strProc.addInParameter(6, 12, patVO.getRefId());
					strProc.addInParameter(7, 12, _userVO.getHospitalCode());
					strProc.addInParameter(8, 12, firstPart);
					strProc.addInParameter(9, 12, patVO.getPatepisodecode());
					strProc.addInParameter(10, 12, patVO.getVisitNo());
					strProc.addInParameter(11, 12, voReq.getStrRequsitionDNo());
					System.out.println("voReq.getStrRequsitionDNo()>>"+voReq.getStrRequsitionDNo());
					strProc.addInParameter(12, 12, voReq.getRequisitionwiseno());
					strProc.addInParameter(13, 12, voReq.getStrReqNo());
				///	System.out.println("patVO.getStrReqNo()>>" + patVO.getStrReqNo());
					strProc.addInParameter(14, 12, patVO.getPatDepartment());
					System.out.println("patVO.getPatDepartment()>>" + patVO.getPatDepartment());
					strProc.addInParameter(15, 12, patVO.getPatdeptunit());
					System.out.println("patVO.getPatdeptunit()>>" + patVO.getPatdeptunit());
					
					strProc.addInParameter(16, 12, patVO.getConsumedNo());
					System.out.println("patVO.getConsumedNo()>>" + patVO.getConsumedNo());
					strProc.addInParameter(17, 12, patVO.getSeqNo());
					System.out.println("patVO.getSeqNo()>>" + patVO.getSeqNo());
					strProc.addInParameter(18, 12, patVO.getReferralId());
					System.out.println("patVO.getReferralId()>>" + patVO.getReferralId());
					strProc.addOutParameter(19, 12);
				
					//setStrReqNo
					strProc.execute(getTransactionContext().getConnection());
					error = (String) strProc.getParameterAt(19);
					//System.out.println(" error" + error);
					//String str = error;
					...
					if(error != null && error.trim().length() > 0) {
					 ...
					 patCRNoList.add(patVO.getPatCRNo());
					i = i+1;
			 }
	...
	public List<String> saveReferralDetailsDataheaderold(Inv_RequisitionRaisingPatientVO patVO, RequistionHeaderVO voReqHeader,UserVO _userVO) throws Exception {
		String error = "";
		String seatId = _userVO.getSeatId();
		String[] parts = seatId.split("\\^");
		String firstPart = parts[1];
		...
		try {
			Procedure strProc = new Procedure("pkg_inv_dml.proc_insert_hivt_reqisition_referral_header");
			strProc.addInParameter(1, 12, "1");
			
			strProc.addInParameter(2, 12, patVO.getHospName());
			strProc.addInParameter(3, 12, patVO.getTestName());
		//	strProc.addInParameter(4, 12, patVO.getSeqNo());
	
			/*
			 * if (patVO.getConsumedNo() == null || patVO.getConsumedNo().isEmpty()) {
			 * strProc.addInParameter(5, 12, "0");
			 * 
			 * } else {
			 * 
			 * strProc.addInParameter(5, 12, patVO.getConsumedNo());
			 * 
			 * }
			 */

			//strProc.addInParameter(6, 12, patVO.getQty());
			strProc.addInParameter(4, 12, patVO.getCghsTestCode());
			strProc.addInParameter(5, 12, patVO.getPatCRNo());
			System.out.println("patVO.getRefId()>>"+patVO.getRefId());
			strProc.addInParameter(6, 12, patVO.getRefId());
			...
			strProc.addInParameter(9, 12, patVO.getPatepisodecode());
			strProc.addInParameter(10, 12, patVO.getVisitNo());
			strProc.addInParameter(11, 12, voReqHeader.getRequisitionNumber());
			System.out.println("voReqHeader.getRequisitionNumber()()>>"+voReqHeader.getRequisitionNumber());
			strProc.addInParameter(12, 12, voReqHeader.getRequisitionwiseno());
			System.out.println("getRequisitionwiseno()()>>"+voReqHeader.getRequisitionwiseno());
			
			strProc.addInParameter(13, 12, patVO.getPatDepartment());
			strProc.addInParameter(14, 12, patVO.getPatdeptunit());
			
			strProc.addOutParameter(15, 12);
		
			
			strProc.execute(getTransactionContext().getConnection());
			error = (String) strProc.getParameterAt(15);
		...
		//	System.out.println(" error" + error);
			...
			throw new HisDataAccessException("No Record Found");
...
public List<String> saveReferralDetailsDataheader(Inv_RequisitionRaisingPatientVO patVO, RequistionHeaderVO voReqHeader, UserVO _userVO) throws SQLException {
    String error = "";
    String seatId = _userVO.getSeatId();
    String[] parts = seatId.split("\\^");
    String firstPart = parts[1];
    ...
    try {
        ...
        strProc.addInParameter(2, 12, patVO.getHospName());
        System.out.println("patVO.getHospName()>>"+patVO.getHospName());
		strProc.addInParameter(3, 12, patVO.getTestName());
		System.out.println("patVO.getTestName()>>"+patVO.getTestName());
	//	strProc.addInParameter(4, 12, patVO.getSeqNo());
		...
		//strProc.addInParameter(6, 12, patVO.getQty());
		strProc.addInParameter(4, 12, patVO.getCghsTestCode());
		System.out.println("patVO.getCghsTestCode()>>"+patVO.getCghsTestCode());
		strProc.addInParameter(5, 12, patVO.getPatCRNo());
		System.out.println("patVO.getPatCRNo()>>"+patVO.getPatCRNo());
		System.out.println("patVO.getRefId()>>"+patVO.getRefId());
		strProc.addInParameter(6, 12, patVO.getRefId());
		strProc.addInParameter(7, 12, _userVO.getHospitalCode());
		System.out.println("patVO.getHospitalCode()>>"+_userVO.getHospitalCode());
		strProc.addInParameter(8, 12, firstPart);
		strProc.addInParameter(9, 12, patVO.getPatepisodecode());
		System.out.println("patVO.getPatepisodecode()>>"+patVO.getPatepisodecode());
		strProc.addInParameter(10, 12, patVO.getVisitNo());
		System.out.println("patVO.getVisitNo()>>"+patVO.getVisitNo());
		strProc.addInParameter(11, 12, voReqHeader.getRequisitionNumber());
		System.out.println("voReqHeader.getRequisitionNumber()()>>"+voReqHeader.getRequisitionNumber());
		strProc.addInParameter(12, 12, voReqHeader.getRequisitionwiseno());
		System.out.println("getRequisitionwiseno()()>>"+voReqHeader.getRequisitionwiseno());
		...
		System.out.println("patVO.getPatDepartment()>>"+patVO.getPatDepartment());
		strProc.addInParameter(14, 12, patVO.getPatdeptunit());
		System.out.println("patVO.getPatdeptunit()>>"+patVO.getPatDepartment());
        ...
        if (error != null && !error.isEmpty()) {
            throw new HisRecordNotFoundException("Procedure execution failed with error: " + error);
        }

        result.add("Procedure executed successfully");
        ...
        e.printStackTrace();
        throw new HisRecordNotFoundException("An unexpected error occurred");
    }
    return result;
}
	...
	public List<Inv_EpisodeVO> getPatientEpisode(Inv_RequisitionRaisingPatientVO patVO, UserVO _userVO) {
		...
		try {
			...
			throw new HisDataAccessException(
					"DailyPatientDAO:getPatientsForPoliceExaminationReqt:HelperMethods :: " + e);
		}
	...
	public List gethospitalnameCombo(UserVO _UserVO) {
		String query = "";
		Map populateMap = new HashMap();
		ResultSet rs = null;
		List<String> lsthospNames = new ArrayList<String>();
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		String queryKey = "SELECT.HOSPITAL_NAME_COMBO";
		...
		try {
			...
			if (rs.next()) {
				rs.beforeFirst();
				if (rs.next()) {
					lsthospNames = HelperMethodsDAO.getAlOfEntryObjects(rs);
				}
			}
	...
	public List<LabTestVO> getTestGroupNames(UserVO _UserVO) {
		String query = "";
		Map populateMap = new HashMap();
		ResultSet rs = null;
		List<LabTestVO> lstTestNames = new ArrayList<LabTestVO>();
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		...
		try {
			...
			if (rs.next()) {
				...
				while (rs.next()) {
					...
					/*
					 * if(voLabTest.getTestCode()!=null) voLabTest.getTestCode().replaceAll("^",
					 * "!!");
					 */
					lstTestNames.add(voLabTest);
	...
	public String generateRequisitionNoSequence(String labCode, UserVO userVO) {
		...
		try {
			...
			System.out.println("userVO.getHospitalCode()>>"+userVO.getHospitalCode());
	...
	public InvestigationRequistionVO ConfirmAppointment(InvestigationRequistionVO searchVO, UserVO _userVO) {
		...
		try {
			...
			throw new HisApplicationExecutionException(
					"HelperMethodsDAO.loadPropertiesFile(filename)OR getting query out of property file" + e + strErr);
		} finally {
	...
	public void tempFunction(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition3 = "and EXISTS (SELECT 1 FROM HIVT_TEST_GROUP_INFO_MST where hgnum_group_code ="
				+ searchVO.getSearchTestGroupName()
				+ " AND gnum_lab_code=a.gnum_labcode AND gnum_test_code=a.gnum_test_code and gnum_hospital_Code=a.gnum_hospital_Code)";
		String orderBy = " ORDER BY labname, testname";
	...
	public List<LabTestVO> searchLabTestSample(UserVO _UserVO) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		List<LabTestVO> lstLabVO = null;
	...
	public void insertRaisingCumCollectionDetails(LabTestVO voLabTest, UserVO _userVO) {
		...
		try {
			...
			strProc.addInParameter(7, 12,
					(voLabTest.getIsConfidential() == null) ? "0" : voLabTest.getIsConfidential());
			...
			strProc.addInParameter(14, 12, (voLabTest.getStrTestGroupCode() == null) ? voLabTest.getTestGroupCode()
					: voLabTest.getStrTestGroupCode());
	...
	public List<LabTestVO> searchBookMarkRaisingCumCollection(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition1 = "and EXISTS (SELECT 1 FROM HIVT_BOOKMARK_MST where ";
		String condition3 = "gnum_isvalid=1 and gnum_bookmark_code =" + searchVO.getBookMarkCode() + " AND";
		String condition2 = "  hivtnum_lab_code=a.gnum_labcode AND hivtnum_test_code=a.gnum_test_code and gnum_hospital_Code=a.gnum_hospital_Code  and gnum_user_id="
				+ _UserVO.getSeatId() + ")";
		String condition4 = "and EXISTS (SELECT 1 from hivt_test_mst where GNUM_TEST_CODE =  a.GNUM_TEST_CODE AND UPPER(GNUM_TEST_CODE) LIKE '%'||UPPER(TRIM('"
				+ searchVO.getSearchTestName() + "'))||'%')";
		String condition5 = "   AND EXISTS (SELECT 1 FROM hivt_test_mst WHERE gnum_test_code = a.gnum_test_code AND  gnum_test_code in("
	...
	public List<LabTestVO> searchLaboratoryWiseTestGroupOnClickRaisingCumCollection(InvestigationSearchVO searchVO,
			UserVO _UserVO) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
	...
	public List<LabTestVO> getGroupCodeWiseDtl(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		List<LabTestVO> lstLabVO = null;
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		String queryKey = "SELECT.LAB_WISE_TEST_GROUP_TEST.HIVT_TEST_MST2_WITHOUT_LAB_PERMISSION";
		Sequence sq = new Sequence();
	...
	public void insertRequisitionDtl_addendum(InvestigationRequistionVO voLabTest, UserVO _userVO) {
		...
		try {
			...
			populateMAP.put(sq.next(),
					(voLabTest.getReportChangeSeatId() == null) ? "0" : voLabTest.getReportChangeSeatId());
			...
			populateMAP.put(sq.next(),
					(voLabTest.getStrCollDateTime() == null) ? "" : voLabTest.getStrCollDateTime().toString());
	...
	public String getTestCodesKeyword(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		String condition = " And lower(gstr_test_name) like '%" + searchVO.getTestSearchKeyword().toLowerCase() + "%'";
	...
	public List<LabTestVO> searchLabWiseTestDtlstariff(InvestigationSearchVO searchVO, UserVO _UserVO,
			Inv_RequisitionRaisingPatientVO patvo, Inv_PatientAdmissionDtlVO patvoadd) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		List<LabTestVO> lstLabVO = null;
		...
		if (searchVO.getSearchTestNamelabwise() != null) {
			if (searchVO.getSearchTestNamelabwise().contains("#")) {
				...
				orderBytestwise = " ORDER BY gnum_test_code=" + searchVO.getSearchTestNamelabwise().split("#")[0]
						+ " desc ";
			}
		}
	...
	public List<LabTestVO> searchLabWiseTestDtlsNEW(InvestigationSearchVO searchVO, UserVO _UserVO) {
		...
		if (val != null && val.contains("#")) {
			orderBy = " ORDER BY gnum_test_code=" + val.split("#")[0] + " desc ,gnum_labcode=" + val.split("#")[1]
					+ " desc";
		}
	...
	public List<BookMarkVO> getBookMarkListraising(UserVO _userVO, String iscallingfromdesk) {
		ResultSet rs = null;
		String query = "";
		Map populateMAP = new HashMap();
		String filename = "new_investigation.transactions.investigationTransactionQueryreq";
		...
		try {
			...
			if (iscallingfromdesk != null && iscallingfromdesk.equals("1")) {
				...
				query = String.valueOf(query)
						+ "and hivtnum_lab_code not in (select nvl(n.gnum_lab_code,'0') from hivt_laboratory_mst n where n.gnum_lab_code=hivtnum_lab_code and n.gnum_isvalid=1 and n.gnum_hospital_code="
						+ _userVO.getHospitalCode() + "  and n.gnum_islabhidefromdesk  in (1) )";
			}
	...
	public List<LabTestVO> searchLabWiseTestDtlstariffxrayprocess(UserVO _UserVO, Inv_RequisitionRaisingPatientVO patvo,
			List<Inv_PatientAdmissionDtlVO> patvoaddm) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		List<LabTestVO> lstLabVO = null;
		...
		String orderBy = " ORDER BY labname,testname,labname";
	...
	public List<BookMarkVO> getBookMarkListraisingxray(UserVO _userVO, String iscallingfromdesk) {
		...
		try {
			...
			query = String.valueOf(query)
					+ "and exists (select 1 from hivt_laboratory_mst n where n.gnum_lab_code=hivtnum_lab_code and n.gnum_isvalid=1 and n.gnum_hospital_code="
					+ _userVO.getHospitalCode() + "   )";
			...
			// query+=" and hivtnum_test_code in (select gnum_test_code from hivt_test_mst
			// where gnum_isvalid=1 and is_show=1) ";
			...
			if (lstLabVO != null && lstLabVO.size() > 0) {
				...
				for (int k = 0; k < lstLabVO.size(); k++) {
					...
					if (mp.containsKey(vo.getReqNo())) {
						...
						String testwiseviewcount = String.valueOf(voo.getTotalviewcount()) + ","
								+ vo.getTotalviewcount();
						voo.setTestCode(testcode);
						voo.setTestName(testname);
						voo.setTotalviewcount(testwiseviewcount);
						mp.put(voo.getRequisitionDNo(), voo);
					} else {
						mp.put(vo.getRequisitionDNo(), vo);
	...
	public List<LabTestVO> gettestcharges(InvestigationSearchVO searchVO, UserVO _UserVO,
			Inv_RequisitionRaisingPatientVO patvo, Inv_PatientAdmissionDtlVO patvoadd) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				/*
				 * if(voLabTest.getTestrate()!=null) voLabTest.getTestrate().replaceAll("^",
				 * "!!");
				 */
				lstLabVO.add(voLabTest);
	...
	public List<LabTestVO> gettestchargesgrp(InvestigationSearchVO searchVO, UserVO _UserVO,
			Inv_RequisitionRaisingPatientVO patvo, Inv_PatientAdmissionDtlVO patvoadd) {
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
	...
	public String generateRequisitionNoWISESequence(UserVO userVO) {
		String reqNo = "";
		String errorMsg = "";
		ResultSet rs = null;

		try {
			Procedure strProc = new Procedure("pkg_inv_unique_no_generation.generate_unique_requisitionwise_no");
			strProc.addInParameter(1, 12, userVO.getHospitalCode());
			// strProc.addInParameter(2, 12, labCode);
			strProc.setReturnType(12);
			return reqNo = (String) strProc.execute(getTransactionContext().getConnection());
	...
	public String insertRequisitionHeaderDtl_reqprocess(RequistionHeaderVO voReqHeader, UserVO _userVO) {
		String error = "";

		try {
			Procedure strProc = new Procedure("pkg_inv_dml.proc_insert_hivt_reqisition_header_reqprocess");
			strProc.addInParameter(1, 12, "3");
			...
			strProc.addInParameter(49, 12,
					voReqHeader.getRequisitionwiseno() == null ? "" : voReqHeader.getRequisitionwiseno());

			strProc.addOutParameter(50, 12);

			strProc.execute(getTransactionContext().getConnection());
			// error = (String)strProc.getParameterAt(49);
			error = (String) strProc.getParameterAt(50);
			System.out.println(" error" + error);
			String str = error;
	...
	public Inv_RequisitionRaisingPatientVO getInvRaisingPatDetails(
			Inv_RequisitionRaisingPatientVO paramInv_RequisitionRaisingPatientVO, UserVO paramUserVO, String desktype) {
		// TODO Auto-generated method stub
		return null;
	}

	/*
	 * public void insertRequisitionDtl(InvestigationRequistionVO voLabTest, UserVO
	 * _userVO, String priorityAll, RequistionHeaderVO voReqHeader) { String
	 * sampleNo = ""; String errorMsg = ""; ResultSet rs = null;
	 * 
	 * 
	 * try { if (voLabTest.getStrTestGroupCode() == null) {
	 * voLabTest.setStrTestGroupType(null); }
	 * 
	 * if (voLabTest.getIslabappointmentbased() != null &&
	 * voLabTest.getIslabappointmentbased().equals("1")) {
	 * 
	 * voLabTest.setStrIsAppointment(voLabTest.getIslabappointmentbased());
	 * 
	 * } else if (voLabTest.getIsAppointment() != null &&
	 * voLabTest.getIsAppointment().equals("1")) {
	 * 
	 * voLabTest.setStrIsAppointment("2"); }
	 * 
	 * 
	 ...
	 * 
	 * 
	 * System.out.println("hello site 1"); if (voLabTest.getSite() == null ||
	 * voLabTest.getSite().equals("") || voLabTest.getSite().equals("null")) {
	 * System.out.println("hello site"); voLabTest.setSite(null); }
	 * 
	 * Procedure strProc = new
	 * Procedure("pkg_inv_dml.proc_insert_hivt_requisition_dtl_new");
	 * strProc.addInParameter(1, 12, "3"); strProc.addInParameter(2, 12,
	 * _userVO.getHospitalCode()); strProc.addInParameter(3, 12,
	 * voLabTest.getStrRequsitionDNo()); strProc.addInParameter(4, 12,
	 * (voLabTest.getStrIsAppointment() == null) ? "0" :
	 * voLabTest.getStrIsAppointment()); strProc.addInParameter(5, 12,
	 * voLabTest.getStrLabCode()); strProc.addInParameter(6, 12,
	 * voLabTest.getStrTestCode().toString()); strProc.addInParameter(7, 12,
	 * (voLabTest.getStrIsConfidential() == null) ? "0" :
	 * voLabTest.getStrIsConfidential().toString()); strProc.addInParameter(8, 12,
	 * voLabTest.getStrPriority()); strProc.addInParameter(9, 12, null);
	 * strProc.addInParameter(10, 12, voLabTest.getStrRequisitionDtlStatus());
	 * strProc.addInParameter(11, 12, _userVO.getSeatId());
	 * strProc.addInParameter(12, 12, voLabTest.getAppointmentRefNo());
	 * strProc.addInParameter(13, 12, null); strProc.addInParameter(14, 12,
	 * voLabTest.getStrTestGroupCode()); strProc.addInParameter(15, 12,
	 * voLabTest.getStrTestGroupType()); strProc.addInParameter(16, 12, null);
	 * strProc.addInParameter(17, 12, null); strProc.addInParameter(18, 12, null);
	 * strProc.addInParameter(19, 12, null); strProc.addInParameter(20, 12,
	 * voLabTest.getStrSampleCode()); strProc.addInParameter(21, 12, null);
	 * strProc.addInParameter(22, 12, null); strProc.addInParameter(23, 12, null);
	 * strProc.addInParameter(24, 12, null); strProc.addInParameter(25, 12, null);
	 * strProc.addInParameter(26, 12, null); strProc.addInParameter(27, 12,
	 * voLabTest.getStrWorkOrderSequence()); strProc.addInParameter(28, 12, null);
	 * strProc.addInParameter(29, 12, null); strProc.addInParameter(30, 12, null);
	 * strProc.addInParameter(31, 12, voLabTest.getStrTypeOfComponent());
	 * strProc.addInParameter(32, 12, voLabTest.getStrReqNo());
	 * strProc.addInParameter(33, 12, null); strProc.addInParameter(34, 12, null);
	 * strProc.addInParameter(35, 12, null); strProc.addInParameter(36, 12, null);
	 * strProc.addInParameter(37, 12, null); strProc.addInParameter(38, 12, null);
	 * strProc.addInParameter(39, 12, voLabTest.getStrAppointmentTime());
	 * strProc.addInParameter(40, 12, null); strProc.addInParameter(41, 12, null);
	 * strProc.addInParameter(42, 12, null); strProc.addInParameter(43, 12, null);
	 * strProc.addInParameter(44, 12, null); strProc.addInParameter(45, 12, null);
	 * strProc.addInParameter(46, 12, voLabTest.getStrAppointmentDate());
	 * strProc.addInParameter(47, 12, priorityAll);
	 * 
	 * strProc.addInParameter(48, 12, (voLabTest.getSite() == null) ? null :
	 * voLabTest.getSite());
	 * 
	 * 
	 * strProc.addInParameter(49, 12, voReqHeader.getPatCrNo());
	 * strProc.addInParameter(50, 12, voReqHeader.getVisitNo());
	 * strProc.addInParameter(51, 12, null); strProc.addInParameter(52, 12,
	 * voReqHeader.getReqHeaderStatus()); strProc.addInParameter(53, 12,
	 * voReqHeader.getEpisodeCode()); strProc.addInParameter(54, 12,
	 * voReqHeader.getPatName()); strProc.addInParameter(55, 12,
	 * voReqHeader.getReqType()); strProc.addInParameter(56, 12,
	 * voReqHeader.getPatDob()); strProc.addInParameter(57, 12,
	 * voReqHeader.getVisitDate()); strProc.addInParameter(58, 12,
	 * voReqHeader.getIsActualDob()); strProc.addInParameter(59, 12,
	 * voReqHeader.getAdvisedByDocNo()); strProc.addInParameter(60, 12,
	 * voReqHeader.getGenderCode()); strProc.addInParameter(61, 12,
	 * voReqHeader.getRequsitionRaisedThrough()); strProc.addInParameter(62, 12,
	 * voReqHeader.getPatAge()); strProc.addInParameter(63, 12,
	 * voReqHeader.getPatAdmissionNo()); strProc.addInParameter(64, 12,
	 * voReqHeader.getPatAddress()); strProc.addInParameter(65, 12,
	 * voReqHeader.getWardCode()); strProc.addInParameter(66, 12,
	 * voReqHeader.getMobileNo()); strProc.addInParameter(67, 12,
	 * voReqHeader.getEmailId()); strProc.addInParameter(68, 12,
	 * voReqHeader.getRoomCode()); strProc.addInParameter(69, 12,
	 * voReqHeader.getBedCode()); strProc.addInParameter(70, 12,
	 * voReqHeader.getMlcNo()); strProc.addInParameter(71, 12,
	 * voReqHeader.getBedName()); strProc.addInParameter(72, 12,
	 * voReqHeader.getRoomName()); strProc.addInParameter(73, 12,
	 * voReqHeader.getWardName()); strProc.addInParameter(74, 12,
	 * voReqHeader.getDeptName());
	 * 
	 * strProc.addInParameter(75, 12, ""); strProc.addInParameter(76, 12,
	 * voReqHeader.getDeptUnitName()); strProc.addInParameter(77, 12, "");
	 * strProc.addInParameter(78, 12, voReqHeader.getDeptCode());
	 * strProc.addInParameter(79, 12, voReqHeader.getDeptUnitCode());
	 * strProc.addInParameter(80, 12, voReqHeader.getIsAutomatedRequest());
	 * strProc.addInParameter(81, 12, voReqHeader.getAdvisedByDocName());
	 * strProc.addInParameter(82, 12, voReqHeader.getRefHospitalCode());
	 * strProc.addInParameter(83, 12, ""); strProc.addInParameter(84, 12,
	 * voReqHeader.getRefLabCode()); strProc.addInParameter(85, 12,
	 * voReqHeader.getHospOrLabName()); strProc.addInParameter(86, 12,
	 * voReqHeader.getExtCrNo()); strProc.addInParameter(87, 12,
	 * voReqHeader.getDeleteReason()); strProc.addInParameter(88, 12, "1");
	 * strProc.addInParameter(89, 12, voReqHeader.getPatCatCode());
	 * strProc.addInParameter(90, 12, voReqHeader.getVisitReason());
	 * strProc.addInParameter(91, 12, ""); strProc.addInParameter(92, 12, "");
	 * strProc.addInParameter(93, 12, ""); strProc.addInParameter(94, 12, "");
	 * strProc.addInParameter(95, 12, "");
	 * 
