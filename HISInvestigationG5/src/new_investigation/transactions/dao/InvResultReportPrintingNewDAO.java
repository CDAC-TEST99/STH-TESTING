...
public class InvResultReportPrintingNewDAO extends DataAccessObject
{
	public InvResultReportPrintingNewDAO(JDBCTransactionContext _tx)
	{    
		super(_tx);
	}
	...
	/*public List<InvResultReportPrintingNewVO> getpatdtl(UserVO _userVO, String strCrNo_p)
    {
            ...
            List<InvResultReportPrintingNewVO> lstInvResultReportPrintingVO = new ArrayList<InvResultReportPrintingNewVO>();
    		ValueObject[] valueObjects = null;
            ...
            if (lstInvResultReportPrintingVO == null || lstInvResultReportPrintingVO.size() == 0)
            {
                    try
                    {
                            Procedure strProc = new Procedure(InvestigationConfig.PROCEDURE_GET_PAT_DTL);
                            strProc.addInParameter(1, Types.VARCHAR, "1");
                            strProc.addInParameter(2, Types.VARCHAR, strCrNo_p);
                    ...
                    try
                    {
                           // alRecord = HelperMethodsDAO.getAlOfEntryObjectsCallable(rs);
                			...
                			{
                				rs.beforeFirst();
                				valueObjects = HelperMethods.populateVOfrmRS(InvResultReportPrintingNewVO.class, rs);
                				//dailyPatientVOs = new PatientDetailVO[valueObjects.length];
                				for (int i = 0; i < valueObjects.length; i++)
                				{
                					//dailyPatientVOs[i] = (PatientDetailVO) valueObjects[i];
                					lstInvResultReportPrintingVO.add((InvResultReportPrintingNewVO)valueObjects[i]);
                				}
                			}
	...
	public List<Inv_RequisitionRaisingPatientVO> getpatdtl(UserVO _userVO, String strCrNo_p)
	{
		ResultSet rs = null;
		String query = "";
		Map populateMAP = new HashMap();
		...
		queryKey="SELECT.PATIENT_DTLS.HRGT_PATIENT_DTL_NEW";
		...
		try
		{
			...
			populateMAP.put(sq.next(), strCrNo_p);
		...
		PatientDetailVO[] dailyPatientVOs = null;
		List<Inv_RequisitionRaisingPatientVO> lstInvResultReportPrintingVO = new ArrayList<Inv_RequisitionRaisingPatientVO>();
		ValueObject[] valueObjects = null;
		...
		try
		{
			...
			{
				rs.beforeFirst();
				valueObjects = HelperMethods.populateVOfrmRS(Inv_RequisitionRaisingPatientVO.class, rs);
				//dailyPatientVOs = new PatientDetailVO[valueObjects.length];
				for (int i = 0; i < valueObjects.length; i++)
				{
					//dailyPatientVOs[i] = (PatientDetailVO) valueObjects[i];
					lstInvResultReportPrintingVO.add((Inv_RequisitionRaisingPatientVO)valueObjects[i]);
				}
			}
		}
		catch (Exception e)
	...
	public List<InvResultReportPrintingNewVO> getdatacrnowise(UserVO _userVO, String strCrNo_p)
	{
		ResultSet rs = null;
		String query = "";
		Map populateMAP = new HashMap();
		...
		queryKey="SELECT_REPORT_DATA_CRNO_WISE_REQ_DTL";
	...
	public List<Inv_RequisitionRaisingPatientVO> getipdpatdtl(UserVO _userVO, String strCrNo_p)
	{
		ResultSet rs = null;
		String query = "";
		Map populateMAP = new HashMap();
		...
		queryKey="SELECT.PATIENT_ADMISSION_DTLS.HIPT_PATADMISSION_DTL";
