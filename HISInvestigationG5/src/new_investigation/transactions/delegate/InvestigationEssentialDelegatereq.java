...
import new_investigation.transactions.bo.InvestigationEssentialBOireq;
import new_investigation.transactions.bo.InvestigationEssentialBOreq;
import new_investigation.transactions.bo.InvestigationEssentialBOxray;
...
public class InvestigationEssentialDelegatereq extends Delegate
{
	public InvestigationEssentialDelegatereq()
	{
		super(new InvestigationEssentialBOreq());
	}
	...
	{
		InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
		return(serviceBO.getPatientRegistration_EpisodeDetailEssentials(patVO,userVO));
	}
	
	
	public   List<Map<String, String>> getreferraltestdata(Inv_RequisitionRaisingPatientVO patVO,UserVO userVO)
	{
		InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
		System.out.println("AJX_GETREFERRALTESTDATA serviceBO");
		return(serviceBO.getreferraltestdata(patVO,userVO));
	}
	
	
	public   List<Map<String, String>> gethospitalname(Inv_RequisitionRaisingPatientVO patVO,UserVO userVO)
	{
		InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
		System.out.println("gethospitalname serviceBO");
		return(serviceBO.gethospitalname(patVO,userVO));
	}
	...
	// Getting Packing List Generation Essentials
		...
		{
			InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
			return(serviceBO.getBookMarkListraising(userVO, iscallingfromdesk));
		}
		...
		public Map  getessentialdetailsforxray(UserVO userVO,Inv_RequisitionRaisingPatientVO patVO,List<Inv_PatientAdmissionDtlVO> patadm,HttpServletRequest request)
		{
			InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
			return(serviceBO.getessentialdetailsforxray(userVO,patVO,patadm, request));
		}
		...
		public List  saveReferralDetails(List<Inv_RequisitionRaisingPatientVO> patVOList ,UserVO _userVO )
		{
			InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq) super.getServiceProvider();
			 return serviceBO.saveReferralDetails(patVOList,_userVO);
		}
		...
		{
			InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
			return(serviceBO.getPrvTestDtlAJAXMAPxray(CrNo,userVO,fromwhicall));
		}	
		...
		{
			InvestigationEssentialBOireq serviceBO = (InvestigationEssentialBOireq)super.getServiceProvider();
			return(serviceBO.getsamplelist(_UserVO));				
		}
