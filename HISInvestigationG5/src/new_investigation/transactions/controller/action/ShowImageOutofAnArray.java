...
public class ShowImageOutofAnArray extends HttpServlet{	
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {		
			...
			byte[] imageArray =(byte[])request.getSession().getAttribute(InvestigationConfig.UPLOADED_FILE_AS_ARRAY);
			if(imageArray!=null){
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {		
		...
		byte[] imageArray =(byte[])request.getSession().getAttribute("uploadedFileAsArray");
		response.setContentType("image/jpg");
		OutputStream os = response.getOutputStream();
		os.flush();
		BufferedOutputStream bos = new BufferedOutputStream(os);
