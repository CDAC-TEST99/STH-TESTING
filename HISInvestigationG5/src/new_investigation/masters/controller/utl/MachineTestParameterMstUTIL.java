...
public class MachineTestParameterMstUTIL implements MasterInterface
{
	...
	public String[] getDeleteQueryold()
	{
		String deleteQuery[] = new String[1];
		String seatId = httpSession.getAttribute("SEATID").toString();
		String chk=httpSession.getAttribute("chk").toString();
		System.out.println("chk>>>"+chk);
       String[] groups = chk.split("\\$");
        
        for (String group : groups) {
            if (!group.isEmpty()) {
                // Step 2: Split each group by the '@' character
                String[] values = group.split("@");
                
                // Ensure that we have the correct number of values
                if (values.length == 4) {
                    String machineCode = values[0];
                    String testCode = values[1];
                    String slno = values[2];
                    String hospitalCode = values[3];
                    
                    // Step 3: Print the values or use them as needed
		...
		///htSession.setAttribute("chk",request.getParameter("chk"));
...
System.out.println("deleteQuery>>>"+deleteQuery);
	 ...
	 public String[] getDeleteQuery() {
	        ...
	        for (String group : groups) {
	            if (!group.isEmpty()) {
	                // Split each group by the '@' character
	                String[] values = group.split("@");
	                ...
	                if (values.length == 4) {
	                    ...
	                    record.put("machineCode", machineCode);
	                    record.put("testCode", testCode);
	                    record.put("slno", slno);
	                    record.put("hospitalCode", hospitalCode);
	                    record.put("seatId", seatId);
	        ...
	        // Process the records as needed (e.g., generate delete queries)
	        for (Map<String, String> record : records) {
	            // Example of generating a delete query using the stored values
	            String baseQuery = "UPDATE HMIT_MACHINE_TESTPARA_MST SET  GNUM_ISVALID='0', GDT_ENTRY_DATE=SYSDATE, GNUM_SEAT_ID = " + seatId
	                    + " where gnum_isvalid=1 and hminum_machine_id=? and hminum_hmis_test_code=? and hminum_slno=? and gnum_hospital_code=?";
	            
	            baseQuery = baseQuery.replaceFirst("\\?", record.get("machineCode"))
	                                 .replaceFirst("\\?", record.get("testCode"))
	                                 .replaceFirst("\\?", record.get("slno"))
	                                 .replaceFirst("\\?", record.get("hospitalCode"));

	            deleteQuery[0] = baseQuery;
	            System.out.println("Generated Delete Query: " + deleteQuery[0]);
	        }

	        System.out.println("deleteQuery>>> " + deleteQuery[0]);
	        return deleteQuery;
	    }
