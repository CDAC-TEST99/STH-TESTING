...
## Project Name							: Generation 5, HIS_MC
## Name of Developer		 			: Pragya Sharma
## Module Name							: HIS_MC Single Sign On and Web Security 
## Process/Database Object Name			: HISSSOClientRequestFilter
## Purpose								: HIS_MC SSO Client Request Filter
## Date of Creation						:  
## Modification Log						:				
...
public class HISSSOClientRequestFilterInvestigation implements Filter
{
	...
	{
		//System.out.println("HBIMS INSIDE HISSSOClientRequestFilter");
		...
		try
		{
			String strURI = objRequest.getRequestURI();
			 System.out.println("inv url >>  "+strURI);
			
			//System.out.println("Condition ...."+strURI.contains("HISDRDESK_MC"));
			
			//System.out.println("Referrer...."+objRequest.getHeader("Referer"));
			// Skipping URLs that can be accessed without Login into AHIMSG5 like strURI.endsWith(".js") || strURI.endsWith(".css") ||
			//SKIP URL RESTFUL FOR PSS WEB SERVICE http://10.226.30.212:8380/HBIMS/services/restful/hisDailyPatientList/3
			/*if(!strURI.contains("rest") && !(objRequest.getHeader("Referer") != null && objRequest.getHeader("Referer").trim().length() > 0))
            {
                RequestDispatcher rd = null;
                System.out.println("Inside referrer ...."+strURI.contains("/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt"));
                rd = objRequest.getRequestDispatcher(HISSSOConfig.SSO_AUTHENTICATION_ERROR_PAGE_URL);
                rd.forward(objRequest, objResponse);
               
                return;
            }*/
			if (strURI.contains(HISSSOClientConfig.SSO_ST_SERVICE_URL) || strURI.contains("/op/pb") || strURI.contains("rest") ||strURI.contains("service") || strURI.contains("QMS/QMS.jsp") || strURI.contains("websocket") )
			{
				
				 System.out.println("inv services allowed  ");
				// Forward as-it-is
				//chain.doFilter(new RequestWrapper((HttpServletRequest) request), response);
				chain.doFilter(request, response);
			}
			else if(strURI.endsWith(".js") || strURI.endsWith(".css")||strURI.endsWith(".gif")||strURI.endsWith(".png")||strURI.endsWith(".jpg")||strURI.endsWith(".jpeg")||strURI.endsWith(".jpeg"))
			{
				
				
				 System.out.println("inv image allowed  ");
			...
			else
			{
				...
				if (session != null)
				{
					//System.out.println("HBIMS SESSION HISSSOClientRequestFilter "+session.getId());
					// If Session Already Exists
					// Fetch ST Id from Session
					strServiceTicketId = (String) session.getAttribute(HISSSOConfig.LOGGEDIN_USER_SERVICE_TICKET_ID);
					//System.out.println("HIS_MC-SSO-CLN:: Service Ticket ID Found in Session-"+ strServiceTicketId);
					...
					else
					{
						...
						//System.out.println("HIS_MC-SSO-CLN:: Service Ticket Found in Session with ST-"+ strServiceTicketId + " TGT-" + reqGrantingTicketId);						
						...
						else
						{
							...
							if (objHISServiceTicket.isExpired())
							{
								...
								//System.out.println("HBIMS SESSION AFTER EXPIRED HISSSOClientRequestFilter "+session.getId());
								...
								// Update New Session into HIS_MC Registry
				...
				else
				{
					...
					//System.out.println("HIS_MC-SSO-CLN:: Session is NUll Ticket Found in Request TGT-" + grantingTicketId);
				...
				if (authenticatioRequired)
				{
					...
					{
						//System.out.println("HIS_MC-SSO-CLN:: TGT-" + grantingTicketId+ " Registered Service-" + objHISService.getContext() +" ST-"+objGrantTicket.getTicketId());
						authenticate = true;
						...
						//System.out.println("HBIMS SESSION AFTER EXPIRED AGAIN HISSSOClientRequestFilter "+session.getId());
						//System.out.println("HBIMS SESSION AFTER EXPIRED AGAIN HISSSOClientRequestFilter "+objSession.getId());
						// Set TGT inSession
				...
				if (authenticate)
				{
					...
					if (objHISServiceTicket.getAuthorization().isAuthorizedURL(strURI))
					{
						...
						System.out.println("inv authoriz url allowed ");
					...
					{
						System.out.println("HISDRDESK_MC_HISSSOClientRequestFilter_SSO_AUTHORIZATION_ERROR_PAGE_URL");
						RequestDispatcher rd = null;
		...
		{
			System.out.println("HISDRDESK_MC_HISSSOClientRequestFilter_SSO_ILLEGAL_ACTIVITY_ERROR_PAGE_URL");
			e.printStackTrace();
			RequestDispatcher rd = null;
		...
		{
			System.out.println("HISDRDESK_MC_HISSSOClientRequestFilter_SSO_UNKNOWN_ERROR_PAGE_URL");
			e.printStackTrace();
			RequestDispatcher rd = null;
			rd = objRequest.getRequestDispatcher(HISSSOConfig.SSO_UNKNOWN_ERROR_PAGE_URL);
			rd.forward(objRequest, objResponse);
		}
		//System.out.println("HBIMS EXIT HISSSOClientRequestFilter");
	}
	...
	private boolean setCredentialsInSession(HttpSession objSession_p, HISServiceTicket objST_p)
	{
		...
		//System.out.println("Docter desk war setCredentialsInSession");
		...
		//	System.out.println("Docter desk war setCredentialsInSession >> "+voGlobalUser.getUserId());
