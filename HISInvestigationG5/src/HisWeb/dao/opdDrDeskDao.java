...
public class opdDrDeskDao {
   			...
   			if(DrugObjArray!=null)
   				{
		   			...
		   			for (int i=0 ;i<DrugObjArray.length();i++)
		   			{
		   				try {
				   	            ...
				   	            dao.setProcInValue(procIndex1, "admno", String.valueOf(i),5); 
				   	            //dao.setProcInValue(procIndex1, "visitno", PatCompleteGeneralDtl.split("#")[4],6); 
				   	            dao.setProcInValue(procIndex1, "visitno", EpisodeVisitNo,6); 
				   	            
				   	            //"DrugCodeCat":["Paracip 150 mg/1mL solution for injection Cipla Limited&& 10006830#100007#662201000190000!1#NA!1#10121034#3 &&-- &&0 &&SOS(As per Requirement) &&14 &&2021-03-1 0&&1#1 && "] -- drug json without remarks
		
				   	            //"DrugCodeCat":["Paratel 650 mg oral tablet Intel Pharmaceuticals&& 10002082#100016#0#N#10108093 &&Single &&14 &&OD&& 11 &&2021-03-10 &&1#1&& Remarks"] -- drug json with remarks
				   	            String tmp = (String) DrugObjArray.get(i);
				   	          //  if(i==0)
				   	           // 	tmp = "GLUCOSAMINE 750 MG + DIACERINE 50 MG TAB&&10008125#100016#0#N#10123087&&Single&&14&&OD&&11&&2021-08-13&&30#&&";
								...
								if(tmpArray.length == 8)
									...
									dao.setProcInValue(procIndex1, "remarks",  tmpArray[8],16);
								...
								dao.setProcInValue(procIndex1, "deptunitcode", PatCompleteGeneralDtl.split("#")[5],25);
								dao.setProcInValue(procIndex1, "deptcode", PatCompleteGeneralDtl.split("#")[6],26);
		   		...
		   		//if(InvesObjArray.length() > 0 ){
		   			...
		   			for(int j=0 ;j<InvesObjArray.length();j++)
		   			{
		   				...
		   				if (InvesObjArray.get(j) != null && !InvesObjArray.get(j).toString().trim().equals("null")) {
			   				...
			   				{
			   					if (InvesObjArray.get(k) != null && !InvesObjArray.get(k).toString().trim().equals("null")) {
		   			...
		   			if(bldMap!=null)
			   		{
		   				...
		   				if(testsb!=null && !testsb.isEmpty() && testsb.split("^").length>0)
						{
							for (int q = 0; q < testsb.split("^").length; q++) {
								...
								if (PatCompleteGeneralDtl.split("#")[11] != null
										&& !PatCompleteGeneralDtl.split("#")[11].trim().equals("null"))
									dao.setProcInValue(procIndex4, "sblnum_chargetype_id",
											PatCompleteGeneralDtl.split("#")[11], 3);
								else
								...
								dao.setProcInValue(procIndex4, "gnum_treatment_cat", PatCompleteGeneralDtl.split("#")[10],
										6);
								// System.out.println("gnum_treatment_cat"+PatCompleteGeneralDtl.split("#")[10]);
								// System.out.println("testsb.toString().substring(0, testsb.toString().length()
								// - 1)"+testsb.toString().substring(0, testsb.toString().length() - 1));
								dao.setProcInValue(procIndex4, "hrgnum_episode_code", EpisodeCode, 7);
								dao.setProcInValue(procIndex4, "hrgnum_puk", Crno, 8);
								dao.setProcInValue(procIndex4, "gstr_tariff",
										testsb.toString().substring(0, testsb.toString().length() - 1), 9);
								dao.setProcInValue(procIndex4, "gstr_reqqty",
										testQtysb.toString().substring(0, testQtysb.toString().length() - 1), 10);
								...
								// PatCompleteGeneralDtl.split("#")[4],23);
								dao.setProcInValue(procIndex4, "visitno", EpisodeVisitNo, 23);
						...
						{
							for (int q = 0; q < testGrpsb.split("^").length; q++) {
								...
								if (PatCompleteGeneralDtl.split("#")[11] != null
									...
									dao.setProcInValue(procIndex5, "sblnum_chargetype_id", "1", 3);
								...
								dao.setProcInValue(procIndex5, "gnum_treatment_cat", PatCompleteGeneralDtl.split("#")[10],
										6);
								// System.out.println("gnum_treatment_cat"+PatCompleteGeneralDtl.split("#")[10]);
								// System.out.println("testsb.toString().substring(0, testsb.toString().length()
								// - 1)"+testsb.toString().substring(0, testsb.toString().length() - 1));
								...
								dao.setProcInValue(procIndex5, "gstr_tariff",
										testGrpsb.toString().substring(0, testGrpsb.toString().length() - 1), 9);
								dao.setProcInValue(procIndex5, "gstr_reqqty",
										testGrpQtysb.toString().substring(0, testGrpQtysb.toString().length() - 1), 10);
								...
								// dao.setProcInValue(procIndex4, "visitno",
								// PatCompleteGeneralDtl.split("#")[4],23);
								dao.setProcInValue(procIndex5, "visitno", EpisodeVisitNo, 23);
   			...
   			if(Diagnosis!=null)
   				{
		   			...
		   			for (int i=0 ;i<Diagnosis.length();i++)
		   			{
		   	            ...
		   	            if(diagnostictypecode != null && !diagnostictypecode.equals("11") && !diagnostictypecode.equals("12") && !diagnostictypecode.equals("14"))
	...
	public static String saveProcdureData(String JsonData) {
    	String err = "";
   			...
   			if(strClinicalProcedure!=null)
   				{
		   			...
		   			for (int i=0 ;i<strClinicalProcedure.length();i++)
		   			{
		   				...
		   				dao = new HisDAO("OPD DR DESK DAO", "opdDrDeskDao.saveProcdureData()");
		   	            ...
		   	            //dao.setProcInValue(procIndex1, "proc_code", PatCompleteGeneralDtl.split("#")[4],6);
		   	            dao.setProcInValue(procIndex1, "proc_code", EpisodeVisitNo,6);
		   	            dao.setProcInValue(procIndex1, "procdate", (tmp.split("#")[1]).split("\\^")[0],7); 
		   	            ...
		   	            dao.setProcInValue(procIndex1, "episodecode", EpisodeCode,13); 
		   	         	//dao.setProcInValue(procIndex1, "visitno", PatCompleteGeneralDtl.split("#")[4],14);
		   	            dao.setProcInValue(procIndex1, "visitno", EpisodeVisitNo,14);
		   	         	dao.setProcInValue(procIndex1, "remarks", (tmp.split("#")[4]),14); 
		   	         	dao.setProcInValue(procIndex1, "paraflag", "1",15); 
		   	         	dao.setProcOutValue(procIndex1, "err", 1,16);
		   	         	dao.setProcInValue(procIndex1, "servicereqno", "1010101",17); 
		   	            dao.setProcInValue(procIndex1, "admissionno", "0",18); 
		   	            dao.setProcInValue(procIndex1, "wardcode", "0",19); 
		   	         	dao.setProcInValue(procIndex1, "tariffid", (tmp.split("#")[1]).split("\\^")[0],20); 
		   	         	dao.setProcInValue(procIndex1, "quantity", "1",21); 
		   	         	dao.setProcInValue(procIndex1, "count", "0" , 22); 
	...
	public static String SaveEMRVitalData(String JsonData) {
        ...
        try {
			...
			/* <!-- ----------------------added for cancer screening--------------- --> */
   			//String strcancerScreening=(String) object.get("strcancerScreening")==null ? "" : (String) object.get("strcancerScreening") ;
            ...
            /* <!-- ----------------------added for cancer screening--------------- --> */
         //   dao.setProcInValue(procIndex1, "hoplstr_cancer_screening", strcancerScreening,44);
	...
	public static void SavePrescriptionBookMarkDataFromPB(String JsonData, String DeptUnitName) {
        ...
        try {
            ...
            dao.setProcInValue(procIndex1, "p_json", JsonData,7);
			dao.setProcInValue(procIndex1, "p_isvalid", "1",8);
			dao.setProcInValue(procIndex1, "p_profiletype", "2",9); //p_deptunitcode  character varying, p_deptcode  character varying, 
	...
	public static String saveOfflinePatCountDtl(String JsonData){
		...
		try {
        	...
        	HisException eObj = new HisException("OPD Lite","opdDrDeskDao.saveOfflinePatCountDtl()-->", e.getMessage() + "-->" + e);
            e.printStackTrace();
            return "0";
        }
        finally {
