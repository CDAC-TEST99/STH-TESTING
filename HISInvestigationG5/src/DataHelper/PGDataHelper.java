...
//    }
    ...
    public String getDynamicTemplateStatusold(String code, String hospitalCode, boolean isGroupWithDynamicTemplate, String labCode)
    {
    ...
    public String getDynamicTemplateStatus(String code, String hospitalCode, boolean isGroupWithDynamicTemplate, String labCode)
    {
    	...
    	String dynamicTemplateID = null;
          ...
          boolean isDynamic = false;
        	...
        	dao = new HisDAO("Inv", "getDynamicTemplateStatus1");
        	
        	nQueryIndex = dao.setQuery(QueryConfig.Q_GET_DYNAMIC_TESTTEMPLATE_STATUS);
        ...
        //  String query = QueryConfig.Q_GET_DYNAMIC_TESTTEMPLATE_STATUS;
          ...
          try {
             if(isGroupWithDynamicTemplate)
            	 nQueryIndex = dao.setQuery(QueryConfig.Q_GET_DYNAMIC_GROUPTEMPLATE_STATUS);
            
             
           //  pstmt = conn.prepareStatement(query);
			   dao.setQryValue(nQueryIndex, 1, code);
             /// pstmt.setString(1, code);
              
              if(isGroupWithDynamicTemplate)
              {    
		           dao.setQryValue(nQueryIndex, 2, hospitalCode);
		           dao.setQryValue(nQueryIndex, 3, labCode);
            	  //pstmt.setString(2, hospitalCode);
            	 // pstmt.setString(3, labCode);
              }
        ...
        try {           
            ...
            if (rs.next()) {
                ...
                if (requisitionFormClob != null) {
                    ...
                    ip = new ByteArrayInputStream(requisitionFormClob.getBytes(Charset.forName("UTF-8")));
                }
   ...
   /// PreparedStatement pstmt = null;
    ...
    try {
       dao = new HisDAO("Inv", "Q_GET_TESTTEMPLATE_XML");
    	
    	//nQueryIndex = dao.setQuery(QueryConfig.Q_GET_TESTTEMPLATE_XML);
    	 nQueryIndex = dao.setQuery(QueryConfig.Q_GET_TESTTEMPLATE_XML);
    ...
    public InputStream getClobObjectFromDatabaseForDocument(String id) {
        ...
        try {           
        	...
        	dao = new HisDAO("Inv", "getClobObjectFromDatabaseForDocument");
        	
        	nQueryIndex = dao.setQuery(QueryConfig.Q_GET_STYLESHEET);

    		dao.setQryValue(nQueryIndex, 1, id);
    ...
    public List<FourStringObject> getResultEntryValidationList(ResultEntryVO resultEntryVO) {
      ...
      //  Connection conn = PGDataHelper.getInstance().getConnection();
    	...
    	nQueryIndex = dao.setQuery(QueryConfig.Q_GET_RESULT_FORXML);
      
        //PreparedStatement pstmt = null;
        try {
          //  pstmt = conn.prepareStatement(query);
        	   dao.setQryValue(nQueryIndex, 1, resultEntryVO.getRequisitionDNo());
        	   dao.setQryValue(nQueryIndex, 2, resultEntryVO.getHospitalCode());
    ...
    public static void processingToPutTheJobs(List<ResultEntryVOGroupByValidatedBy> resultEntryVOGroupByValidatedBy) {
       ...
       // PreparedStatement pstmt = null;
    	...
    	nQueryIndex = dao.setQuery(QueryConfig.Q_PUT_PDF_JOB);
       /// String query = QueryConfig.Q_PUT_PDF_JOB;
        ...
        for (int i = 0; i < resultEntryVOGroupByValidatedBy.size(); i++) {
            ...
            if (!reportingCRNo.containsKey(voGroupByValidatedBy.getPatCRNo() + voGroupByValidatedBy.getLaboratoryCode())) {
                ...
                try {
               dao.setQryValue(nQueryIndex, 1,voGroupByValidatedBy.getLaboratoryCode());
               dao.setQryValue(nQueryIndex, 2, voGroupByValidatedBy.getPatCRNo());
               dao.setQryValue(nQueryIndex, 3,"1");
               dao.setQryValue(nQueryIndex, 4,"1");
               dao.setQryValue(nQueryIndex, 5,voGroupByValidatedBy.getHospitalCode());
               dao.setQryValue(nQueryIndex, 6,"1");
               dao.setQryValue(nQueryIndex, 7, voGroupByValidatedBy.getIsDeptEntry());
    ...
    public static boolean getPrintWithStandardRanges(String testCode,
 		...
 		dao = new HisDAO("Inv", "Q_GET_PRINTED_WITHSTANDARD_RANGES");
    	
    	nQueryIndex = dao.setQuery(QueryConfig.Q_GET_PRINTED_WITHSTANDARD_RANGES);
		...
		{
			 //pstmt = conn.prepareStatement(query);
			 dao.setQryValue(nQueryIndex, 1, testCode);
			 dao.setQryValue(nQueryIndex, 2, labCode);
			 dao.setQryValue(nQueryIndex, 3, hospitalCode);
    ...
    public List<FourStringObject> getDeptResultEntryValidationList(ResultEntryVO resultEntryVO) {
      ...
      ///  query = QueryConfig.Q_GET_DEPT_RESULT_FORXML;
    	...
    	nQueryIndex = dao.setQuery(QueryConfig.Q_GET_DEPT_RESULT_FORXML);
        Log(Level.INFO, "Query :WorkOrder List For Result Validation " + nQueryIndex);

       
        try {
        	  dao.setQryValue(nQueryIndex, 1, resultEntryVO.getRequisitionDNo());
        	  dao.setQryValue(nQueryIndex, 1, resultEntryVO.getHospitalCode());
           // pstmt.setString(1, resultEntryVO.getRequisitionDNo());
          //  pstmt.setString(2, resultEntryVO.getHospitalCode());
    ...
    public List<String> getPatientDetailsold(String patCRNo) {
        String query = null;
        ResultSet rs = null;
...
System.out.println("patCRNo"+patCRNo);
          dao = new HisDAO("Inv", "getPatientDetails");
        	
        	nQueryIndex = dao.setQuery(QueryConfig.FETCH_PATIENT_DTL);
        	
        	
   try {
         
		    dao.setQryValue(nQueryIndex, 1, patCRNo);
            
         
            
           rs = dao.executeQry(nQueryIndex);
           System.out.println("rs.next()>>"+rs.size());
            while (rs.next()) {
   ...
   public static String getcount(String crNo) {
   	//100 000
  	  ...
  	  String last5Digits = crNo.substring(2,7);
