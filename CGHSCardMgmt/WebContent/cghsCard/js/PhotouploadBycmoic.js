...
function initPage(){
  ...
  var configJson={
    				serviceName:"/getData/getfamilymembersactivatephotouploadbycmoic",
    				primaryKeys:[Benid],			
    				callBackFunctionName:"populatefamilymembers",
 ...
 //  alert("userid>>>>>>>>>"+userid);
    ...
    var requestData = {
        ...
        "dispshortname":dispshortname,
         "PPOdocname":PPOdocname,
    };

    // Send the AJAX request
    $.ajax({
        url: 'services/restful/cardapi/v1/BenDetails/getconvertimageBase64tofilebycmoic',  // URL of your REST service
        type: 'POST',
...
var ppovalue= $("#filename12").val();
	//	alert("ppovalue>>>>>>>"+ppovalue);
		if(ppovalue=='')
		{
			alert("Please Upload PPO")
			return;
		}
...
{
	   var configJson = {
        serviceName: "/DMLSAVE/UpdatePhotostatusbycmoic",
        callBackFunctionName: "callbackSaveDataUpdatePhotostatus",
        inputData: processSerializeArray("ENTRYFORM")
    };
...
function callbackSaveDataUpdatePhotostatus(configJson, dataJson){
...
//alert(JSON.stringify(configJson));
	if(dataJson["message"].indexOf("ERROR")>=0){
		...
		var dialogConfigJson={callOnOK:"resetPagecallbackSaveDataUpdatePhotostatus","parameterJson":{}}
...
function resetPagecallbackSaveDataUpdatePhotostatus(){
//alert("iinn reset page");
	alert("Application Approved")
}