...
var configJson={
			serviceName:"/getData/getNomdependentrelation",
			comboId:"nomrelation",			
			callBackFunctionName:"commonPopulateCombo",
			defaultOption:{"optionValue":"","optionText":"Select Relation"}	
...
*/
   ...
   // alert("Selected Date: " + formattedDate);  // Alert in dd-mm-yyyy format
    ...
    //  alert("deprelation>>>."+deprelation);
            if(age>18 && deprelation!=='2'){
		  //alert("innniff age");
		  $('#DepPAN').show();
	...
	*/
	...
	/*
	 $('#patsubcardtypeserving').change(function(){
		...
		if (valueofsubcategory==16 || valueofsubcategory==18)
    	{
...
if($('#cardtypevaluehidden').val()=='SA'){
 ...
 $('#patcardtypecategorypen').change(function(){
  ...
  // alert("valuesubcategory>>>>"+valuesubcategory);
    if (valuesubcategory==17 ||  valuesubcategory==21)
    	{
...
function resetPage(){
	$('#masterKey').val("getselectcardtype");
    ...
    const imgSrc = $("#fileContent2").val();
     ...
     const mainchdob = $('#patDOB').val();
		...
		// alert(mainchage);
		 ...
		 const depage = calculateAge(dependentdob);
			...
			//	alert(dependentIdProofdoctypeidvalue);
						if(dependentIdProofdoctypeidvalue=='')
						{
		...
		//alert(Dependentidproofdoctypename);
	            ...
	            var dependentadoptiondoctypeidvalue=$("#adoptiondoctypeid option:selected").text();
	          //  alert("dependentadoptiondoctypeidvalue"+dependentadoptiondoctypeidvalue);
     ...
     "dependentMarriagedoc":dependentMarriagedoc,
      ...
      "dependentadoptiondoctypeidvalue": dependentadoptiondoctypeidvalue,
...
if(json["dependentpannotavailbleundertaking"]){
    html += "<label class='form-label'>PAN Not Available Undertaking:</label>";  // Add hardcoded label here
    html += "<a class='uploadA' target='_blank' href='/CGHSCardMgmt/FormFlowXACTION?hmode=ftpFileDownload&fileName=" + json["dependentpannotavailbleundertaking"] + "&isGlobal=" + isGlobal + "&varSSOTicketGrantingTicket=" + ticket + "&folderName=CARDDOCUMENT' style='margin-left: 10px;'>" + json["dependentpannotavailbleundertaking"].split('_').slice(1).join('_') + "</a><br>";
}

if(json["depadopiontypedocname"]){
    html += "<span>" + json["dependentadoptiondoctypeidvalue"] + "</span>";
    html += "<a class='uploadA' target='_blank' href='/CGHSCardMgmt/FormFlowXACTION?hmode=ftpFileDownload&fileName=" + json["depadopiontypedocname"] + "&isGlobal=" + isGlobal + "&varSSOTicketGrantingTicket=" + ticket + "&folderName=CARDDOCUMENT' style='margin-left: 10px;'>" + json["depadopiontypedocname"].split('_').slice(1).join('_') + "</a><br>";
}
     ...
     "dependentMarriagedoc":"",
      ...
      "dependentAgeDoc":"",
       ...
       "depmobile":$("#patMobileNo").val(),
	        "dependentadoptiondoctypeidvalue": "",
        "depadopiontypeid":"",
       "depadopiontypedocname":"" 
	     } 
	 createDependentRow(json);
...
/*	if($('#divDependentEntry').is(":visible")){
   ...
   if(nomineeJson) 
   {
    ...
    // if dependent is not checked then creating self row and hiding dependent div
/*    if($('#customCheck12').prop('checked')==false) {
		$('#customCheck12').attr('checked',true);
	    showdependentpensioner();
	    $('#customCheck12').attr('checked',false);
	    $('#dependentinfo').hide();
	 }*/
	 
	/*    if($('#chkDependent1').prop('checked')==false) {
		$('#chkDependent1').attr('checked',true);
	    showdependentserving();
	    $('#chkDependent1').attr('checked',false);
	    $('#chkDependent1').hide();
	 }
	 */
         ...
         $("#dialogcghscity").html($("#PatientCghsCity option:selected").text())
           $("#dialogpincodedepon").text(patpincode);
        	...
        	if (cardtypevalue === 'P' || cardtypevalue === 'SA'){	
			   ...
			   $("#dialogcardtype").html($("#patcardtypecategorypen option:selected").text())
      ...
      var dateofretirement= $("#Patdorpensioner").val(); 
	   ...
	   var name = $("#patName").val();
           ...
           $("#mainchnameformdepon").text(mainname);
            ...
            $("#mainchnameformdeponundertaking").text(mainname);
          ...
          $("#dialoggen").html($( "#patGender option:selected" ).text())
                  ...
                  $("#dialogcghscitydepon").html($("#PatientCghsCity option:selected").text())
                     $("#dialogbasicpaydepo").html($("#patBasicpayserving option:selected").text())
                                       $("#dialogbasicpaydepoundertaking").html($("#patBasicpayserving option:selected").text())
	   ...
	   $("#dialogcardtypedeo").html($("#patcardcategoryserving option:selected").text())
	   
	        $("#dialogsubcardtypedeo").html($("#patsubcardtypeserving option:selected").text())
	    
	      $("#dialogdeptserving").html($("#patdeptserving option:selected" ).text())
	      $("#dialogsubdeptserving").html($("#patsubdeptserving option:selected" ).text())
		   ...
		   $("#diagogpayscaledepon").html($( "#patpayscaleserving option:selected" ).text())
		   		   $("#diagogpayscaledeponundertaking").html($( "#patpayscaleserving option:selected" ).text())
