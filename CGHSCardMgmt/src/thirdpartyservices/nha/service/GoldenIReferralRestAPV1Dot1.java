...
public class GoldenIReferralRestAPV1Dot1 { 
	...
	public Response getBeneficiaryEligibilty(NHARefRequest nhaRefRequest) {// System.out.println("inside referral api");
		...
		objFilterJson.put("processName", "/nha/tms/v1.0/benificary/getNHABeneficiaryRef");

		try {

			Usefulmethods objUsefulmethods = new Usefulmethods();
			String strServiceName = "service/getNHABeneficiaryRef";
			...
			if (objResponse != null) {
				...
				if (dataArray == null || dataArray.length() <= 0) {
					...
					response.setResponseMessage("The Referral ID provided is either incorrect, does not exist, or may have been deleted. Please check and try again !");
				...
				beneficiaryDetails.setBenId(commonDataObject.optString("benId"));
				beneficiaryDetails.setBenName(commonDataObject.optString("benName"));
				beneficiaryDetails.setDispensaryCode(commonDataObject.optString("dispensaryCode"));
				beneficiaryDetails.setDispensaryName(commonDataObject.optString("dispensaryName"));
					...
					for (int i = 0; i < dataArray.length(); i++) {
						...
						if (dataObject != null) {
							...
							if (i == 0) {
								...
								resultsItem.setRefHospitalName(commonDataObject.optString("dispensaryName"));
								...
								resultsItem.setRefRemarks(dataObject.optString("refRemarks"));
							...
							refCompDetails.get(refTypeCode).append(dataObject.optString("refComp"));
							...
							refCompRemarks.get(refTypeCode).append(StringUtils.isNotBlank(dataObject.optString("refCompRemarks")) ? dataObject.optString("refCompRemarks") : "NA");
