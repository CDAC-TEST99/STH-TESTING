...
public final class HisProcedure {
	...
	/**
	 * flag indicating whether err parameter is defined in procedure or not. It is
	 * mandatory parameter
	 */
	 ...
	 */
	/*
	 * public void setOutValue(String parameterName,int outputType) { //initialize
	 * the ArrayList if(this.proc_single_value == null) this.proc_single_value = new
	 * ArrayList<String>(2); if(this.proc_out_index == null) this.proc_out_index =
	...
	public Object getOutputValue(int paramIndex) throws Exception {
		...
		try {
			if (proc_out_value != null) {
				/*
				 * if(!proc_out_value.containsKey(parameterName)) { this.errMsg =
				 * "Invalid Parameter Name ( " + parameterName + " )"; throw new
				 * Exception(this.errMsg); } else
				 */
				...
				if (this.proc_single_value != null) {
					...
					for (int i = 0; i < lcl_col_size; i++) {
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							 ...
							 * cs.registerOutParameter(keyName,java.sql.Types.VARCHAR); else //web row set
							...
							{// web row set
								...
								String strProcName = ((this.proc.substring(0, this.proc.indexOf("("))).replace("{call",
										"")).trim();
								// System.out.println("ajay his dao" + strProcName);

								if (HisUtil.getParameterFromHisPathXML("PROC_NAMED_PORTAL_FOR_RESULTSET")
										.equalsIgnoreCase("ON")) {
									cs.setString(i + 1, strProcName + "(" + firstValue + ")");
								}
					...
					for (int i = 0; i < lcl_col_size; i++) {
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							...
							if (keyValue[1].equals("1")) // string
								...
								if (HisUtil.getParameterFromHisPathXML("PROC_NAMED_PORTAL_FOR_RESULTSET")
										.equalsIgnoreCase("ON")) {
									cs.setString(index, strProcName + "(" + firstValue + ")");
								}
	...
	/**
	 * This function executes a single procedure at a time which is in batch. It is
	 * called from {@link HisDAO#fire()} method. This method does not set the output
	 * parameter.
	 * 
	 * @param conn - Connection Object
	 * @throws Exception - if any Exception arrives at the time of Execution.
				...
				if (this.proc_multiple_value != null) {
					...
					for (int i = 0; i < this.colSize; i++) {
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							...
							if (Integer.valueOf(keyValue[1]) == 1) { // string
								this.cs.registerOutParameter(keyName, java.sql.Types.VARCHAR);
							}else {
								cs.setNull(keyName, java.sql.Types.NULL);
					...
					for (int i = 0; i < lcl_col_size; i++) {
						...
						if (keyValue[1].equals("1")) { // string return value
							...
							if (keyValue[0].toUpperCase().trim().equals("ERR")) {
								strTemp = (String) outValue;
								if (!strTemp.equals("") || strTemp.trim().length() > 0) {
									this.errMsg = this.proc + " >> Error thrown by procedure : " + strTemp;
									throw new Exception(this.errMsg);
								}
							}
					...
					for (int i = 0; i < this.colSize; i++) {
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							...
							if (Integer.valueOf(keyValue[1]) == 1) { // string
								...
								this.cs.setNull(index, java.sql.Types.NULL);
							...
							}else if (strProcValue == null || strProcValue.trim().length() == 0) { // Added By Pragya
																								// 2012.11.15
					...
					for (int i = 0; i < this.colSize; i++) {
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							...
							if (Integer.valueOf(keyValue[1]) == 1) { // string
								...
								this.cs.setNull(i+ 1, java.sql.Types.NULL);
