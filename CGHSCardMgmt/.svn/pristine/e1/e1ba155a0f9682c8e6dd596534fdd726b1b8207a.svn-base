...
<script type="text/javascript" src="/CGHSCardMgmt/cghsCard/js/Plasticcarddatadwbld.js"></script>
...
<form action="/CGHSCardMgmt/FormFlowXACTION" name="FormFlowXACTION" method="post" id='plasticCardForm'>
     ...
     <th style="width:5%"><b>S.No</b></th>
      ...
      <th style="width:10%">Name</th>
       ...
       <th style="width:10%">Relationship</th>
        ...
        <th style="width:10%">Card Color</th>
          <th>Photo</th>
              <th>Download Card</th>
          <th>Appointment</th>    
              <th>myEMR</th>
...
<button class='btn btn-his' id="BTNAPPROVE" onclick="DownloadExcel()"><i class='fas fa-save fa-fw' ></i>&nbsp;Download Data in Excel</button>
 ...
 <input type="hidden" name="strCrNo" id="strCrNo" />
 <input type="hidden" name="benLogFlag" id="benLogFlag" value="1" />
   ...
   <input type='hidden' name="Benidvalue" id='Benidvalue' value="<%=request.getParameter("Benidvalue")%>"  />  
	    ...
	    <%String primaryKeyFromListPage=(String) request.getSession().getAttribute("SEATID");
	     ...
	     <input type='hidden' name="primaryKeyFromListPage" id='primaryKeyFromListPage' value="<%=primaryKeyFromListPage %>"  /> 
	     	    ...
	     	    <%String varTicket=(String) request.getSession().getAttribute("tid");
	     	    
	     	 
				System.out.println("benEMr varTicketvarTicketvarTicket >>>>> " + varTicket);
	       if(varTicket==null){
	    	   varTicket="";
	       }	     	
	     %>
	    ...
	    // <input type='hidden' name="varTicket" id='varTicket' value="<%=varTicket %>"  />   
</form>