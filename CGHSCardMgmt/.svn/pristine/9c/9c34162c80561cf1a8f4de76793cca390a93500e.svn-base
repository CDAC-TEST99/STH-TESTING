...
function initPage() {
 ...
 var configJson={
  ...
  //console.log("inputData>>>> " +processSerializeArray("ENTRYFORM") );
	...
	//console.log("inputData>>>> "  , configJson.inputData.patMobileNo );
	//console.log("data ben id >>> " , configJson.inputData.Benid);
...
function callbackSaveData1(configJson, dataJson){
	...
	if(dataJson["message"].indexOf("ERROR")>=0){
		...
		var dialogConfigJson={callOnOK:"resetbenedit","parameterJson":{}}
...
$('#divDependentAddInfo').hide();
    ...
    var relation = cells[3].innerText;  // Assuming Relation is in the fifth column
