...
import HisWeb.util.CghsSmsManager;
...
public class FormFlowXACTION extends MainServlet {
	...
	public static void fileUpload(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
	...
	public static void ftpFileDownload(final HttpServletRequest request, final HttpServletResponse response) {
		try {
			FormFlowXUTIL.downloadFileFromFTP(request, response);
		} catch (Exception e2) {
			try {
				response.getWriter().append("<div style='text-align:center'><h1>File Not Found<h1></div>");
			} catch (IOException e1) {
	 ...
	 public static void sendSMS( HttpServletRequest request, HttpServletResponse response) { // Dynamic function to send sms
	        try {
	        	// System.out.println("Inside FORM FLOW X Send SMS CardMGMT " ); 
	        	
	        	String inputData =request.getParameter("inputData"); 
	        //	System.out.println("inputData>>>>  "+ inputData);
	        	new SMSClient().SendSMSClient(inputData);
				        } catch (Exception e) {
	            
	          //  System.out.println("Error occurred while after calling CGHS SMS Manager " + e.getMessage());
	            e.printStackTrace();
	        }
	    }

		public static void downloadMultipleFilesAsZip(final HttpServletRequest request, final HttpServletResponse response) {
			try {
				FormFlowXUTIL.downloadMultipleFilesAsZip(request, response);
			} catch (Exception e2) {
				try {
					response.getWriter().append("<div style='text-align:center'><h1>File Not Found<h1></div>");
				} catch (IOException e1) {
		...
		public static void downloadExcelFromBase64(final HttpServletRequest request, final HttpServletResponse response) {
			try {
				FormFlowXUTIL.downloadExcelFromBase64(request, response);
			} catch (Exception e2) {
				try {
					response.getWriter().append("<div style='text-align:center'><h1>File Not Found<h1></div>");
				} catch (IOException e1) {
		...
		public static void uploadExcel(final HttpServletRequest request,
		 final  HttpServletResponse response) { try { 
		 System.out.println("inside uploadExcel"); 
		 FormFlowXUTIL.uploadExcel(request,	 response); 
		 } catch (Exception e2) {
			 try 
		 { 
				 response.getWriter().append("<div style='text-align:center'><h1>File Not Found<h1></div>"); 
		...
		public static void uploadAirIndiaExcel(final HttpServletRequest request,
				 final  HttpServletResponse response) { try { 
				 System.out.println("inside uploadAirIndiaExcel"); 
				 FormFlowXUTIL.uploadAirIndiaExcel(request,	 response); 
				 } catch (Exception e2) {
					 try 
				 { 
						 response.getWriter().append("<div style='text-align:center'><h1>File Not Found<h1></div>"); 
