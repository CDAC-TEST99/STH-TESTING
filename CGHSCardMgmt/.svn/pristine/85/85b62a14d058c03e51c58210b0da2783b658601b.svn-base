...
function uploadDoc(fileId,folderName,mandatory) {
	...
	if ((regex.test(filename))) 
	{
		...
		if (oFile.size > 2097152) //2,097,152 for 2mb  //200000 for 200 kb
		...
		var encodedFileContent=null;
		 if (oFile) {
		        ...
		        r.onload = function(e) {
		    		...
		    		$
		    				.ajax({
		    					url : '/CGHSCardMgmt/FormFlowXACTION?hmode=fileUpload&isGlobal='+isGlobal+'&varSSOTicketGrantingTicket='+ticket +'&tkn='+token+'&folderName='+folderName,
		    					type : "POST",
		    					data : formData,
		    				...
		    				.done(
		    						function(data) {
		    							...
		    							if(data=="1"|| data=="2" || data=="3"|| data=="4" ||data=="5"||data=="6"||data=="7"){
		    								...
		    								/*var strhtml="<link rel='stylesheet' href='/CGHSCardMgmt/global/template/dist/css/adminlte.min.css'>";
		    								strhtml+="<link type='text/css' rel='stylesheet' href='/HIS/global/error/error.css' />";
		    								strhtml+="<link rel='stylesheet' href='/CGHSCardMgmt/global/template/plugins/fontawesome-free/css/all.min.css'>";
...
function fileUploadSuccessWithoutView(data,fileId,folderName,mandatory) {
	...
	var ticket=$('#varSSOTicketGrantingTicket').val()!=undefined?$('#varSSOTicketGrantingTicket').val():"";
	var url="/CGHSCardMgmt/FormFlowXACTION?hmode=fileTempDownload&fileName="+data+"&isGlobal="+isGlobal+"&varSSOTicketGrantingTicket="+ticket+"&folderName="+folderName;
