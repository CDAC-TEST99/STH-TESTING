...
public class GlobalEssentialBO implements GlobalEssentialBOi
{
	...
	public List getPatientAuditLogEssentials(UserVO userVO){
		JDBCTransactionContext tx = new JDBCTransactionContext();
		List patientAuditLogVOList=null;
		try
		{
			tx.begin();
			//PatientAuditLogMstDAO dao = new PatientAuditLogMstDAO(tx);
			//patientAuditLogVOList = dao.selectPatientAuditLog(userVO);
		}
		catch (HisRecordNotFoundException e)
		{
			tx.rollback();
	...
	public Map getPatientAuditLogDataList(List<PatientAuditLogMstVO> patientAuditLogVOList, String patCrNo, UserVO userVO){
		
		JDBCTransactionContext tx = new JDBCTransactionContext();
		List auditLogList=null;
		List allAuditRecordList=null;
		Map map=new HashMap();
		Map rowColumnMap=new HashMap();
		Map auditLogHeaderDataMap=new HashMap();
		try
		{
			tx.begin();
			GlobalEssentialDAOi globalEssentialDAO = new GlobalEssentialDAO(tx);
			PatientAuditLogMstDAO auditLogDAO = new PatientAuditLogMstDAO(tx);
			/*if(!globalEssentialDAO.isCrNoExists(patCrNo, userVO)){
				throw new HisRecordNotFoundException("CR No is Not Valid");
			}*/
			
			Iterator itr=patientAuditLogVOList.iterator();
			while (itr.hasNext()) {
				allAuditRecordList=new ArrayList();
				PatientAuditLogMstVO patientAuditLogMstVO = (PatientAuditLogMstVO) itr.next();
				rowColumnMap=auditLogDAO.selectPatientCurrentRecord(patientAuditLogMstVO, patCrNo, userVO);

				if(patientAuditLogMstVO.getDisplayLogic().equals(Config.AUDIT_LOG_DISPLAY_LOGIC_NEEDED_FOR_BASERECORD)){
					allAuditRecordList=auditLogDAO.selectPatientBaseRecord(patientAuditLogMstVO, patCrNo, userVO);
				}
				auditLogList = auditLogDAO.selectPatientAuditLogDetail(patientAuditLogMstVO, patCrNo, userVO);
				allAuditRecordList.addAll(auditLogList);
				rowColumnMap.put(Config.AUDIT_LOG_ROW_LIST, allAuditRecordList);
				
				auditLogHeaderDataMap.put(patientAuditLogMstVO.getModuleId()+"#"+patientAuditLogMstVO.getAuditLogId(), rowColumnMap);
			}
	...
	{	
		...
		List mongoDBEssentials = new ArrayList();
		
		JDBCTransactionContext tx = new JDBCTransactionContext();
		try {
			tx.begin();
			//EssentialDAO objEssentialDAO = new EssentialDAO(tx);
			GlobalEssentialDAO objEssentialDAO = new GlobalEssentialDAO(tx);
			mapData = objEssentialDAO.getMongoDBConfigurationsfromDB();
			/*mapData.put(RegistrationConfig.MONGODB_ESSENTIALS,mongoDBEssentials);*/
		}
	...
	@Override
	public hisglobal.business.MasterVerificationVO getMasterVerification(
			hisglobal.business.MasterVerificationVO masterVerificationVO, UserVO userVO) {
