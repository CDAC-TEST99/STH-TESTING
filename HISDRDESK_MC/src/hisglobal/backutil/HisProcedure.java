	...
	public void executeProcByPosition(Connection conn) throws Exception {
		...
		try {
			if(!this.proc.equals("")) {
				...
				if(this.proc.contains("PROC_GBLT_OCCUPATION_MST")) {
					
					System.out.println("PROC_GBLT_OCCUPATION_MST");
				...
				if(this.proc_multiple_value != null) {
					...
					for(int i = 0;i<this.colSize;i++) { 
						...
						if(keyValue[0].equals("OUTPARAMETER")) {
						 ...
						 else if(strProcValue==null || strProcValue.trim().length() == 0) // Added By Pragya 2012.11.15 
