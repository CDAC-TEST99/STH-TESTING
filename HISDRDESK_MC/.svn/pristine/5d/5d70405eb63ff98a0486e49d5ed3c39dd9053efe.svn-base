...
/* 		$(this).parent().parent().parent().find('.checkedInput:checked').parent().parent().remove();*/
	...
	/*$("input[name=txt-snomed-ct-search_VR2]").on('keypress', function (e) {
 	...
 	$('input[name=visitDate]').attr('min',dtVal.getFullYear()+"-"+curMon +"-"+curDate);
 	document.getElementById('visitDateId').valueAsDate = new Date();
 	...
 	$('.drugsAdviceAdd').click(function(){	
 		...
 		$('input[name=drugsAdvices]').each(function()
		{
 			...
 			var drugEpiCode = $(this).val().split('&&')[9];
 			var drugVisitNo = $(this).val().split('&&')[10];
 			var mode = $(this).val().split('&&')[11];
			//console.log('value '+$(this).val());
			//console.log('Epi Cde val='+$('#patEpisodeCodePrescriptionPanel').text());
			//console.log('Visit NO='+$('#patEpisodeVisitNoPrescriptionPanel').text());
			//console.log('testCode ='+testCode);
			console.log(drugEpiCode);
			if(drugEpiCode===$('#patEpisodeCodePrescriptionPanel').text() && drugVisitNo === $('#patEpisodeVisitNoPrescriptionPanel').text()){
				reason =1;
				drugMode = mode;
			}				
		});
 		 ...
 		 if(drugInstructions=='')
 			drugInstructions = ' ';
 		 var drugAdmId = $(this).parent().parent().find('input[name=drugAdmId]').val();
 		 if(drugAdmId =='')
 			drugAdmId ='0';
 		 var drugAdmName = $(this).parent().parent().find('input[name=drugAdmName]').val();
 		 if(drugAdmName =='Select'|| drugAdmName =='')
 			drugAdmName='--';
 		 var drugRouteId = $(this).parent().parent().find('input[name=drugRouteId]').val();
 		 if(drugRouteId =='')
 			drugRouteId ='0';
 		 var drugRouteName = $(this).parent().parent().find('input[name=drugRouteName]').val();
 		 if(drugRouteName =='Select'|| drugRouteName =='')
 			drugRouteName='--';
 		 var tmp = 0; 
			$('#drugAdviceListTable tbody').find('tr').each(function(index){
				if($(this).find('td').eq(0).children('input').val().split('&&')[1].split('#')[0]==drugId.trim().split('#')[0] && reason==1) 
				{	tmp = 1; 
					return false;  }
			});
		...
		{ 
			if(drugMode==="EDIT")
				swal("The drug you are trying to add is already advised today");
			else
				swal("Already Added!!");
			 ...
			 $(this).parent().parent().find('input[name=drugAdmId]').val('');
			 $(this).parent().parent().find('input[name=drugAdmName]').val('');
			 $(this).parent().parent().find('input[name=drugRouteId]').val('');
			 $(this).parent().parent().find('input[name=drugRouteName]').val('');
			return false;
		} 
 		 ...
 		 {    
 			...
 			var HiddenDrugAdvice=drugName+'&&'+drugId+'&&'+drugDosage+'&&'+drugDosageID+'&&'+drugFrequency+'&&'+drugFrequencyId+'&&'+drugStartDate+'&&'+drugDays+'#'+drugQuantity+'&&'+drugInstructions+'&&'+$('#patEpisodeCodePrescriptionPanel').text()+'&&'+$('#patEpisodeVisitNoPrescriptionPanel').text()+'&&NEW&&'+drugAdmId+'&&'+drugRouteId+'&&'+drugRouteName;
   	...
   	//	  console.log('drugDays::::::::'+drugDays);
   	//	console.log('drugDays --- '+ $('input[name=drugDays]').val());
   	//	  $('input[name=drugDays]').val(drugDays);
			...
			/* $(this).parent().parent().find('input[name=drugStartDate]').val('');*/
		 	// document.getElementById('drugStartDate').valueAsDate = new Date();		//commented by ashutoshk to continue with date of last drug added
			// $(this).parent().parent().find('input[name=drugDays]').val('');			//commented by ashutoshk to continue with days of last drug added
			 $(this).parent().parent().find('input[name=drugQuantity]').val('');
			 $(this).parent().parent().find('input[name=drugInstructions]').val('');
			 $(this).parent().parent().find('input[name=drugAdmId]').val('');
			 $(this).parent().parent().find('input[name=drugAdmName]').val('');
 	...
 	$('.otListAdd').click(function(){
 		var operationDate = $(this).parent().parent().find('input[name=operationDate]').val();
 		var operation = $(this).parent().parent().find('select[name=operation] option:selected').text();
 		var operationId = $(this).parent().parent().find('select[name=operation]').val();
 		var surgeonId = $(this).parent().parent().find('select[name=Surgeon]').val();
		var surgeon = $(this).parent().parent().find('select[name=Surgeon] option:selected').text();
		var optFindings = ($('#operationFindings').val()!='')?$('#operationFindings').val():' ';
		var complication = ($('#complication').val()!='')?$('#complication').val():' ';
		var implant = ($('#implant').val()!='')?$('#implant').val():' ';
		var pop = ($('#pop').val()!='')?$('#pop').val():' ';
		var pohp = ($('#pohp').val()!='')?$('#pohp').val():' ';
		...
		console.log("optFindings=="+optFindings);
		console.log("complication=="+complication);
		console.log("implant=="+implant);
		console.log("pop=="+pop);
		console.log("pohp=="+pohp);
		
		if(operationDate =='')
		{
			swal("Operation date should not be left blank");
			return false;
		}
		if(operationId =='')
		{
			swal("Please select the Operation");
			return false;
		}
		if(surgeonId =='')
		{
			swal("Please select Surgeon");
			return false;
		}
		
		if(operationDate!='' && operationId!='' && surgeonId!='')
		{
			var months = new Array(12);
		 	...
		 	months[11] = "Nov";
		 	months[12] = "Dec";
		 	var tmp = new Date(operationDate);
			var tmpDate=tmp.getDate()+'-'+months[tmp.getMonth()+1]+'-'+tmp.getFullYear();
			...
			var otJson = {
					"operationDate" : 	tmpDate ,
					"operationId"	:	operationId ,
					"operation"		:	operation ,
					"surgeonId"		:	surgeonId ,
					"surgeon"		:	surgeon ,
					"optFindings"	:	optFindings ,
					"complication"	:	complication ,
					"implant"		:	implant ,
					"pop"			: 	pop ,
					"pohp"			:  	pohp	
			};
			console.log('otJson =='+JSON.stringify(otJson));	
			
			$(this).parent().parent().parent().parent().find('#otListTable').children('tbody').append('<tr> <td><input type="checkbox" class="checkedInput" name="otCheck" value="'+hiddenOt+'"  checked><i class="text1" style="display :none">'+JSON.stringify(otJson)+' </i></td><td>'+tmpDate+'</td><td>'+operation+'</td><td>'+surgeon+'</td><td><a class="OtModalBtn" style="color: #109f1c" headerTxt="Findings" otDtlTxt="'+optFindings+'" onclick="$(\'#opertationTheatreModal\').modal(\'show\');">'+optFindings.substring(0, 4)+'..'+'</a></td><td><a class="OtModalBtn" style="color: #109f1c" headerTxt="Complications" otDtlTxt="'+complication+'" onclick="$(\'#opertationTheatreModal\').modal(\'show\');">'+complication.substring(0, 4)+'..'+'</a></td><td><a class="OtModalBtn" style="color: #109f1c" headerTxt="Implant" otDtlTxt="'+implant+'" onclick="$(\'#opertationTheatreModal\').modal(\'show\');">'+implant.substring(0, 4)+'..'+'</a></td><td><a class="OtModalBtn" style="color: #109f1c" headerTxt="Pre-op Preparation" otDtlTxt="'+pop+'" onclick="$(\'#opertationTheatreModal\').modal(\'show\');">'+pop.substring(0, 4)+'..'+'</a></td><td><a class="OtModalBtn" style="color: #109f1c" headerTxt="Post-op Hospital Stay Course" otDtlTxt="'+pohp+'" onclick="$(\'#opertationTheatreModal\').modal(\'show\');">'+pohp.substring(0, 4)+'..'+'</a></td></tr>');
			...
			document.getElementById('operationDate').valueAsDate = new Date();
	 		$(this).parent().parent().find('select[name=operation]').val('');
	 		$(this).parent().parent().find('select[name=Surgeon]').val('');
			$('#operationFindings').val('');
			$('#complication').val('');
			$('#implant').val('');
			$('#pop').val('');
			$('#pohp').val('');
 		...
 		// alert(drugFrequency+drugFrequencyId);
 		 ...
 		 {  
 			...
 			drugDays= (drugDays=='') ? '0' : drugDays ;
	 			console.log('drugDays::::::::'+drugDays);
	 			if(externalDrugQuantity=='' || externalDrugQuantity=='0' ){
					swal("Quantity Must be Greater than Zero!!");
					return false;
				}
...
/* 	$('.drugEditBtn').click(function(){
	  ...
	  {  
		 ...
		 $(this).parent().parent().find('#InvestigationRemarksId').val('');
		  ...
		  //$(this).parent().parent().parent().find('.investigationAdded').append('<a style="padding-left: 5px;" class="'+$('input[name=externalInvestigation]').val().trim()+'"><label> <input type="checkbox" class="checkedInput" name="reasonOfVisit" value="0^0^0^bld"  checked> '+$('input[name=externalInvestigation]').val().trim()+'</label></a>');
			 ...
			 var txtInvestList="0^0^0^0^0^0^0^"+$('#patEpisodeCodePrescriptionPanel').text()+"^"+$('#patEpisodeVisitNoPrescriptionPanel').text();
			 console.log(isValid+"---"+JSON.stringify(InvestigationDtlsJson));
			 var sideremks=' ';
			 if($("#investigationSiteId").val() !='' && $("#investigationSiteId").val() !='0')
				 sideremks = sideremks + '('+ siteId + ')' ;
 	...
 	$('.nextPatientPresc').click(function(e){
		...
		var admno=$('#patAdmNoPrescriptionPanel').text();
		...
		getPrevData(CR_No,episodeCode,hospitalCode,lastVisitDate,admno ,$('input[name=eTeleconsultancyreq]').val(),visitNo);
		...
		$('#allergiesModal .close').click(function(){
		 ...
		 if($('#isAdmittedHiddenValId').val()=='18'){
			 $('#vitalModify').hide();
			 $('#saveBtn').text('Discharge Prepared');
			 $('#saveBtn').prop('disabled', true);
			 $('#hoplCrsFileUpload').hide();
			 $('#administrationBtn').hide();
			 $('#strDyanmicHistoryTemplateId').hide();
			 $('#strDyanmicExaminationTemplateId').hide();
			 $('#strDyanmicComplaintTemplateId').hide();
		 }
			 
		 else{
			 $('#saveBtn').show();
			 $('#vitalModify').show();
			 $('#hoplCrsFileUpload').show();
			 $('#administrationBtn').show();
			 $('#strDyanmicHistoryTemplateId').show();
			 $('#strDyanmicExaminationTemplateId').show();
			 $('#strDyanmicComplaintTemplateId').show();
		 }
			 
		 
		 openIPDPastRxDetailModal();
 });	 
...
{ 
	...
	if(freq!= -1 && days!="")
	{
		...
		console.log("freq=="+freq+"&&dos=="+dos+"&&days=="+days);
		calcquan=freq*dos*days; 
	...
	function Save(mode,e , flg)
	{
		...
		$('input[name=referalchk]:checked').each(function()
				{
					...
					var strrefrealId=$(this).parent().find('i').text().split("^")[0];						
					Referal[y]= JSON.parse(strrefrealId); 
					y++; 
				});
		console.log("Referal");
		console.log(Referal);
		z=0;
		 $('input[name=otCheck]:checked').each(function()
			{
				console.log('otCheck::>>> '+$(this).val());
				OtDtl[z] = $(this).val();						
				OtJsonArray[z]= JSON.parse($(this).parent().find('i').text()); 
				z++; 
			});
			console.log("OtDtl");
			console.log(OtDtl);
		...
		var plannedVisitDate = '';//$('input[name=visitDate]').val(); 
		...
		var courseNote=$("#courseNote").val();
		var insTreatNote=$("#insTreatNote").val();
		var hoplCrsVisitDate=$("#visitDateId").val()+":"+$("#visitTimeHourId").val()+":"+$("#visitTimeMinId").val();
		...
		*/
		...
		var prescJSON = {
			...
			"patQueueNo": '0',//patGeneralDtl[3].split('/')[3],
			"hosp_code":hospCode,	
			...
			"admissionNo" : $('#patAdmNoPrescriptionPanel').text() ,
			"courseNote" : courseNote ,
			"insTreatNote" : insTreatNote,
			"hoplCrsVisitDate" : hoplCrsVisitDate ,
			"OtDtl" : OtDtl
		...
		var FormattedJson ={
				...
				"PatientQueueNo"				:	'0',//patGeneralDtl[3].split('/')[3],
				"hosp_code"						:	hospCode,	
				"hrgnum_is_docuploaded"			:	0,
				...
				"insTreatNote" : insTreatNote,
				"hoplCrsVisitDate" : hoplCrsVisitDate,
				"OtJsonArray" :	OtJsonArray				
			
		};
			console.log('FormattedJson');
		console.log($('#patEpisodeVisitNoPrescriptionPanel').text());
			...
			else 
			{
				...
				$('#printPrescriptionModal .modal-body').prepend('<iframe id="printPrescFrameId" style="width:100%;height:80vh;" src="/HISDRDESK_MC/new_opd/transaction/IPDDoctorDeskAction.cnt?hmode=PRINTPRESC"></iframe>');
	...
	$(document).ready(function(){
		  ...
		  window.admNo = $('#patAdmNoPrescriptionPanel').text();
		...
		$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getPatDataForPastPrescription?Modval=2&CrNo='+window.patCrNo+'&episodeCode='+window.episodeCode +'&visitNo='+window.visitNo+'&seatId=&Entrydate=&hosp_code='+window.hospCode+'&admNo='+window.admNo+'',
			async:true,
				...
				success:function(data){ 
					...
					if(Object.keys(data.pat_details).length > 0){
						...
						{   
							...
							//console.log('jsondata ashukkk ---- '+ JSON.stringify(data.pat_details[i].HRSTR_JSON_DATA));
						    ...
						    var tempdate =	data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim().split('#')[7].split('-');
						 	...
						 	//var maxDate1 =    tempdate[0]+"-"+tempdate[1] +"-"+tempdate[2].substring(2,4)+" {"+count+"} {"+data.pat_details[i].DATASAVE_TIME.split(" ")[1].trim()+"}";
							...
							//console.log('HRSTR_JSON_DATA : '+data.pat_details[i].HRSTR_JSON_DATA);
							...
							str+='<div class="row"><div style="float:left"> <h4 class="text-center" style="font-weight:bold"> '+
										'<img alt="" src="/HIS/hisglobal/images/e-clinic/sikkimlogo.jpg" style="height: 56px"></h4> </div><br>'+
						...
						if(Object.keys(data.pat_details).length > 0){
							...
							if(data.pat_details[0].HRSTR_JSON_DATA.patGender.trim() == 'M'){
								strProfileInfo+='<div class="col-md-3" style="">'+
								  '<img src="/HISDRDESK_MC/hisglobal/images/MaleIcon.png" style="height:108px;width:108px;">'+
								'</div>';
							}
							else if(data.pat_details[0].HRSTR_JSON_DATA.patGender.trim() == 'F'){
								strProfileInfo+='<div class="col-md-3" style="">'+
								  '<img src="/HISDRDESK_MC/hisglobal/images/FemaleIcon.png" style="height:108px;width:108px;">'+
								'</div>';
							}
							...
							var referalNav = '<li><a href="#TreeStructurePrescriptionModalTab16" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-ambulance"></i> Patient Ref.</a></li>';
							if(strClinicalProcedure!='')
								referalNav = '<li><a href="#TreeStructurePrescriptionModalTab16" data-toggle="tab"><i class="fas fa-ambulance"></i> Patient Ref.</a></li>';
							var confNav = '<li><a href="#TreeStructurePrescriptionModalTab17" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-ambulance"></i>Confidential info.</a></li>';
							if(strconfidentialInfo!='')
								confNav = '<li><a href="#TreeStructurePrescriptionModalTab17" data-toggle="tab"><i class="fas fa-ambulance"></i>Confidential info.</a></li>';
							var tempInfoNav = '<li><a href="#TreeStructurePrescriptionModalTab18" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-shield"></i> Template Info.</a></li>';
							if(fileName!='')
								tempInfoNav = '<li><a href="#TreeStructurePrescriptionModalTab18" data-toggle="tab"><i class="fas fa-user-shield"></i> Template Info.</a></li>';
							var procAdmNav = '<li><a href="#TreeStructurePrescriptionModalTab19" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-shield"></i>Procedure Administration</a></li>';
							if(fileName1!='')
								procAdmNav = '<li><a href="#TreeStructurePrescriptionModalTab19" data-toggle="tab"><i class="fas fa-user-shield"></i>Procedure Administration</a></li>';
	...
	*/
	...
	$('#referralTypeId').change(function(){
		 ...
		 if(strFlag == 5 ){
			 ...
			 $('#strEmpanelledDepartmentListDiv').show();
				$('#strExternalrefferalInstituteDivId').show(); 
				$('#strExternalrefferalDaysDivId').show(); 
				$('#strExternalrefferalReasonDivId').show();
				$('#referralReasonDivId').hide();
				$('#externalReferralReasonDivId').show();
			 	
			 	$('#strExternalrefferalHospitalDivId1').hide();
				$('#strExternalrefferalHospitalDivId').hide();
				$('#strExternalDivisionListDiv').hide();
				$('#strExternalrefferalZonesListDivId').hide();
		 ...
		 if(strFlag == 4 ){
			 ...
			 $('#strEmpanelledDepartmentListDiv').hide();
			 $('#strExternalrefferalDaysDivId').hide(); 
			 $('#strExternalrefferalReasonDivId').hide();
			 $('#strExternalrefferalInstituteDivId').hide(); 
			 console.log("Railtel Flag==="+$('#strRailTailFlgId').val());
			 if($('#strRailTailFlgId').val() == '1')
			 {
			 	$('#strExternalrefferalZonesListDivId').show();
			 	$('#strExternalrefferalHospitalDivId1').show();
			 	$('#strExternalDivisionListDiv').show();
			 	$('#externalReferralReasonDivId').show();
			 	$('#referralReasonDivId').hide();
			 	//$('#strExternalrefferalHospitalDivId').hide(); 
			 }	  
			 else
			 {
			 	...
			 	$('#externalReferralReasonDivId').hide();
			 	$('#referralReasonDivId').show();
			 ...
			 if($('#strRailTailFlgId').val() == '1'){
				// Getting zone list data
				 $('#strExternalrefferalZoneId').html('');
				 $.ajax({
		                type: "GET",
		                url: '/HISServices/service/railtelService/getZoneList',
		                //data: ({subSystemId : $('#subSystem'+elementIdIndex+'').val() }),
		                dataType:'json',
		                async: true,
		                success: function(items) {
		                    //console.log(items);
		                    if(items.status==1 && items.zone_details!=null)
		                 	   {
		                     	   if(items.zone_details.length>0)
		                     	   {
		                     		  $('#strExternalrefferalZoneId').append($('<option>', {value: '', text: 'Select Zone'}));
		                     		  for(var j=0;j<items.zone_details.length;j++)
		                             	{
		                                 	...
		                                 	$('#strExternalrefferalZoneId').append($('<option>', {value: items.zone_details[j].ZONE_ID, text: items.zone_details[j].ZONE_NAME}));
		                                 	}
		                     	   }
		                     	   else
		                     		{
...
/*$('#strExternalrefferalInstituteId').change(function(){
	...
	//	if(!localStorage.getItem('ClinicalProcedureObj'))
		...
		console.log(clinicalServiceAreaIdva);
			...
			for (i = 0; i < ClinicalProcedureObj.length; i++) {
			 ...
			 console.log('ClinicalProcedureObj '+jsonarray);
	...
	function  getDrugAdvice(){
		...
		$('#drugAdmId').val($('#drugAdministration').val());
		$('#drugAdmName').val($('#drugAdministration option:selected').text());
		$('#drugRouteId').val($('#drugRoute').val());
		$('#drugRouteName').val($('#drugRoute option:selected').text());
	...
	function openIPDPastRxDetailModal()
 	{
		//$('#ipdEmrModal').remove();
	var patCrNo = $('#patCrNoPrescriptionPanel').text();
	var episodeCode = $('#patEpisodeCodePrescriptionPanel').text();
	var visitNo = $('#patEpisodeVisitNoPrescriptionPanel').text();
	var seatId = $('#patSeatIdPrescriptionPanel').text();
	var hospCode = $('#patHospitalCodePrescriptionPanel').text();
	var deptName = $('#patDeptName').text();
	var admNo = $('#patAdmNoPrescriptionPanel').text();
	var todaysys = new Date(); 
	 var dd = todaysys.getDate();
	 dd = dd.toString().length>1 ? dd : "0"+dd;
	...
	$
			.ajax({
				url : '/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=2&CrNo='
						+ patCrNo
						+ '&episodeCode='
						+ episodeCode
						+ '&visitNo='
						+ visitNo
						+ '&seatId=&Entrydate=&hosp_code='
						+ hospCode
						+ '&admNo=' + admNo + '',
				async : true,
				beforesend : $('.patPreviousIpdPrescriptionList')
						.parent()
						.append(
								'<p id="ipdPrescriptionListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
				success : function(data) {
					 
					var str = '';
					var strHistory = '' , strExaminations = '' , strChronics = '' , strAllergies = '' , strClinicalProc = '' , strVitalsInfo = '' ;
					var count = 1;
					var menuId = 'ipdEmrModalNavMenuItem';
					//alert(Object.keys(data).length);
					//alert(Object.keys(data.pat_details).length);
					var strEpisodeCodeChk=episodeCode;
					console.log('======:::::::::::::::::::::::::::::::::::::::::::::::======data.pat_details'+Object.keys(data.pat_details).length);
					//console.log('data ashukk ---- '+ JSON.stringify(data.pat_details));
					if(Object.keys(data.pat_details).length > 0){
						
						$('.ipdEmrModalNavMenu').children('li').remove();
						$('.ipdEmrModalNavMenuContent').children('div').remove();
						
						$('#ipdPrescriptionListMsg').remove();
						for(var i=0;i<Object.keys(data.pat_details).length;i++)
						{   
							var visitDate = data.pat_details[i].HRSTR_JSON_DATA.currentVisitDate.trim();
							var temp = visitDate.split("/");
							...
							*/
							var months = new Array(12);
						 	...
						 	months[11] = "Nov";
						 	months[12] = "Dec";
						    var tempdate =	data.pat_details[i].TO_CHAR.split("-");//data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim().split('#')[7].split('-');
						 	var maxDate = temp[0]+" "+months[temp[1]] +", "+temp[2];
						 	//var maxDate1 =    tempdate[0]+"-"+tempdate[1] +"-"+tempdate[2].substring(2,4)+" {"+count+"} {"+data.pat_details[i].DATASAVE_TIME.split(" ")[1].trim()+"}";
						 	var maxDate1 =    tempdate[0]+"-"+tempdate[1] +"-"+tempdate[2].substring(2,4)+" {"+data.pat_details[i].DATASAVE_TIME.split(" ")[1].trim()+"}";
							...
							$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getIpdPatVitalDataForDtlPrescription?Modval=3&CrNo='+patCrNo+'&episodeCode='+episodeCode +'&visitNo='+visitNo+'&seatId=&Entrydate=&hosp_code='+hospCode+'&admNo='+admNo+'',
								async:false,
								success:function(result){ 
										...
										if(Object.keys(result.pat_vital_details).length > 0){
											...
											for(var j=0;j<Object.keys(result.pat_vital_details).length;j++){   
												...
												if(result.pat_vital_details[j].TO_CHAR == data.pat_details[i].TO_CHAR){
													if(j<7){
												 		strVitalsInfo+='<div class="row"><div class="col-sm-12"><p><b> VITALS : </b><small class="vitalsInfo1">';
							...
							$('.ipdEmrModalNavMenu').append('<li><a href="#'+menuId+(i+1)+'" data-toggle="tab">'+maxDate1+'</a></li>'); 
							console.log('data.pat_details[i].TO_CHAR : '+data.pat_details[i].TO_CHAR);
							//console.log('HRSTR_JSON_DATA : '+data.pat_details[i].HRSTR_JSON_DATA);
							console.log('HOSPNAME -- '+data.pat_details[i].HOSPNAME);
							console.log('HOSPADD -- '+data.pat_details[i].HOSPADD);
							console.log('HRSTR_JSON_DATA -- '+ JSON.stringify(data.pat_details[i].HRSTR_JSON_DATA));
							str+='<div style="right:8px; position:fixed">'+
							'<input type="button" onclick="printDiv(\''+menuId+(i+1)+'\');" value="Print" class="btn btn-info previousIPDPrescPrintBtn1 pull-right" id="printIPDPastPrescBtn1'+(i+1)+'"/>'+'</div>';
							
							str+='<div id="printIPDPrescPage_tabContent'+(i+1)+'" class="printIpdPreviousPrescPage">';
							str+='<div class="row"><div style="float:left"> <h4 class="text-center" style="font-weight:bold"> '+
										'<img alt="" src="/HIS/hisglobal/images/e-clinic/sikkimlogo.jpg" style="height: 56px"></h4> </div><br>'+
							 			'<div style="float:left;text-align:center">'+
							 			  '<h4><b>'+data.pat_details[i].HOSPNAME+'</b></h4> '+
									...
									'</div>'+  
									'<div class="" style="border-top: 1px solid grey; border-bottom: 1px solid grey;">'+
									'<table class="table table-condensed printIpdPrescPatDtlTbl table-responsive">'+
									'<tbody><tr>'+
									' <th colspan="5">'+
									'<table align="center">'+
									'	<tr><td></td><td><h4><font color="blue">IPD Prescription</font></h4></td><td></td></tr>'+
									'</table>'+
								'</th>'+
									'<tr>'+
									'<th>Name</th><td class="ipdPatName">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.pat_Name.trim();
							str+='</td><th>CR No.</th><td class="ipdPatCrNo">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.CR_No.trim();
							str+='</td></tr><tr><th>Age/Gender</th><td class="ipdPatAgeGen">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.patAge.trim()+'/'+data.pat_details[i].HRSTR_JSON_DATA.patGender.trim();
							str+='</td><th>Patient Category</th><td class="ipdPatCat">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.patCat.trim();
							str+='</td></tr><tr>'+
								'<th>Father/Spouse Name</th><td class="ipdPatRelName">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.patGaurdianName.trim();
							str+='</td><th>Department</th><td class="ipdPatDeptU">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.patDept.trim();
							str+='</td></tr><tr><th>Admission Date</th><td class="ipdPatVisitDate">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim().split('#')[7];
							str+='</td><th>Consultant</th><td class="ipdConsultantName">';
							str+=data.pat_details[i].HRSTR_JSON_DATA.patConsultantName.trim();
							str+='</td></tr></tbody></table></div>'; 
							...
							str+='<div class="row">'+
							'<div class="col-sm-12">'+
								'<p><b>CHIEF COMPLAINT : </b><small class="ipdReasonOfVisit">';
								for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.ReasonOfVisit.length;j++){
							...
							console.log("Diagnosis.log::::::::::::::::::1");
								...
								'<div class="col-sm-12">'+
									'<p><b>INVESTIGATIONS ADVISED : </b></p>'+
									'<ul class="ipdInvestigation" id="ipdMenu1">';
								for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint.length;j++){
									var temp=data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split("^");
									var tempinvs=(data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^').length ==6 ?  data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^')[5].trim():data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^')[7].trim()) ;
									str+='<li>'+ (j+1)+') ' +temp[5].trim() +' ,</li>';
								...
								str+='<div class="row">'+
								'<div class="col-sm-12">'+
									'<p style="text-align: justify;"><b>TREATMENT ADVICE : </b><small class="ipdClinicalNote">';
							...
							'<br>';
								...
								'<div class="col-sm-12">'+
									'<p style="text-align: left;font-size: small;font-family: cursive;margin-bottom: 0px;">Rx</p>'+
									'<ol class="printIpdPrescTreatmentLst"> ';
								for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.DrugCodeCat.length;j++){
							...
							if("strDrugAllergy" in (data.pat_details[i].HRSTR_JSON_DATA)){	
								if(data.pat_details[i].HRSTR_JSON_DATA.strDrugAllergy !=''){
									...
									'<div class="col-sm-12">'+
										'<p><b>DRUG ALLERGY : </b></p>'+
										'<ul class="ipdDrugallergy" id="menu1">';
									for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.strDrugAllergy.length;j++){
										var temp=data.pat_details[i].HRSTR_JSON_DATA.strDrugAllergy[j].strAllergyName;
										str+='<li>'+ (j+1)+') ' +temp+' ,</li>';
									}
							...
							str+='</div>'+
								...
								for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.FOLLOW_UP.length;j++){
									...
									str+='<div class="row">'+
									'<div class="col-sm-12">'+
									'<p style="text-align: justify;"><b>CLINICAL NOTE : </b><small class="ipdClinicalNote">';
									str+=data.pat_details[i].HRSTR_JSON_DATA.FOLLOW_UP[j].progressNote.trim();
									str+='</small></p>'+
									'</div>'+
									'</div>'+
								...
								if("OtDtl" in (data.pat_details[i].HRSTR_JSON_DATA)){	
									if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl !=''){
										str+='<div class="row">'+
										'<div class="col-sm-12">'+
											'<p><b>OPERATION DETAIL : </b></p>'+
											'<ul class="ipdOtDtl" id="menu2">';
										for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.OtDtl.length;j++){
											var otDtlTxt = ''; 
											if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j]!= '')
											{
												otDtlTxt = data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[0].trim()+', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[2].trim()+', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[4].trim();
												if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[5].trim()!='')
													otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[5].trim();//finding
												if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[6].trim()!='')
													otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[6].trim();//complication
												if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[7].trim()!='')
													otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[7].trim();//implant
												if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[8].trim()!='')
													otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[8].trim();//pop
												if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[9].trim()!='')
													otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[9].trim();//pohp
											}
											str+='<li>'+ (j+1)+') ' +otDtlTxt+' <br></li>';
										}
										str+='</ul>'+
												'</div>'+
											'</div>'+
								...
								if(data.pat_details[i].HRSTR_JSON_DATA.courseNote!='')
								{
									str+='<br><p style="text-align: justify;"><b>Course/Stay Note : </b><small class="ipdClinicalNote">';
									str+=data.pat_details[i].HRSTR_JSON_DATA.courseNote.trim();
									str+='</small></p>';
								}
								
								if(data.pat_details[i].HRSTR_JSON_DATA.insTreatNote!='')
								{
									str+='<br><p style="text-align: justify;"><b>Instruction/Teatment : </b><small class="ipdInsNote">';
									str+=data.pat_details[i].HRSTR_JSON_DATA.insTreatNote.trim();
									str+='</small></p>';
								}
								...
								for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.FOLLOW_UP.length;j++){
									str+='<div class="row">'+
									'<div class="col-sm-12">'+
										'<p style="text-align: justify;"><b>FOLLOW UP : </b><small class="ipdFollowUp">';
								...
								str+='<br>'+
								'<h5 style="font-weight:bold; margin-top:0;margin-bottom:0;">Signature of Consultant / Resident:</h5>'+'<br>'+ 
								'<h5 class="ipdConsultantName" style="margin-top:0">'+data.pat_details[i].USER_NAME.trim()+'<br>'+data.pat_details[i].DATASAVE_TIME.trim()+'</h5>'
							...
							$('.ipdEmrModalNavMenuContent').append('<div class="tab-pane" id="'+menuId+(i+1)+'">'+str+'</div>');
							
						}
						$('.ipdEmrModalNavMenu').children('li').eq(0).addClass('active');
						/*var temp = $('.ipdEmrModalNavMenu li.active a').attr('href');
						alert(temp);*/
						$('.ipdEmrModalNavMenuContent').children('div').eq(0).addClass('active');
						
					}
					else{
						$('#ipdPrescriptionListMsg').text('No Past Precriptions Found !');	
					}
				},
				complete: $('#ipdPrescriptionListMsg').text('Error !!!')
			});
	//$('#ipdEmrModal').modal('show');
	ipdEmrDetailsOnClick();
}
...
function openCourseModal(){
	...
	$
			.ajax({
				url : '/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=9&CrNo='
						+ patCrNo
						+ '&episodeCode='
						+ episodeCode
						+ '&visitNo='
						...
						+ '&admNo=' + admNo + '',
				async : true,
				beforesend : $('.patCourseStayNoteList')
						.parent()
						.append(
								'<p id="ipdCourseNoteListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
				success : function(data) {
					 
					var str = '';
					
					var menuId = 'ipdCourseModalNavMenuItem';
					//alert(Object.keys(data).length);
					//alert(Object.keys(data.pat_details).length);
					console.log('course stay note data.pat_details length =='+Object.keys(data.pat_details).length);
					//console.log('data ashukk ---- '+ JSON.stringify(data.pat_details));
					if(Object.keys(data.pat_details).length > 0){
						
						$('.ipdCourseModalNavMenu').children('li').remove();
						$('.ipdCourseModalNavMenuContent').children('div').remove();
						
						$('#ipdCourseNoteListMsg').remove();
						for(var i=0;i<Object.keys(data.pat_details).length;i++)
						{  	
							// console.log("###########################################");
							 console.log("data.pat_details[i]---"+JSON.stringify(data.pat_details[i]));	
							// console.log("###########################################");
						    var tempdate =	data.pat_details[i].GDT_ENTRY_DATE.split(" ")[0].split("-");
						 	
						 	var maxDate1 =    tempdate[0]+"-"+tempdate[1] +"-"+tempdate[2].substring(2,4)+" {"+data.pat_details[i].GDT_ENTRY_DATE.split(" ")[1].trim()+"}";
							str = '';
								
							if(data.pat_details[i].GSTR_COURSE_REMARKS!='' && data.pat_details[i].GSTR_COURSE_REMARKS!=undefined){
								$('.ipdCourseModalNavMenu').append('<li><a href="#'+menuId+(i+1)+'" data-toggle="tab">'+maxDate1+'</a></li>');
							}
							//console.log('GSTR_VISIT_DATE -- '+data.pat_details[i].GSTR_VISIT_DATE);
							//console.log('GSTR_COURSE_REMARKS -- '+ JSON.stringify(data.pat_details[i].GSTR_COURSE_REMARKS));

							if(data.pat_details[i].GSTR_COURSE_REMARKS!='' && data.pat_details[i].GSTR_COURSE_REMARKS!=undefined){
									str+='<div class="row">'+
									'<div class="col-sm-12">'+
									'<p style="text-align: justify;"><b>Course/Stay Note : </b><small class="ipdCourseStayNote">';
									str+=data.pat_details[i].GSTR_COURSE_REMARKS;
									str+='</small></p>'+
									'</div>'+
									'</div>'+
									'<br>';
									$('.ipdCourseModalNavMenuContent').append('<div class="tab-pane" id="'+menuId+(i+1)+'">'+str+'</div>');
								}	
						}
						$('.ipdCourseModalNavMenu').children('li').eq(0).addClass('active');
						/*var temp = $('.ipdEmrModalNavMenu li.active a').attr('href');
						alert(temp);*/
						$('.ipdCourseModalNavMenuContent').children('div').eq(0).addClass('active');
						
					}
					else{
						$('#ipdCourseNoteListMsg').text('No Course/Stay Note Found !');	
					}
				},
				complete: $('#ipdCourseNoteListMsg').text('Error !!!')
			});
	$('#ipdShowPastCourseModal').modal('show');
}
function openInstructionModal(){
	...
	$
			.ajax({
				url : '/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=10&CrNo='
						+ patCrNo
						+ '&episodeCode='
						+ episodeCode
						+ '&visitNo='
						...
						+ '&admNo=' + admNo + '',
				async : true,
				beforesend : $('.patTreatmentNoteList')
						.parent()
						.append(
								'<p id="ipdTreatmentNoteListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
				success : function(data) {
					 
					var str = '';
					
					var menuId = 'ipdTreatmentModalNavMenuItem';
					//alert(Object.keys(data).length);
					//alert(Object.keys(data.pat_details).length);
					console.log('Treatment note data.pat_details length =='+Object.keys(data.pat_details).length);
					//console.log('data ashukk ---- '+ JSON.stringify(data.pat_details));
					if(Object.keys(data.pat_details).length > 0){
						
						$('.ipdTreatmentModalNavMenu').children('li').remove();
						$('.ipdTreatmentModalNavMenuContent').children('div').remove();
						
						$('#ipdTreatmentNoteListMsg').remove();
						for(var i=0;i<Object.keys(data.pat_details).length;i++)
						{  	
							// console.log("###########################################");
							 console.log("data.pat_details[i]---"+JSON.stringify(data.pat_details[i]));	
							...
							if(data.pat_details[i].GSTR_MED_ADVICE!='' && data.pat_details[i].GSTR_MED_ADVICE!=undefined)
								$('.ipdTreatmentModalNavMenu').append('<li><a href="#'+menuId+(i+1)+'" data-toggle="tab">'+maxDate1+'</a></li>');
							//console.log('GSTR_VISIT_DATE -- '+data.pat_details[i].GSTR_VISIT_DATE);
							//console.log('GSTR_MED_ADVICE -- '+ JSON.stringify(data.pat_details[i].GSTR_MED_ADVICE));

							if(data.pat_details[i].GSTR_MED_ADVICE!='' && data.pat_details[i].GSTR_MED_ADVICE!=undefined){
									str+='<div class="row">'+
									'<div class="col-sm-12">'+
									'<p style="text-align: justify;"><b>Instruction/Treatment : </b><small class="ipdCourseStayNote">';
									str+=data.pat_details[i].GSTR_MED_ADVICE;
									str+='</small></p>'+
									'</div>'+
									'</div>'+
									'<br>';
									$('.ipdTreatmentModalNavMenuContent').append('<div class="tab-pane" id="'+menuId+(i+1)+'">'+str+'</div>');
								}
						}
						$('.ipdTreatmentModalNavMenu').children('li').eq(0).addClass('active');
						$('.ipdTreatmentModalNavMenuContent').children('div').eq(0).addClass('active');
						
					}
					else{
						$('#ipdTreatmentNoteListMsg').text('No Instruction/Treatment Note Found !');	
					}
				},
				complete: $('#ipdTreatmentNoteListMsg').text('Error !!!')
			});
	$('#ipdShowPastTreatmentModal').modal('show');
}
function ipdEmrDetailsOnClick()
{
	console.log("IPD EMR DETAIL ON LOAD")
	var patCrNo = $('#patCrNoPrescriptionPanel').text();
	var episodeCode = $('#patEpisodeCodePrescriptionPanel').text();
	var visitNo = $('#patEpisodeVisitNoPrescriptionPanel').text();
	var seatId = $('#patSeatIdPrescriptionPanel').text();
	...
	$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=2&CrNo='+patCrNo+'&episodeCode='+episodeCode +'&visitNo='+visitNo+'&seatId=&Entrydate=&hosp_code='+hospCode+'&admNo='+admNo+'',
		async:true,
		beforesend : $('.IpdTreeStructurePrescriptionModalErrMsg').parent().append('<p id="IpdTreeStructurePrescriptionListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
			success:function(data){ 
				var strProfileInfo = '' , strChiefComplaint = '' , strDiagnosis = '' , strInvestigations = '' , strDrugsAndAdvices = '' , strClinicalNote = '';
				var strHistory = '' , strExaminations = '' , strAllergies = '' , strChronics = '' , strClinicalProc = '' ;
				var strtreatmentAdvice123 ='';
				var strOpeationTheatre ='',strReferral ='';
				var strTreatmentAdviceService ='';
				var str = '';
				var strconfidentialInfo ='';
				var strEpisodeCodeChk=episodeCode;
				//alert(Object.keys(data).length);
				//alert(Object.keys(data.pat_details).length);
				//$('.IpdTreeStructurePrescriptionModalNavMenu').children('li').remove();
				//$('.IpdTreeStructurePrescriptionModalNavMenuContent').children('div').remove();
				if(Object.keys(data.pat_details).length > 0){
					
					//$('.IpdTreeStructurePrescriptionModalNavMenu').children('li').remove();
					//$('.IpdTreeStructurePrescriptionModalNavMenuContent').children('div').remove();
					$('.IpdTreeStructurePrescriptionModalNavMenu').children('li').eq(0).remove();
					$('.IpdTreeStructurePrescriptionModalNavMenuContent').children('div').eq(0).remove();

					$('#IpdTreeStructurePrescriptionListMsg').remove();
					
					//$('.IpdTreeStructurePrescriptionModalNavMenu').remove();
					//$('.IpdTreeStructurePrescriptionModalNavMenuContent').remove();
					...
					str+='</td></tr>';
					str+='<tr>'+
					'<th>Department(Unit/Consultant)</th><td class="ipdPatDeptU">';
					str+=data.pat_details[0].HRSTR_JSON_DATA.patDept.trim();
					str+='</td><th>Reprinted On</th><td class="ipdPrintedOn">';
					str+=printDate+' / '+printTime;
					str+='</td></tr>';
					
					str+='<tr>'+
					'<th>Mobile No</th><td class="ipdPatDeptU">';
					if((data.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[14].trim())
					str+=(data.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[14].trim();
					
					if($('#strRailTailFlgId').val() == '0'){
					
					str+='</td><th>Occupation</th><td class="ipdPrintedOn">';
					str+=(data.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[17].trim();
					str+='</td></tr>';
					}else{
						var patOthersDetails1=$('#patOthersDetailsPrescriptionPanel').html()
						if(patOthersDetails1 !='{}'){
						str+='</td><th>Designation</th><td class="ipdPrintedOn">';
						str+=(JSON.parse(patOthersDetails1).designation);
						str+='</td></tr>'
							
							str+='<tr>'+
							'<th>Station</th><td class="ipdPatDeptU">';
					...
					strProfileInfo+='<div class="row"><div class="ipdProfileInfo col-md-6">'+
					'<table class="table table-condensed printIpdPrescPatDtlTbl table-responsive">'+
					'<tbody>'+
					'<tr>'+
					'<th>Patient Name : </th><td class="ipdPatName">';
					strProfileInfo+=data.pat_details[0].HRSTR_JSON_DATA.pat_Name.trim();
					strProfileInfo+='</td></tr><tr><th>CR No. : </th><td class="ipdPatCrNo">';
					strProfileInfo+=data.pat_details[0].HRSTR_JSON_DATA.CR_No.trim();
					strProfileInfo+='</td></tr><tr><th>Gender : </th><td class="ipdPatGen">';
					...
					strProfileInfo+='</td></tr><tr><th>Age : </th><td class="ipdPatAge">';
					strProfileInfo+=data.pat_details[0].HRSTR_JSON_DATA.patAge.trim();
					strProfileInfo+='</td></tr><tr><th>Category : </th><td class="ipdPatCat">';
					strProfileInfo+=data.pat_details[0].HRSTR_JSON_DATA.patCat.trim();
					strProfileInfo+='</td></tr>';
					/*strProfileInfo+='<tr><th>Refered By : </th><td>';
					strProfileInfo+=''; // Fill in the refered by Consultant name
					strProfileInfo+='</td></tr>';
					strProfileInfo+='<tr>'+*/
					strProfileInfo+='<th>Father/Spouse Name : </th><td class="ipdPatRelName">';
					strProfileInfo+=data.pat_details[0].HRSTR_JSON_DATA.patGaurdianName.trim();
					strProfileInfo+='</td></tr> '; 

					strProfileInfo+='</tr><tr>'+
				    '<th>Occupation : </th><td class="ipdPatRelName">';
					strProfileInfo+=(data.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[17];
					strProfileInfo+='</td></tr> ';
					
					
					strProfileInfo+='</tr><tr>'+
				    '<th>Mobile No : </th><td class="ipdPatRelName">';
					strProfileInfo+=(data.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[14];
					strProfileInfo+='</td></tr> ';
					if($('#strRailTailFlgId').val() == '0'){
					...
					strProfileInfo+='</td></tr> ';
						...
						if(patOthersDetails1 !='{}'){
						strProfileInfo+='</tr><tr>'+
					    '<th>Designation : </th><td class="ipdPatRelName">';
						strProfileInfo+=(JSON.parse(patOthersDetails1).designation);
						strProfileInfo+='</td></tr> ';
						
						strProfileInfo+='</tr><tr>'+
					    '<th>Station : </th><td class="ipdPatRelName">';
						strProfileInfo+=(JSON.parse(patOthersDetails1).custom_unit);
						strProfileInfo+='</td></tr> ';
						}else{
							strProfileInfo+='</tr><tr>'+
					...
					strProfileInfo+='</div>'; //End of row
					strProfileInfo+='<p>IPD Summary: </p>';
					...
					for(var i=0;i<Object.keys(data.pat_details).length;i++){  
						...
						var months = new Array(12);
					 	...
					 	if(i<1){ //For details of last 7 visits
					 	strProfileInfo+='<table class="table table-condensed ipdPatVisitSummaryDtlTbl table-responsive">'+
						'<tbody>'+
						'<tr><th width="5%">Dept: </th><td class="ipdDept" width="40%">';
					 	strProfileInfo+=data.pat_details[i].HRSTR_JSON_DATA.patDept.trim();
					 	strProfileInfo+='</td><th width="15%">Admission Date: </th><td class="ipdVisitDate" width="12%">';
					 	strProfileInfo+=data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim().split('#')[7];
					 	strProfileInfo+='</td><th width="10%">Unit Head: </th><td width="18%">';
					 	//strProfileInfo+=data.pat_details[i].HRSTR_JSON_DATA.patConsultantName.trim();
					 	if($('#strRailTailFlgId').val() == '1')
					 	...
					 	strProfileInfo+='</td></tr></table>';
					 	console.log("data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()"+data.pat_details[i].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim())
					 	/*strProfileInfo+='<p><u>'+maxDate+' ('+data.pat_details[i].HRSTR_JSON_DATA.patDept.trim()+')</u></p>';
					 	strProfileInfo+='<p>Consultant Name : <small>';
						strProfileInfo+=data.pat_details[i].HRSTR_JSON_DATA.patConsultantName.trim();
						strProfileInfo+='</small></p>';*/
						...
						if(data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint.length > 0){
							...
							strInvestigations+='<p><ol class="ipdInvestigation">';
							for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint.length;j++){
								var temp=data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split("^");
								var tempinvs=(data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^').length ==6 ?  data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^')[5].trim():data.pat_details[i].HRSTR_JSON_DATA.InvTestCodeToPrint[j].split('^')[7].trim()) ;
								strInvestigations+='<li>'+temp[5].trim()+'</li>';
						...
						if(data.pat_details[i].HRSTR_JSON_DATA.DrugCodeCat.length > 0){
							...
							strDrugsAndAdvices+='<p><ol class="ipdPrintPrescTreatmentLst">';
							for(var j=0;j<data.pat_details[i].HRSTR_JSON_DATA.DrugCodeCat.length;j++){
								var temp=data.pat_details[i].HRSTR_JSON_DATA.DrugCodeCat[j].split("&&");
								if(temp[8].trim() !='')
								strDrugsAndAdvices+='<li>'+temp[0].trim()+', '+temp[2].trim()+', '+temp[4].trim()+', '+temp[7].trim().split('#')[0]+' Days,'+' Qty : '+temp[7].trim().split('#')[1]+', '+temp[6].trim() + '('+temp[8].trim()+') </li>';
						...
						if("OtDtl" in data.pat_details[i].HRSTR_JSON_DATA){
						if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl != '' && data.pat_details[i].HRSTR_JSON_DATA.OtDtl.length){
							//console.log('Inside referral Advice');
							strOpeationTheatre+='<p><u>'+maxDate+' ('+data.pat_details[i].HRSTR_JSON_DATA.patDept.trim()+')</u></p>';
							//strOpeationTheatre+='<p><small>';
							strOpeationTheatre+='<p><ol class="ipdPrintPrescOtLst">';
							for(var j=0 ; j < data.pat_details[i].HRSTR_JSON_DATA.OtDtl.length ; j++){
								//if( (data.pat_details[i].HRSTR_JSON_DATA.strReferal[r].strReffralReason) =! '' )
								var otDtlTxt='';
								if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl!=''){
									otDtlTxt = '<li>'+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[0].trim()+', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[2].trim()+', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[4].trim();
									if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[5].trim()!='')
										otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[5].trim();//finding
									if(data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[6].trim()!='')
										otDtlTxt +=', '+data.pat_details[i].HRSTR_JSON_DATA.OtDtl[j].split('&&')[6].trim();//complication
									...
									strOpeationTheatre+=otDtlTxt+'</li>';	
								
								}
							}
							strOpeationTheatre+='</ol></p>';
						...
						if("strReferal" in (data.pat_details[i].HRSTR_JSON_DATA)){
							if(data.pat_details[i].HRSTR_JSON_DATA.strReffralDepttext != '' && data.pat_details[i].HRSTR_JSON_DATA.strReferal.length){
								//console.log('Inside referral Advice');
								strReferral+='<p><u>'+maxDate+' ('+data.pat_details[i].HRSTR_JSON_DATA.patDept.trim()+')</u></p>';
								strReferral+='<p><small>';
								...
								for(var r=0 ; r < data.pat_details[i].HRSTR_JSON_DATA.strReferal.length ; r++){
									...
									var strrefertext='';
										...
										strReferral+=strrefertext+' ,<br>';	
									
									}else
										strReferral+=data.pat_details[i].HRSTR_JSON_DATA.strReferal[r].strReffralDepttext.trim()+'('+data.pat_details[i].HRSTR_JSON_DATA.strReferal[r].strReffralReason.trim()+') ,';
					...
					var profileNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab2" data-toggle="tab" style="background-color:#6e93a6;"><i class="far fa-address-card"></i> Profile</a></li>';
					if(strProfileInfo!='')
						profileNav ='<li><a href="#IpdTreeStructurePrescriptionModalTab2" data-toggle="tab"><i class="far fa-address-card"></i> Profile</a></li>';
					var chiefComNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab4" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-md"></i> Chief Complaint</a></li>';
					if(strChiefComplaint!='')
						chiefComNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab4" data-toggle="tab"><i class="fas fa-user-md"></i> Chief Complaint</a></li>';
					var diagNav ='<li><a href="#IpdTreeStructurePrescriptionModalTab5" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-diagnoses"></i> Diagnosis</a></li>';
					if(strDiagnosis!='')
						diagNav ='<li><a href="#IpdTreeStructurePrescriptionModalTab5" data-toggle="tab"><i class="fas fa-diagnoses"></i> Diagnosis</a></li>';
					var investigationNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab6" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-microscope"></i> Investigations</a></li>';
					if(strInvestigations!='')
						investigationNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab6" data-toggle="tab"><i class="fas fa-microscope"></i> Investigations</a></li>';
					var drugNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab7" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-pills"></i> Drug/Advices</a></li>';
					if(strDrugsAndAdvices!='')
						drugNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab7" data-toggle="tab"><i class="fas fa-pills"></i> Drug/Advices</a></li>';
					var historyNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab9" data-toggle="tab" aria-expanded="false" style="background-color:#6e93a6;"><i class="fas fa-pen-square"></i> History</a></li>';
					if(strHistory!='')
						historyNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab9" data-toggle="tab" aria-expanded="false"><i class="fas fa-pen-square"></i> History</a></li>';
					var exmNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab10" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-stethoscope"></i> Examinations</a></li>';
					if(strExaminations!='')
						exmNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab10" data-toggle="tab"><i class="fas fa-stethoscope"></i> Examinations</a></li>';
					var allergyNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab11" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-allergies"></i> Allergies</a></li>';
					if(strAllergies!='')
						allergyNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab11" data-toggle="tab"><i class="fas fa-allergies"></i> Allergies</a></li>';
					var chronicNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab12" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-x-ray"></i> Chronic Diseases</a></li>';
					if(strChronics!='')
						chronicNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab12" data-toggle="tab"><i class="fas fa-x-ray"></i> Chronic Diseases</a></li>';
					var clinicalProNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab13" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-procedures"></i> Clinical Procedures</a></li>';
					if(strClinicalProc!='')
						clinicalProNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab13" data-toggle="tab"><i class="fas fa-procedures"></i> Clinical Procedures</a></li>';
					var treatAdvNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab15" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-prescription"></i> Treatment Advices</a></li>';
					if(strtreatmentAdvice123!='')
						treatAdvNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab15" data-toggle="tab"><i class="fas fa-prescription"></i> Treatment Advices</a></li>';
					var OTNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab16" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-ambulance"></i> Operation Detail</a></li>';
					if(strOpeationTheatre!='')
						OTNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab16" data-toggle="tab"><i class="fas fa-ambulance"></i> Operation Detail</a></li>';
					var ReferNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab21" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-ambulance"></i> Patient Referral</a></li>';
					if(strReferral!='')
						ReferNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab21" data-toggle="tab"><i class="fas fa-ambulance"></i> Patient Referral</a></li>';
					var confNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab17" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-ambulance"></i>Confidential info.</a></li>';
					if(strconfidentialInfo!='')
						confNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab17" data-toggle="tab"><i class="fas fa-ambulance"></i>Confidential info.</a></li>';
					var tempInfoNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab18" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-shield"></i> Template Info.</a></li>';
					if(fileName!='')
						tempInfoNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab18" data-toggle="tab"><i class="fas fa-user-shield"></i> Template Info.</a></li> ';
					var procAdmNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab19" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-shield"></i>Procedure Administration</a></li>';
					if(fileName1!='')
						procAdmNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab19" data-toggle="tab"><i class="fas fa-user-shield"></i>Procedure Administration</a></li>';
					var docUploadNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab20" data-toggle="tab" style="background-color:#6e93a6;"><i class="fas fa-user-shield"></i>Patient Document</a></li>';
					if(strPatientDocumentUpload!='')
						docUploadNav = '<li><a href="#IpdTreeStructurePrescriptionModalTab20" data-toggle="tab"><i class="fas fa-user-shield"></i>Patient Document</a></li>';
					
					
					$('.IpdTreeStructurePrescriptionModalNavMenu').children('li').eq(0).addClass('active');
					$('.IpdTreeStructurePrescriptionModalNavMenuContent').children('div').eq(0).addClass('active');
					
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab2').append(strProfileInfo);
					if(strChiefComplaint=='')
						strChiefComplaint+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab4').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab4').append(strChiefComplaint);
					if(strDiagnosis=='')
						strDiagnosis+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab5').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab5').append(strDiagnosis);
					if(strInvestigations=='')
						strInvestigations+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab6').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab6').append(strInvestigations);
					if(strDrugsAndAdvices=='')
						strDrugsAndAdvices+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab7').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab7').append(strDrugsAndAdvices);
					if(strClinicalNote=='')
						strClinicalNote+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab8').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab8').append(strClinicalNote);
					if(strHistory=='')
						strHistory+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab9').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab9').append(strHistory);
					if(strExaminations=='')
						strExaminations+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab10').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab10').append(strExaminations);
					if(strAllergies=='')
						strAllergies+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab11').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab11').append(strAllergies);
					if(strChronics=='')
						strChronics+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab12').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab12').append(strChronics);
					if(strClinicalProc=='')
						strClinicalProc+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab13').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab13').append(strClinicalProc);
					if(strtreatmentAdvice123=='')
						strtreatmentAdvice123+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab15').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab15').append(strtreatmentAdvice123);
					if(strOpeationTheatre=='')
						strOpeationTheatre+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab16').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab16').append(strOpeationTheatre);
					if(strReferral=='')
						strReferral+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab21').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab21').append(strReferral);
					if(strconfidentialInfo=='')
						strconfidentialInfo+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab17').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab17').append(strconfidentialInfo);
					if(fileName=='')
						fileName+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab18').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab18').append(fileName);
					if(fileName1=='')
						fileName1+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab19').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab19').append(fileName1);
					if(strPatientDocumentUpload=='')
						strPatientDocumentUpload+="<p><b>No Record Found<b></p>";
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab20').append(str);
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab20').append(strPatientDocumentUpload);
					
					
					var strmodalNavContent =profileNav+
					'<li><a href="#IpdTreeStructurePrescriptionModalTab3" data-toggle="tab"><i class="fa fa-heartbeat" aria-hidden="true"></i> Vitals / GE</a></li>'+
	                historyNav+
	                exmNav+
	                allergyNav+
	                chronicNav+
					...
					$('.IpdTreeStructurePrescriptionModalNavMenu').append(strmodalNavContent);
					
				}
				else{
					$('#IpdTreeStructurePrescriptionListMsg').text('No Details Found !');	
				}
			},
				complete: $('#IpdTreeStructurePrescriptionListMsg').text('Error !!!')
	});
	//$('#IpdTreeStructurePrescriptionModal').modal('show');
	getVitalsForIpdEmr();
}
function getVitalsForIpdEmr()
{
	console.log("IPD EMR VITAL DETAIL ON LOAD")
	var patCrNo = $('#patCrNoPrescriptionPanel').text();
	var episodeCode = $('#patEpisodeCodePrescriptionPanel').text();
	var visitNo = $('#patEpisodeVisitNoPrescriptionPanel').text();
	var seatId = $('#patSeatIdPrescriptionPanel').text();
	...
	$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getIpdPatVitalDataForDtlPrescription?Modval=3&CrNo='+patCrNo+'&episodeCode='+episodeCode +'&visitNo='+visitNo+'&seatId=&Entrydate=&hosp_code='+hospCode+'&admNo='+admNo+'',
		...
		//beforesend : $('.TreeStructurePrescriptionModalErrMsg').parent().append('<p id="TreeStructurePrescriptionVitalsListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
			success:function(data){ 
				...
				var strDisabilityArr=[] ,strSmokingArr=[] , strAnemicArr=[] , strPregnancyArr=[] ; 
				var strDisability='' ,strSmoking='' , strAnemic='' , strPregnancy='' ; 
				var menuId = 'IpdTreeStructureVitalsDetails';
				...
				$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=2&CrNo='+patCrNo+'&episodeCode='+episodeCode +'&visitNo='+visitNo+'&seatId=&Entrydate=&hosp_code='+hospCode+'&admNo='+admNo+'',
					...
					success:function(result){ 
								...
								if(Object.keys(result.pat_details).length > 0){
									...
									'<th>Name</th><td class="patName">';
									profileInfoString+=result.pat_details[0].HRSTR_JSON_DATA.pat_Name.trim();
									profileInfoString+='</td><th>CR No.</th><td class="ipdPatCrNo">';
									profileInfoString+=result.pat_details[0].HRSTR_JSON_DATA.CR_No.trim();
									profileInfoString+='</td></tr>';
									profileInfoString+='<tr>'+
									'<th>Department(Unit/Consultant)</th><td class="ipdPatDeptU">';
									profileInfoString+=result.pat_details[0].HRSTR_JSON_DATA.patDept.trim();
									profileInfoString+='</td><th>Reprinted On</th><td class="ipdPrintedOn">';
									profileInfoString+=printDate+' / '+printTime;
									profileInfoString+='</td></tr>';
									...
									profileInfoString+=(result.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[14].trim();
									if($('#strRailTailFlgId').val() == '0'){
									profileInfoString+='</td><th>Occupation</th><td class="ipdPrintedOn">';
									profileInfoString+=(result.pat_details[0].HRSTR_JSON_DATA.PatCompleteGeneralDtl.trim()).split('#')[17].trim();
									profileInfoString+='</td></tr>';
									}else{
										var patOthersDetails1=$('#patOthersDetailsPrescriptionPanel').html();
										if(patOthersDetails1 !='{}'){
										profileInfoString+='</td><th>Designation</th><td class="ipdPrintedOn">';
										profileInfoString+=(JSON.parse(patOthersDetails1).designation);
										profileInfoString+='</td></tr>';
				...
				if(Object.keys(data.pat_vital_details).length > 0){
					...
					strVitalsChart+='<div class="table-responsive"><table class="table table-hover table-striped table-condensed" id="IpdVitalDataTreeRow">';
				 	strVitalsChart+='<thead><tr style="color:#6d6db7"><th>Parameter/Date</th>';	
					...
					for(var i=0;i<Object.keys(data.pat_vital_details).length;i++){   
						...
						console.log("date array -->> "+i+" -->> "+strDateArr[i]);
					 	//console.log(i+" ** "+data.pat_vital_details[i].HOPLSTR_JSON_DATA.strfasting);
						console.log(i+" Vital JSON data "+ JSON.stringify(data.pat_vital_details[i].HOPLSTR_JSON_DATA));
					...
					strVitalsChart+='</tr></thead>'; 
  					...
  					strVitalsChart+='<tr><td style="font-weight:bold;">Remarks</td>';
  					strVitalsChart+=strSymptoms;
  					strVitalsChart+='<td class="text-right"><button id="parameterIpdVitalsTrendBtn" class="btn btn-sm btn-info testTrendsBtn" type="button">Trends</button></td>';
  					strVitalsChart+='</tr>';
  					...
  					strVitalsChart+='<h5 class="text-left ipdCumulativeTestChartHeadingForVitalsTreePresc"  style="display:none;"></h5>';
  					strVitalsChart+='<div class="col-sm-12" style="position: relative;"><canvas id="'+menuId+'_chart"></canvas></div>';
					...
					console.log('time ::>>>>>>>>>>>>>>'+date.getHours()+':'+date.getMinutes()+':'+date.getSeconds());
  					...
  					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab3').append(profileInfoString);
  					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab3').append("<br>");
					$('.IpdTreeStructurePrescriptionModalNavMenuContent #IpdTreeStructurePrescriptionModalTab3').append(strVitalsChart);
				...
				for(var k=0; k<strDateArr.length;k++){
					...
					$('#parameterIpdVitalsTrendBtn').click(function(){
						$('.ipdCumulativeTestChartHeadingForVitalsTreePresc').show();
						var ctx = document.getElementById(menuId+'_chart').getContext('2d');
						window.myLine = new Chart(ctx, config);
					});
...
function opdEmrDtlView()
{
	$('#opdEmrBtn').addClass('active').css('background-color', 'green');
	$('#ipdEmrBtn').removeClass('active').css('background-color', '');
	$('.emrTitle').text('OPD EMR');
	$('#emrDiv1').show();
	$('#emrDiv2').show();
	$('#emrDiv3').hide();
	$('#emrDiv4').hide();
...
function ipdEmrDtlView()
{
	$('#ipdEmrBtn').addClass('active').css('background-color', 'green');
	$('#opdEmrBtn').removeClass('active').css('background-color', '');
	$('.emrTitle').text('IPD EMR');
	$('#emrDiv3').show();
	$('#emrDiv4').show();
	$('#emrDiv1').hide();
	$('#emrDiv2').hide();
}
function opdPastRxDtlView()
{
	$('#opdPastRxBtn').addClass('active').css('background-color', 'green');
	$('#ipdPastRxBtn').removeClass('active').css('background-color', '');
	$('.pastRxTitle').text('OPD Past Rx');
	$('#pastRx1').show();
	$('#pastRx2').show();
	$('#pastRx3').hide();
	$('#pastRx4').hide();
...
function ipdPastRxDtlView()
{
	$('#ipdPastRxBtn').addClass('active').css('background-color', 'green');
	$('#opdPastRxBtn').removeClass('active').css('background-color', '');
	$('.pastRxTitle').text('IPD Past Rx');
	$('#pastRx3').show();
	$('#pastRx4').show();
	$('#pastRx1').hide();
	$('#pastRx2').hide();
...
function getDrugRoute(){
	var drugName = $('input[name=drugName]').val();
	//console.log("drugItemTypeId=="+drugItemTypeId);
	var drugId='';
	if(drugName!='')
	{
		$.each(JSON.parse($("#drugJsonObjDiv").text().trim()), function(i, v) {
		...
		$('#drugRoute').html('');
		var drugItemTypeId = drugId.split('#')[1];
		$.ajax({
	        type: "GET",
	        url: '/HISDRDESK_MC/services/restful/IpdDrDesk/getDrugRoute?drugItemTypeId='+drugItemTypeId,
	        dataType:'json',
	        async: true,
	        success: function(items) {
	            console.log(items);
	            if(items.status==1 && items.drugRoute_details!=null)
	         	   {
	             	   if(items.drugRoute_details.length>0)
	             	   {
	             		  $('#drugRoute').append($('<option>', {value: '', text: 'Select'}));
	             		  for(var j=0;j<items.drugRoute_details.length;j++)
	                     	{
	                         	$('#drugRoute').append($('<option>', {value: items.drugRoute_details[j].HGNUM_DRUG_ROUTE_ID, text: items.drugRoute_details[j].HGSTR_DRUG_ROUTE_NAME}));
	                         	}
	             	   }
	             	  else
	        		   {
...
function openNurseNoteModal(){
	...
	$
			.ajax({
				url : '/HISDRDESK_MC/services/restful/patdata/getIpdPatDataForPastPrescription?Modval=11&CrNo='
						+ patCrNo
						+ '&episodeCode='
						+ episodeCode
						+ '&visitNo='
						...
						+ '&admNo=' + admNo + '',
				async : true,
				beforesend : $('.patProgressNoteList')
						.parent()
						.append(
								'<p id="ipdNurseProgressNoteListMsg"><i class="fa fa-spinner fa-spin"></i> Loading</p>'),
				success : function(data) {
					 
					var str = '';
					
					var menuId = 'ipdNurseProgressNoteModalNavMenuItem';
					//alert(Object.keys(data).length);
					//alert(Object.keys(data.pat_details).length);
					console.log('course stay note data.pat_details length =='+Object.keys(data.pat_details).length);
					//console.log('data ashukk ---- '+ JSON.stringify(data.pat_details));
					if(Object.keys(data.pat_details).length > 0){
						...
						$('.ipdNurseRoundNoteModalNavMenu').children('li').remove();
						$('.ipdNurseRoundNoteModalNavMenuContent').children('div').remove();
						
						$('#ipdNurseProgressNoteListMsg').remove();
						for(var i=0;i<Object.keys(data.pat_details).length;i++)
						{  	
							// console.log("###########################################");
							 console.log("data.pat_details[i]---"+JSON.stringify(data.pat_details[i]));	
							...
							if(data.pat_details[i].HIPSTR_PROGRESS_NOTES!='' && data.pat_details[i].HIPSTR_PROGRESS_NOTES!=undefined){
								$('.ipdNurseRoundNoteModalNavMenu').append('<li><a href="#'+menuId+(i+1)+'" data-toggle="tab">'+maxDate1+'</a></li>');
							}
							//console.log('GSTR_VISIT_DATE -- '+data.pat_details[i].GSTR_VISIT_DATE);
							//console.log('GSTR_COURSE_REMARKS -- '+ JSON.stringify(data.pat_details[i].GSTR_COURSE_REMARKS));
							...
							if(data.pat_details[i].HIPSTR_PROGRESS_NOTES!='' && data.pat_details[i].HIPSTR_PROGRESS_NOTES!=undefined){
									str+='<div class="row">'+
									'<div class="col-sm-12">'+
									'<p style="text-align: justify;"><b>Progress Note : </b><small class="ipdNurseRoundNote">';
									str+=data.pat_details[i].HIPSTR_PROGRESS_NOTES;
									str+='</small></p>'+
									'</div>'+
									'</div>'+
									'<br>';
									$('.ipdNurseRoundNoteModalNavMenuContent').append('<div class="tab-pane" id="'+menuId+(i+1)+'">'+str+'</div>');
								}	
						}
						$('.ipdNurseRoundNoteModalNavMenu').children('li').eq(0).addClass('active');
						/*var temp = $('.ipdEmrModalNavMenu li.active a').attr('href');
						alert(temp);*/
						$('.ipdNurseRoundNoteModalNavMenuContent').children('div').eq(0).addClass('active');
						
					}
					else{
						$('#ipdNurseProgressNoteListMsg').text('No Progress Note Found !');	
					}
				},
				complete: $('#ipdNurseProgressNoteListMsg').text('Error !!!')
			});
	$('#ipdShowNurseRoundNoteModal').modal('show');
}