...
<body id="printPrescFrameBody" onload="">
	      ...
	      <div id="printPrescPage" class="container printPrescPage"> 
           <div class="row Logoimg" id="LogoimgId">
				<div style="float:left">
					<img alt="" src="/HIS/hisglobal/images/e-clinic/sikkimlogo.jpg" style="height: 56px"></div><br>
			      <div style="text-align:center">
			       <h4><b>${IPDDoctorDeskFB.strHospitalName}</b></h4>
			       <h5>${IPDDoctorDeskFB.strHospitalAddres}</h5> 
			     </div>
	</div>  
			<div class="row divPrintPrescPatDtlTbl" id="divPrintPrescPatDtlTblId"  style="border-top: 1px solid grey; border-bottom: 1px solid grey;">
				<table class="table table-condensed printPrescPatDtlTbl">
					...
					<tr>
						...
						<tr>
							<th>Admission Date</th><td class="patAdmissionDate"></td><th>Admission No.</th><td class="admissionNo"></td>
						</tr>
						<tr>
							<th>Age/Gender</th><td class="patAgeGen"></td><th>Patient Category</th><td class="patCat"></td>
						</tr>
						<tr>
							<th>Father/Spouse Name</th><td class="patRelName"></td><th>Department/Unit</th><td class="patDeptU"></td>
						</tr>
						
						<tr>
							<th>Ward Name</th><td class="patWard"></td><th>Room/Bed</th><td class="room_bed"></td>
						</tr>
						...
						<tr>
							<th>Consultant</th><td class="consultantName"></td>
						</tr>
			...
			<div class="row" id="OtDivId" >
				<div class="col-sm-12" style="" align="center">
					<p style="text-align: left;font-size: small;margin-bottom: 0px;">POST OPERATION DETAIL :</p>
					<ol class="operationLst"> 
					</ol>
				</div>
			</div>
			&nbsp;
			...
			&nbsp;
			<br> 
			<div class="row" id="hoplCrsDivId" >
				<div class="col-sm-12" style="">
					<p>HOSPITAL COURSE :  <small class="hoplCrsVisitDate"></small>  <small class="hoplCrsVisitTime"></small><br>
					<br>  <small class="hoplCrsStayNote"></small>
					<br>  <small class="hoplCrsInsTreatNote"></small> 
					</p>
				</div>
			</div>
			<!--&nbsp;
	...
	$(document).ready(function(){
	  ...
	  if(myJSON==undefined)
		...
		$('.printPrescPage .visitDateonsultannt').text(new Date().getDate()+'/'+(new Date().getMonth()+1)+'/'+new Date().getFullYear()+' '+ new Date().getHours()+':'+('0'+new Date().getMinutes()).slice(-2) );
		...
		$('.printPrescPage .patDeptU').text(data.patDept+'/'+(data.PatCompleteGeneralDtl).split('#')[9]);
		var datef=(data.PatCompleteGeneralDtl).split('#')[12];
		$('.printPrescPage .patAdmissionDate').text(datef.split('/')[2]+'/'+datef.split('/')[1]+'/'+datef.split('/')[0] );
		...
		$('.printPrescPage .admissionNo').text(data.admissionNo);
		$('.printPrescPage .patWard').text((data.PatCompleteGeneralDtl).split('#')[20]);
		$('.printPrescPage .room_bed').text((data.PatCompleteGeneralDtl).split('#')[2]+'/'+(data.PatCompleteGeneralDtl).split('#')[21]);
		...
		/* Not in Use *//*  $('.printPrescPage .printPrescTreatmentTbl tbody').append('<tr><td>'+data.DrugCodeCat[i].split('&&')[0].trim()+'</td><td>'+data.DrugCodeCat[i].split('&&')[2].trim()+'</td><td>'+data.DrugCodeCat[i].split('&&')[4].trim()+'</td><td>'+data.DrugCodeCat[i].split('&&')[7].trim()+'</td><td>'+data.DrugCodeCat[i].split('&&')[8].trim()+'</td><tr>');*/
			 if(data.DrugCodeCat[i].split('&&')[8].trim()!='' && data.DrugCodeCat[i].split('&&')[8].trim() != null)
			{
				...
				if(data.DrugCodeCat[i].split('&&')[7].trim()!='' && data.DrugCodeCat[i].split('&&')[7].trim() !=null)
					$('.printPrescPage .printPrescTreatmentLst').append('<li><p>'+data.DrugCodeCat[i].split('&&')[0].trim()+', <small>'+data.DrugCodeCat[i].split('&&')[2].trim()+', '+data.DrugCodeCat[i].split('&&')[4].trim()+', '+data.DrugCodeCat[i].split('&&')[7].trim().split('#')[0]+' Days, '+'Qty : '+data.DrugCodeCat[i].split('&&')[7].trim().split('#')[1]+', '+ tmpDate +' ('+data.DrugCodeCat[i].split('&&')[8].trim()+') </small></p></li>');
				else
			...
			else
			{
				...
				if(data.DrugCodeCat[i].split('&&')[7].trim()!='' && data.DrugCodeCat[i].split('&&')[7].trim() !=null)
					$('.printPrescPage .printPrescTreatmentLst').append('<li><p>'+data.DrugCodeCat[i].split('&&')[0].trim()+', <small>'+data.DrugCodeCat[i].split('&&')[2].trim()+', '+data.DrugCodeCat[i].split('&&')[4].trim()+', '+data.DrugCodeCat[i].split('&&')[7].trim().split('#')[0]+' Days, '+'Qty : '+data.DrugCodeCat[i].split('&&')[7].trim().split('#')[1]+', '+ tmpDate +' </small></p></li>');
				else
	...
	if(data.strReferal.length == 0){
	 ...
	 for(var j = 0 ; j< data.strReferal.length ; j++){
		 ...
		 if(data.strReferal[j].strReffralReason !='')
			 strrefertext = strrefertext + ' (Note: ' +data.strReferal[j].strReffralReason +') ' ;
		 if(data.strReferal[j].strExternalInstituteDays !='')
			 strrefertext = strrefertext + '(Days: ' +data.strReferal[j].strExternalInstituteDays +') ' ;
		 if(data.strReferal[j].strExternalInstituteReasonId !='')
			 strrefertext = strrefertext + '(Reason: ' +data.strReferal[j].strExternalInstituteReasonName +') ' ;
		...
		if("OtDtl" in data ){
			...
			if(data.OtDtl.length == 0){
				$( "#OtDivId" ).parent().find('br').remove();
				$( "#OtDivId" ).remove();
			}
			console.log("data.OtDtl.length=="+data.OtDtl.length);
			for(var j = 0 ; j< data.OtDtl.length ; j++)
			{
				var otDtlTxt = ''; 
				if(data.OtDtl[j]!= '')
				{
					//var temp =data.OtDtl[i].split('&&')[0].trim();
					//var tmpDate=temp.split('-')[2]+'-'+temp.split('-')[1]+'-'+temp.split('-')[0];
					//otDtlTxt =data.OtDtl[j] ;
					otDtlTxt = data.OtDtl[j].split('&&')[0].trim()+', '+data.OtDtl[j].split('&&')[2].trim()+', '+data.OtDtl[j].split('&&')[4].trim();
					if(data.OtDtl[j].split('&&')[5].trim()!='')
						otDtlTxt +=', '+data.OtDtl[j].split('&&')[5].trim();//finding
					if(data.OtDtl[j].split('&&')[6].trim()!='')
						otDtlTxt +=', '+data.OtDtl[j].split('&&')[6].trim();//complication
					if(data.OtDtl[j].split('&&')[7].trim()!='')
						otDtlTxt +=', '+data.OtDtl[j].split('&&')[7].trim();//implant
					if(data.OtDtl[j].split('&&')[8].trim()!='')
						otDtlTxt +=', '+data.OtDtl[j].split('&&')[8].trim();//pop
					if(data.OtDtl[j].split('&&')[9].trim()!='')
						otDtlTxt +=', '+data.OtDtl[j].split('&&')[9].trim();//pohp
					$('.printPrescPage .operationLst').append('<li><p>'+otDtlTxt+ '</p></li>');
				}
		...
		if("hoplCrsVisitDate" in data){
			if(data.hoplCrsVisitDate != '')
				{
				var temp =data.hoplCrsVisitDate.split(":")[0];
				var tmpDate=temp.split('-')[2]+'-'+temp.split('-')[1]+'-'+temp.split('-')[0];
				//$('.printPrescPage .investigationNoteID').append(data.strInvestgationNote);
				$('.printPrescPage .hoplCrsVisitDate').append("<span style='font-weight: bold;'>Date of Visit: </span>"+tmpDate);
				}
			else{
				$( ".printPrescPage .hoplCrsVisitDate" ).remove();
				}
		...
		if("hoplCrsVisitDate" in data){
			if(data.hoplCrsVisitDate != '')
				{
				//$('.printPrescPage .investigationNoteID').append(data.strInvestgationNote);
				$('.printPrescPage .hoplCrsVisitTime').append("<span style='font-weight: bold;'>Time of Visit: </span>"+data.hoplCrsVisitDate.split(":")[1]+" Hr:"+data.hoplCrsVisitDate.split(":")[2]+" Min");
				}
			else{
				$( ".printPrescPage .hoplCrsVisitTime" ).remove();
				}
			
			} 
		if("courseNote" in data){
			if(data.courseNote != '')
				{
				//$('.printPrescPage .investigationNoteID').append(data.strInvestgationNote);
				$('.printPrescPage .hoplCrsStayNote').append("<br> Course/Stay Notes: "+data.courseNote);
				}
			else{
				$( ".printPrescPage .hoplCrsStayNote" ).remove();
				}
			
			} 
		if("insTreatNote" in data){
			if(data.insTreatNote != '')
				{
				//$('.printPrescPage .investigationNoteID').append(data.strInvestgationNote);
				$('.printPrescPage .hoplCrsInsTreatNote').append("<br> Instruction/Treatment: "+data.insTreatNote);
				}
			else{
				$( ".printPrescPage .hoplCrsInsTreatNote" ).remove();
				}
