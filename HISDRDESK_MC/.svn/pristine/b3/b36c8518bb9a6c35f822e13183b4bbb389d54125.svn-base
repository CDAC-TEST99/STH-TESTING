	$(document).ready(function(){
		...
		/*The default value for trigger is 'hover focus', thus the tooltip stay visible after a button is clicked, 
		until another button is clicked, because the button is focused.So, define trigger as 'hover' only.*/
...
// ~~~~~ Util Functions for Prescription.jsp	    
		...
		$(document).ready(function(){ 
			...
			//console.log('patientSumBtn called');
			...
			$('#patientSumBtn').click(function(){
				...
				$('input[name=invDrugName]').each(function()
						{
							//console.log('name=drugsAdvices::::>>>'+$(this).val());
							if($(this).val().trim()!='') 
							{
								//DrugCodeCat[j]=$(this).val();
								console.log("InvDrugJson::::::: "+$(this).val());
								drug = drug+'<li>'+$(this).val()+'</li>';
								//DrugJsonArray[j]=JSON.parse($(this).parent().find('i').text());
								//j++;
							}
				...
				$('input[name=chronicDiseaseChk]').each(function()
						{
							...
							if($(this).val().trim().split('^')[2]!=''){
								chronicDisease = chronicDisease+'<li>'+($(this).val()).split("^")[0]+' ['+($(this).val()).split("^")[2]+' years]'+'</li>';
							}else{
								chronicDisease = chronicDisease+'<li>'+($(this).val()).split("^")[0]+'</li>';
							}
				...
				var chiefComplaint='';
				$('input[name=visitReason]').each(function()
						{
							console.log('visitReason::>>> '+$(this).val());
							chiefComplaint = chiefComplaint+'<li>'+($(this).val()).split("^")[1]+'</li>';
						});
				
				var hopi = $('#hopiId').val();
				var diagnosis='';
				$('input[name=diagnosisAdded]').each(function()
						{
							console.log('diagnosisAdded::>>> '+$(this).val());
							diagnosis = diagnosis+'<li>'+($(this).val()).split("#")[2].split("^")[1]+'</li>';
						});
				
				var clinicalProc='';
				$('input[name=clinicalProc]').each(function()
						{
							console.log('clinicalProc::>>> '+$(this).val());
							clinicalProc = clinicalProc+'<li>'+($(this).val()).split("#")[0]+'</li>';
						});
				var allergiesDtlChk='';
				$('input[name=allergiesDtlChk]').each(function()
						{
							var allergy =JSON.parse($(this).parent().find('i').text());
							//console.log('allergy'+allergy.strAllergyName);
							if(allergy.strAllergyName!='')
								allergiesDtlChk = allergiesDtlChk+'<li>'+allergy.strAllergyName+'</li>';
						});
				
				$('#patMedicineDatePrescriptionPanel').html('<ul>'+drug+'</ul>'); 
				$('#patRecentLabTestPrescriptionPanel').html('<ul>'+test+'</ul>');
				$('#patChronicDiseasePrescriptionPanel').html('<ul>'+chronicDisease+'</ul>');
				$('#patChiefCompDatePrescriptionPanel').html('<ul>'+chiefComplaint+'</ul>');
				$('#patHoplDatePrescriptionPanel').html(hopi);
				$('#patDiagnosisDatePrescriptionPanel').html('<ul>'+diagnosis+'</ul>');
				$('#patClinicalProDatePrescriptionPanel').html('<ul>'+clinicalProc+'</ul>');
				$('#patAllergiesPrescriptionPanel').html('<ul>'+allergiesDtlChk+'</ul>');
				...
				if(chronicDisease=='')
					$('.chrDiseasePS').hide();
				if($('#otherInfoIdValue').text()=='')
					$('.vitalPS').hide();
				if(chiefComplaint=='' && hopi=='' && diagnosis=='')
					$('.currAdmPS').hide();
				if(chiefComplaint=='')
				{
					$('.ccheader').hide();
       ...
       $(document).ready(function(){ 
        ...
        function getImage(episodeCode,count,visitNo,visitDate){
               ...
               { 
        		 ...
        		 $.ajax({
               	 url:'/HISDRDESK_MC/services/restful/UserService/retriveImageData?DeptValue=hello^'+crNo+'^'+visitNo+'^'+episodeCode+'^'+hospCode+'',   
               	 type:'GET', 
               	 async: true,
   ...
   /*                  	for(var j=0;j<count;j++)
                   	...
                   	/*  $.ajax({url:'https://220.156.189.222/HBIMS/services/restful/UserService/retriveImageData?DeptValue=one^331011800025470^two^10511001^33101',type:'GET', crossOrigin: true,success:function(result)
                       	{
                    		 ...
                    		 $.ajax({
                           	 url:'/HISDRDESK_MC/services/restful/UserService/getPatDataViewPrescription?CrNo='+crNo+'&seatid='+seatId+'',    //getPatData
                           	 type:'GET',
                           	 async: true,
       ...
       $(document).ready(function(){ 
      	 ...
      	 $.ajax({url:'/HISDRDESK_MC/services/restful/invService/investigationDataList?crNo='+crNo+'&hosCode='+hospCode+'',  //http://10.226.21.120:8088/eHMS_Web_Services/service/invService/investigationDataList?crNo=331011800003255&hosCode=33101
      			async:true,
      			...
      			$.ajax({url:'/HISDRDESK_MC/services/restful/invService/reportList?crNo='+crNo+'&hosCode='+hospCode+'',
      				async:true,
      	...
      	{	
      		...
      		$.ajax({url:'/HISDRDESK_MC/services/restful/invService/paraRawData?crNo='+crNo+'&hosCode='+hospCode+'&paraCode='+paramCode+'',
      		async:true,
    	...
    	{ 
    		...
    		$.ajax({url:'/HISDRDESK_MC/services/restful/invService/reportData?crNo='+crno+'&reqDNo='+reqdno+'&hosCode='+hospCode+'',
    			async:true,
    			success:function(result){
    				// Chrome 1 - 68
    	...
    	{ 
      		var crNo = $('#patCrNoPrescriptionPanel').text(); //  331011800025054   331011800015989   331011800025470 
         	var UMID = $('#patUmidPrescriptionPanel').text(); // '33101' 
    		...
    		console.log('crno::::: '+crNo+' ::::::::::UMID::::: >>'+UMID+' episodeCode::::: '+episodeCode+' ::::::::::hospCode:::::>> '+hospCode);
    		var type = 'data:image\/jpg;base64,';
    		//var profile = '';
    		//profile_pic = type.concat(profile);
    		//console.log('profile_pic ---  '+profile_pic);	
    		
    		//$("#viewPatientImage").attr("src", JSON.parse(patOthersDetails).profile_pic);
    		//'/HISServices/service/AppOpdService/getImageByCrNoAndUmid?crNo='+crno+'&episodeCode='+episodeCode+'&hospCode='+hospCode+'&seatid='+seatId+'&umid='+UMID+'',
    		$.ajax({url:'/HISServices/service/AppOpdService/getImageByCrNoAndUmid?crNo='+crNo+'&episodeCode='+episodeCode+'&hospCode='+hospCode+'&seatid='+seatId+'&umid='+UMID+'',
    			async:true,
    			//beforesend: $("#viewPatientImage").append('<li role="presentation" id="prescViewVisitLstMsgId"><a><i class="fa fa-spinner fa-sping"></i> Loading</a></li>'),
    			success:function(result){
    			...
    			if(result.status=="1")
             	{
	    			if(!result.profilePicBase64.includes("data:image\/jpg;base64,"))
	    				profile_pic = type.concat(result.profilePicBase64);
	    			else
	    				profile_pic = result.profilePicBase64;
	    				
	    			//console.log('profile_pic final  '+profile_pic);	
	    			$("#viewPatientImage").attr("src", profile_pic);
	        		$("#viewPatientImage").attr("alt", "No Image Found");
             	}else{
             		$("#viewPatientImage").attr("src", "");
             		$("#viewPatientImage").attr("alt", "No Image Found");
             	}
    	...
    	$.ajax({ url:'/HISServices/service/eConsultReq/getDocStatus2?requestId='+reqId+'&hospCode='+hospCode+ ' ',   
          	...
          	// async: true,
          	 ...
          	 success:function(result2)
           	{
          		 console.log('result2result2result2result2'+JSON.parse(result2));
   	...
   	$(document).ready(function(){
		...
		console.log('reqFlg = '+reqFlg);
      	
		if(reqFlg == '1'){
      	$.ajax({ url:'/HISServices/service/eConsultReq/getHrgtEconsultationReq?pReqId='+reqId+'&hospCode='+hospCode+ ' ',   
            	 type:'GET', 
            	// async: true,
            	 beforesend: $('.prescViewVisitLst').append('<li role="presentation" id="prescViewVisitLstMsgId"><a><i class="fa fa-spinner fa-sping"></i> Loading</a></li>'),
            	 success:function(result2)
             	{
            		 console.log('Teleconsultation Remarks ---- '+JSON.stringify(result2));
            		 
            		 	var strTeleconsultationRemarks = 'No Remarks Found.';
						if(result2.status=="1"){
							strTeleconsultationRemarks = '';
							strTeleconsultationRemarks+='<p>';
						
							if(result2.data[0].PROBLEM_DES != '' || result2.data[0].PROBLEM_DES != "")
								strTeleconsultationRemarks+='<li><b>Problem Description : </b>'+result2.data[0].PROBLEM_DES+'</li>';
							if(result2.data[0].COMPLAINTS != '' || result2.data[0].COMPLAINTS != "")
								strTeleconsultationRemarks+='<li><b>Complaints : </b>'+result2.data[0].COMPLAINTS+'</li>';
							if(result2.data[0].PAST_DIAGNOSIS != '' || result2.data[0].PAST_DIAGNOSIS != "")
								strTeleconsultationRemarks+='<li><b>Past Diagnosis : </b>'+result2.data[0].PAST_DIAGNOSIS+'</li>';
							if(result2.data[0].MEDICATIONS != '' || result2.data[0].MEDICATIONS != "")
								strTeleconsultationRemarks+='<li><b>Medications : </b>'+result2.data[0].MEDICATIONS+'</li>';
							if(result2.data[0].ALLERGIES != ''  || result2.data[0].ALLERGIES != "")
								strTeleconsultationRemarks+='<li><b>Allergies : </b>'+result2.data[0].ALLERGIES+'</li>';
							if(result2.data[0].HEIGHT != ''  || result2.data[0].HEIGHT != "")
								strTeleconsultationRemarks+='<li><b>HEIGHT : </b>'+result2.data[0].HEIGHT+'</li>';
							if(result2.data[0].WEIGHT != ''  || result2.data[0].WEIGHT != "")
								strTeleconsultationRemarks+='<li><b>WEIGHT : </b>'+result2.data[0].WEIGHT+'</li>';
							strTeleconsultationRemarks+='</p>';
							//console.log("strInvDrug==>"+strInvDrug);
							
						}
						
						$('.opdTeleconsultationModalNavMenuContent').append("<div>"+strTeleconsultationRemarks+"</div>");
