...
public static Document createPdf(String jsonData , String vitalJsonData , String hospitalHeader , String UserName, String DataSaveTime) throws Exception {
		 ...
		 * System.out.println("osType : "+osType); if(osType.startsWith("Win")){ path =
		 * Paths.get("c:/AIIMSBBW/AHIMSG5/reportimage/AIIMS_Logo.jpg");
		 * System.out.println("LOGO_IMAGE_PATH_WIN : "+path.toString()); img =
		 * Image.getInstance(path.toAbsolutePath().toString());
		 * System.out.println("LOGO_IMAGE_ABSOLUTE_WIN : "+path.toAbsolutePath().
		 * toString());
		 * 
		 * }else{ path = Paths.get("/opt/AIIMSBBW/AHIMSG5/reportimage/AIIMS_Logo.jpg");
		 * System.out.println("LOGO_IMAGE_PATH_LIN : "+path.toString()); img =
		 * Image.getInstance(path.toAbsolutePath().toString());
		 * System.out.println("LOGO_IMAGE_ABSOLUTE_PATH_LIN : "+path.toAbsolutePath().
		 * toString()); }
		 */
	...
	//	System.out.println("drugstl.size() - "+drugstl.size());
		...
		if(!(clinicalnotes.isEmpty())  ) {
			document.add( new Phrase("\n") );
			if(!clinicalnotes.toString().contains("Chief Complaints:"))
...
private static void patientRefferTo(List list, String json) throws JSONException {
		...
		for(int i=0;i<ja.length();i++) {
			Chunk ch=new Chunk();
			ch.append(((JSONObject) (ja.get(i))).getString("strShowData"));
...
private static void Investigation(Chunk chunk, String json) throws JSONException {
	...
	private static void cgeifComplaint(Chunk chunk, String json) throws JSONException {
		...
		for(int i=0;i<ja.length();i++) {
			...
			if(DiagnosisName != null && !DiagnosisName.equals("") && DiagnosisTypeNamee != null && !DiagnosisTypeNamee.equals("") )
				chunk.append("" + DiagnosisName+"("+ DiagnosisTypeNamee+")")	;
			
			if(!(((JSONObject) (ja.get(i))).getString("DiagnosisSideCode")).equalsIgnoreCase("0")) {
				if(((JSONObject) (ja.get(i))).getString("DiagnosisSideName") != null && !((JSONObject) (ja.get(i))).getString("DiagnosisSideName").equals("") )
...
private static void followup(Chunk chunk, String json) throws JSONException {
	...
	for(int i=0;i<ja.length();i++) {
		...
		for(int j=0;j<ja1.length();j++) {
			if(((JSONObject) (ja1.get(i))).getString("plannedVisitSos") != null && !((JSONObject) (ja1.get(i))).getString("plannedVisitSos").equalsIgnoreCase("")) {
				chunk.append(((JSONObject) (ja1.get(i))).getString("plannedVisitSos"));
			}
			if(((JSONObject) (ja1.get(i))).getString("plannedVisitDays") != null && !((JSONObject) (ja1.get(i))).getString("plannedVisitDays").equalsIgnoreCase("")) {
				chunk.append(", After "+((JSONObject) (ja1.get(i))).getString("plannedVisitDays")+" Days");
			}
