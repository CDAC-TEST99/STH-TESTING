...
<head>
<meta>
<title>Path Care Lab API Sent</title>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    selectAllCheckbox.addEventListener("change", function () {
        const dataCheckboxes = dataContainer.querySelectorAll("input[type='checkbox'][name='strCheckALL']");
...
function onGoButton(event) {
	...
	function getPatDtls(){
		    ...
		    var strPresDate = $("#strPresDate").val();
		    var strDeskCombo = $("#strDeskCombo").val();
		    var strAPIStatus = $("#apiCombo").val();
		    
		$('input[name=strCheckALL]').each(function (i) {
	        	   	$("input[name=strCheckALL]")[i].disabled = true;
	        	   	$("input[name=strHiddenValues]")[i].disabled = true;
		});

			var url = "MCPathLabSampleHandoverDetailTransMcNewCNT.cnt?hmode=WebServicesSentAjax";
		    url += "&strPresDate=" + strPresDate + "&strDeskCombo=" + strDeskCombo + "&strAPIStatus=" + strAPIStatus;
		    ajaxFunction(url, "1");
	...
	function getAjaxResponse(res, mode) {
		...
		if (mode == "1") {
			...
			var objVal = document.getElementById("myTable");
	...
	function saveTheData(){
	 ...
	 const checkboxes = document.querySelectorAll("input[type='checkbox'][name='strCheckALL']");
	    ...
	    $('input[name=strCheckALL]').each(function (i) {
		 ...
		 document.forms[0].hmode.value="saveForWebServicesSent";						  
	     document.forms[0].submit();
		}
...
<center>
		...
		<legend class='legendHeader' id='nonPrintableLegend'>Path Care Lab API Sent</legend>
		...
		<button type="button" id="savebutton" class="btn btn-outline-success mt-1 btn-circle savebtn" tabindex='2' onClick="return saveTheData();" 
    <logic:equal name="mcPathLabSampleHandoverDetailFB" property="strAPIStatus" value="0">
        style="display: none;"
    </logic:equal>>
    <div class="popupToast"><i class="fa fa-save iround" title="Save"></i> <span class="popuptextToast">Save</span></div>
</button>
		...
		<div class="prescriptionTile" style="display:none;">
			...
			<div class="row rowFlex reFlex" style="margin-top: 10px;">
				...
				<div class="row rowFlex reFlex" style="margin-top: 10px;">
					...
					<div class="col-sm-2" style="color:black;">Desk</div>
					<div class="col-sm-2" style="margin-right: 100px">
					<select class="form-control" onchange="getPatDtls();" name="strDeskCombo" id="strDeskCombo">
										  <bean:write name="mcPathLabSampleHandoverDetailFB" property="strComboDrDesk" filter="false"/>	
										</select></div>
					
					
					<div class="col-sm-2" style="color:black;">API</div>
					<div class="col-sm-2"><select class="form-control" name="strAPIStatus" id="apiCombo" onchange="getPatDtls();" value="${mcPathLabSampleHandoverDetailFB.strAPIStatus}">
										  <bean:write name="mcPathLabSampleHandoverDetailFB" property="strComboAPISent" filter="false"/>	
										</select></div>
				...
				<legend class='legendHeader' id='nonPrintableLegend' style="font-size: 1.2rem;color:black;">Patient List (Pending)</legend>
					<table class="table" style="margin-top: 10px;">
						<thead><tr>
							<th style="text-align:center;width:10%;color:black;">Check All 
							
							<logic:equal name="mcPathLabSampleHandoverDetailFB" property="strAPIStatus" value="0">	
							<input type="checkbox" disabled id="selectAllCheckbox">
							</logic:equal>
							
							<logic:notEqual name="mcPathLabSampleHandoverDetailFB" property="strAPIStatus" value="0">
								<input type="checkbox" id="selectAllCheckbox">
							</logic:notEqual>
							
							</th><th style="text-align:center;width:10%;color:black;">CR Number
							</th><th style="text-align:center;width:15%;color:black;">Patient Name
							</th><th style="text-align:center;width:10%;color:black;">Mobile No.
							</th><th style="text-align:center;width:20%;color:black;">Requisition No.
							</th><th style="text-align:left;width:20%;color:black;">Test Names
							</th><th style="text-align:left;width:20%;color:black;">Error Msg
...
<!-- 				</div> -->
			...
			<fieldset>				
				...
				<div class="col-sm-2" style="color:black;"> Handover To :</div>
				...
				<div class="col-sm-2" style="color:black;"> Handover Person's Mobile No. :</div>
				...
				<div class="col-sm-2" style="color:black;"> Remarks :</div>
