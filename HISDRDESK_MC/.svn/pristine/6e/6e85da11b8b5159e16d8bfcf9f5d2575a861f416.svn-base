...
var data = JSON.stringify(ViewTemplateData);
	...
	$.ajax({url:'/HISDRDESK_MC/services/restful/Template/getHtml/',type:'POST',data:{JsonData:data},success:function(result)
    	{
		...
		/*if(localStorage.getItem("FormattedHTML1"))
			...
			$('#printPrescriptionModal .modal-body').prepend('<iframe id="printPrescFrameId" style="width:100%;height:80vh;" src="/HISDRDESK_MC/new_opd/transaction/OPDTemplateMstAction.cnt?hmode=PRINTPRESCVIEW"></iframe>');
			$('#printPrescriptionModal').modal('show');
