...
/**
 * 
 * Global Class for all kind of HIS_MC Utilities. The Class contains Various
 * utility methods which are used in various part of HIS_MC Project.
 * 
...
public static String getParameterFromHisPathXML(String _Param){
	...
	*/
	...
	public static String getDatePickerMultiRowTLD(String fieldName, String dateValue, boolean readOnly,int i)
    {
	 ...
	 /**
	 * validate the file extension as per the provided extension
	 * 
	 * @param strFileExt
	 * @return
	 * @throws Exception
	 * Added by Vasu on 8.Aug.2018 for file upload using Tika
	 */
	/*public static void validateFileContentType(String strFileExt,
			File myFile) throws Exception {

		Tika tika;
		tika = new Tika();
										
		//if(ext==".PNG" || ext==".JPG" || ext==".JPEG" || ext==".GIF" || ext==".PDF" || ext==".DOC" || ext==".DOCX"|| ext==".RTF")
		String filetypea = tika.detect(myFile);
		System.out.println("Tika File Type-->>>"+filetypea);
		
		if (strFileExt.equalsIgnoreCase("pdf")) {

			// detecting the file type using detect method
			String filetype = tika.detect(myFile);
			// System.out.println("Tika File Type-->>>"+filetype);
			if (!filetype.equals("application/pdf")) {
				throw new Exception("File Content Type [  " + filetype
						+ " ] is Not Match With File Ext ::" + strFileExt);
			}
		...
		else if (strFileExt.equalsIgnoreCase("GIF")) {
		...
		else if (strFileExt.equalsIgnoreCase("rtf")) {
			//tika = new Tika();
			// detecting the file type using detect method
			String filetype = tika.detect(myFile);
			// System.out.println("Tika File Type-->>>"+filetype);
