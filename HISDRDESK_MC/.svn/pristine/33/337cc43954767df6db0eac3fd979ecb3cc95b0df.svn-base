...
<body id="printPrescFrameBody" onload="">
           ...
           <%HospitalMstVO objHospitalMstVO =ControllerUTIL.getHospitalVO(request);%>
             <div class="row">
             	<table class="table table-condensed printPrescPatDtlTbl">
					...
					<th style='width:33%;text-align: center;'><img alt="" src="/HIS/hisglobal/images/e-clinic/pb_logo.png" style='width: 30%;'></th>
					<th style='width:33%'><div id="patQrCode" class="pull-right"></div> </th>
					</tr>
						<tr style="border-bottom: 2px solid;">
							<th colspan="3" style="text-align: center;" >
							<div>
						 	<h4 class="" style="font-weight:bold; margin-left:-20px"><%=objHospitalMstVO.getHospitalName().toUpperCase() %> </h4> 
								
							<%if(objHospitalMstVO.getAddress1()!=null && !objHospitalMstVO.getAddress1().trim().equals("") ){ %>
								<h5 style="font-weight:bold; margin-left:-25px" ><%=objHospitalMstVO.getAddress1() %></h5>
							<%} %>
							<%if(objHospitalMstVO.getAddress2()!=null && !objHospitalMstVO.getAddress2().trim().equals("") ){ %>
								<h5 class="text-center" style="font-weight:bold; margin-left:-25px"><%=objHospitalMstVO.getAddress1() %></h5>
							<%} %>
							<%if(objHospitalMstVO.getCity()!=null && !objHospitalMstVO.getCity().trim().equals("") ){ %>
								<h5 class="text-center" style="font-weight:bold"><%=objHospitalMstVO.getCity() %></h5>
							<%} %>
							<%if(objHospitalMstVO.getDistrictName()!=null && !objHospitalMstVO.getDistrictName().trim().equals("") ){ %>
								<h5 class="text-center" style="font-weight:bold"><%=", "+ objHospitalMstVO.getDistrictName() %></h5>
							<%} %>
							<%if(objHospitalMstVO.getPinCode()!=null && !objHospitalMstVO.getPinCode().trim().equals("")  && !objHospitalMstVO.getPinCode().trim().equals("0") ){ %>
								<h5 class="text-center" style="font-weight:bold"><%="- "+ objHospitalMstVO.getPinCode() %></h5>
							<%} %>
							
														
							<%if(objHospitalMstVO.getStateName()!=null && !objHospitalMstVO.getStateName().trim().equals("") ){ %>
								<h5 class="text-center" style="font-weight:bold"><%=", "+ objHospitalMstVO.getStateName() %></h5>
							<%} %>
			...
			<div class="row divPrintPrescPatDtlTbl" id="divPrintPrescPatDtlTblId"  style=" border-bottom: 1px solid grey;">
				<table class="table table-condensed printPrescPatDtlTbl">
					<tbody>
					<tr>
						 <th colspan="5">
						...
						<tr>
							<th>Name</th><td class="patName"></td><th>BEN ID.</th><td class="patCrNo"></td>
						</tr>
						<tr>
	...
	$(document).ready(function(){
	  ...
	  if(localStorage.getItem("flag")=="0")
	  {
		...
		//alert("inside if===" + myJSON);
	  }
	  else
	  {
		  $('.prescSaveBtn').hide();
		  var ajxRqstPatDtl = localStorage.getItem("ajxRqstPatDtl").split('#');
		//  alert('/HISDRDESK_MC/services/restful/patdata/getPatData?Modval=1&CrNo='+ajxRqstPatDtl[0]+'&episodeCode='+ajxRqstPatDtl[1]+'&visitNo='+ajxRqstPatDtl[2]+'&seatId=&Entrydate='+ajxRqstPatDtl[4]+'&hosp_code='+ajxRqstPatDtl[5]);
