...
{ 
	populateteplatesInSession();
	///alert("OpenTemplate>>>");
	//console.log('Open Template id >'+tempId);
	var tempId=$(strTemplateTypeId).attr("data-value");
	if(tempId==undefined ){
		alert("uname to open clinical template");
		return;
	}
	...
	if(visitNo=="")
		visitNo="0";
	console.log(patGeneralDtl[3]);
	console.log(patGeneralDtl[3].split('/').length);
	if(patGeneralDtl[3].split('/').length == 3)
		patGeneralDtl[3]+="/0";
	console.log(patGeneralDtl[3]);
	...
	var today = new Date();
	var strDate = ('0'+today.getDate()).slice(-2)+'_'+months[today.getMonth()+1]+'_'+today.getFullYear();
	...
	var vitalJSON = { 
			...
			"patQueueNo"			:patGeneralDtl[3].split('/')[3]==""?"0":patGeneralDtl[3].split('/')[3] ,
			 "episodeVisitNo"		:visitNo,
	...
	//alert('/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt?hmode=TEMPLATESAVE&strTemplatetype='+(tempId).split('#')[0]+'&strtype='+(tempId).split('#')[1])
	...
	$('#OpenTemplateModal .modal-body').prepend('<iframe id="OpenTemplateModalFrameId" style="width:100%;height:80vh;" src="/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt?hmode=TEMPLATESAVE&strTemplatetype='+(tempId).split('#')[0]+'&strtype='+(tempId).split('#')[1]+'"></iframe>');
	$('#OpenTemplateModal').modal('show');
	$("#OpenTemplateModal").find(".modal-backdrop").css({"z-index": "-1"});	

	/*if(highRiskPregnancy!='-11'){
		setTimeout(function() {
			
			console.log("inside highRiskPregnancy");
			
			$("#templateheader_-11").css("display","none");
			$('#templateheader_-11').hide()
			debugger;
			window.parent.$('#templateheader_-11').hide();
...
function saveHtmlData(templateid,id){
	...
	if(localStorage.getItem("tempName")){
		oldItems += localStorage.getItem("tempName");
		oldItems +=', '+tempName;
		//console.log("tempName oldItems  "+oldItems);
		localStorage.setItem("tempName", oldItems);
	}
	else
		localStorage.setItem("tempName", tempName);
	
	console.log("tempName - "+localStorage.getItem("tempName"));
	...
	*/
	...
	//console.log(':::::::::::::');
	//console.log(JSON.stringify(parametreArr));
	// alert('data '+JSON.stringify(parametreArr));
	 var myJSON='';
	 if(localStorage.getItem("PatDtlsJsonData"))
	...
	console.log('Save Html Template'+ JSON.stringify(jsonForm));
	  var vitalJSON = { 
			 ...
			 // alert('data '+JSON.stringify(vitalJSON));
			  
	  		$('#OpenTemplateModal').modal('hide');
	  		$('.loader', parent.document).css("display","");
	$.ajax({url:'/HISDRDESK_MC/services/restful/Template/Save/',type:'POST',data:{JsonData:data},success:function(result)
    	{
...
/*function printDiv(varid) { 
    ...
    a.document.write('<link rel="stylesheet" type="text/css" href="/HISDRDESK_MC/new_opd/css/opdtemplate.css">');
    a.document.write('<link rel="stylesheet" type="text/css" href="/HISDRDESK_MC/new_opd/css/fb.css">');
    ...
    a.document.write('<script src="/HISDRDESK_MC/new_opd/js/fb.js"></script>');
...
function printDiv(varid ,tempName){
	...
	let myJSON='';
	 ...
	 myJSON = localStorage.getItem("PatDtlsJsonData");
			...
			console.log("column::"+$(b).prop("id"));
				...
				if($(b).find(".mainElDiv").length>0)
				{
					...
					//$(b).find('.mainElDiv').next().find('img').prop('src')
							...
							let vall="";
								if($(b).find(".mainElDiv .labelElDiv").css("display")!="none" && ($(b).find(".mainElDiv .addedEl").val()!="" || $(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="MultiAc" || $(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="Autocomplete")){
								 vall="<td "+align+"><table><tr>";
									...
									if($(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="Unit_Based"){
										...
										($(b).find(".mainElDiv .addedEl .input-group-append button").text()=="Select"?"":$(b).find(".mainElDiv .addedEl .input-group-append button").text());
									}
									else if($(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="Autocomplete"){
										vall+=$(b).find(".autoCompHelper").val()==undefined?"":$(b).find(".autoCompHelper").val().split("_attach_")[1];	
									}
				...
				if($(b).find(".addedElGrp").length>0){
					...
					if( $(b).find(".addedElGrp").prop("id").split("-")[0]=="checkbox"){	
						...
						if($(b).find(".addedElGrp").find(".mainLabel").length>0)
							...
							if($(b).find(".addedElGrp input[name^='optcheck-']:checked").val()!="" && $(b).find(".addedElGrp input[name^='optcheck-']:checked").val()!=undefined)
								printData+=chkGrpLabel(b)
							}
					...
					//$(b).find('.mainElDiv').next().find('img').prop('src')
							...
							console.log("with label");
								...
								//	if($(b).find(".mainElDiv .labelElDiv label font").css("display")=="none")
										...
										if( $(b).find(".mainElDiv .addedEl").val()=="" ){
											if(($(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="MultiAc" && $(b).find(".MultiAcBadge span").length>0 ))
												vall+="<td "+align+"><b>"+$(b).find(".mainElDiv .labelElDiv label").text()+"</b></td>";
											if(($(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]=="Unit_Based" && ($(b).find("button").text()!="Select" || $(b).find(".mainElDiv .addedEl input").val()!="")))
												vall+="<td "+align+"><b>"+$(b).find(".mainElDiv .labelElDiv label").text()+"</b></td>";
											else
												vall+="<td "+align+"></td>";
										}
										else{
											if($(b).find(".mainElDiv .addedEl").val()==undefined)
												vall+="<td "+align+"></td>";
											else
												vall+="<td "+align+"><b>"+$(b).find(".mainElDiv .labelElDiv label").text()+"</b></td>";
								...
								vall+="</td></tr></table>";								
									...
									console.log($(b).find(".mainElDiv .addedEl").prop("id").split("-")[0]);
									if( $(b).find(".mainElDiv").find(".addedEl").prop("id").split("-")[0]=="Image" ){
										if(!($(b).find('.mainElDiv').next().find('img').prop('src').startsWith("http:"))){
											vall+="<img  height='"+(250/parseInt(curnt))+"' width='"+(515/parseInt(curnt)-8)+"' src='imgTmp_"+l+"'>"
					...
					if($(b).find(".addedElGrp").length>0){
						...
						if( $(b).find(".addedElGrp").prop("id").split("-")[0]=="radio"){
							if($(b).find(".addedElGrp").find(".mainLabel").length>0)
							{
								...
								if($(b).find(".addedElGrp input[name^='optradio-']:checked").val()!="" && $(b).find(".addedElGrp input[name^='optradio-']:checked").val()!=undefined)
									printData+=chkGrpLabel(b);
							}
		...
		$.ajax({url:'/HISDRDESK_MC/services/restful/Template/printTemplate/',type:'POST',data:{printData:printData ,myJSON : JSON.stringify(myJSON) , varid : varid,imgs:JSON.stringify(imgs)},success:function(result)
	    	{
			console.log("----------------------");
...
{ 
	...
	$.ajax({url:'/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt?hmode=populateteplatesInSession',
		...
		error: function (xhr, ajaxOptions, thrownError) {
	        console.log("Problem in loading template. Check FTP connection");
	      }
	});
