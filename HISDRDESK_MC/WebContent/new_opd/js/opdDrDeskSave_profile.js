...
{
	var deptunit = document.getElementById("deptUnitName");
	console.log('deptUnitName saveFromIframe ---- '+ deptUnitName);
	...
	$.ajax({url:'/HISDRDESK_MC/services/restful/DrDesk/saveData1/',type:'POST',data:{JsonData:data ,FormattedJsonDataArray:FormattedData},success:function(result)
    	{
...
function SavePatientReffralData(data,e)
{
	...
	$('.visitedPatCount').text(saveCount);
	var patData = JSON.parse(data);
	$.ajax({url:'/HISDRDESK_MC/services/restful/DrDesk/saveData1/',type:'POST',data:{JsonData:data},success:function(result)
    	{
		console.log('::::saved successfully:::');
		$('.patientListBlock:contains('+patData.CR_No+')').removeClass('isAttended_1').addClass('isAttended_2'); 
		$('.patientListBlock:contains('+patData.CR_No+') td').eq(0).find('i').remove(); 
...
{   
	var deptunit = document.getElementById("deptUnitNamePB").value;
	console.log('PB deptUnit  saveFromIframe >>>  '+ deptunit);
	...
	$.ajax({url:'/HISDRDESK_MC/services/restful/DrDesk/saveData1/',type:'POST',data:{JsonData:data ,FormattedJsonDataArray:FormattedData,DeptUnitName:deptunit},success:function(result)
    	{
		console.log('::::saved successfully:::');
		console.log(JSON.stringify(result)); 
		$('.patientListBlock:contains('+patData.CR_No+')').removeClass('isAttended_1').addClass('isAttended_2'); 
...
function getPrevData(CR_No,episodeCode,hospitalCode,lastVisitDate ,visitNo , eTeleconsultancyReqId)
{
	...
	$.ajax({url:'/HISDRDESK_MC/new_opd/transaction/DoctorDeskAction.cnt?hmode=GETPREV&PUK='+CR_No+'&EPISODECODE='+episodeCode+'&HOSPITALCODE='+hospitalCode+'&visitNo='+visitNo+'&eTeleconsultancyReqId='+eTeleconsultancyReqId +' ',type:'GET',async: true,success:function(result)
    	{
		//console.log(url);
			...
			if(result.DRUGDATA.length!=null)
			{
				...
				result.DRUGDATA.forEach(function(item,index){   
					if(index<3)   // rest of items will be hidden with 'more...' link
					{
						...
						var chckDrugVal = item.HSTSTR_ITEM_NAME+'&&'+item.HSTNUM_ITEM_ID+'#'+item.HSTNUM_ITEMTYPE_ID+'#'+'0'+'#'+'N'+'#'+item.HSTNUM_ITEMBRAND_ID+'&&'+item.HGSTR_DOSE_NAME+'&&'+item.HGNUM_DOSE_ID+'&&'+strfrequnctName+'&&'+item.HGNUM_FREQUENCY_ID+'&&'+item.HRGDT_START_DATE+'&&'+item.HGNUM_DAYS+'#'+item.HGNUM_DOSE_QTY+'&&'+item.HRGSTR_REMARKS;
			...
			if(result.VISTREASON!=null)
			{
				...
				result.VISTREASON.forEach(function(item){ 
					...
					for(var q=0 ; q< item.HRSTR_JSON_DATA.strReferal.length;q++)
					{
						...
						$('.refferalAdded').append('<label><button tabindex="0" style="padding-left: 5px;font-weight:700;" type="button"  data-toggle="tooltip" title='+ tooltipdata +'  class="value btn btn-xs '+datashow.trim()+'">'+
