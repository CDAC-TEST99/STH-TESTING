...
* Created by Pankaj Kumar (CE I) Guided By Ajay Kumar Gupta (SPE) HIS_MC CDAC Noida
...
function multiRow(_objTableId, _strArr, _nNoOfRow, _nMinRowMustExist, _fDeleteButtonRequired){
	...
	if(_fDeleteButtonRequired){
		if(_nNoOfRow>1){
			...
			objCol.innerHTML="<img name='"+_objTableId+"plusForDelete' onkeypress='onPressingEnter(this,event)' tabIndex=1 style='cursor: pointer;' src='/HIS/hisglobal/images/avai/minus.gif' title='Delete Row' onclick='deleteRowFromTable(\""+ _objTableId +"\", this, "+ _nNoOfRow +","+_nMinRowMustExist+",\""+strIds+"\")'>";
			if(nCurrentRowNo%2==0)
				objCol.className="multiControl1"
			else
				objCol.className="multiControl"		} else {
			var objCol = objRows[0].insertCell(_strArr.length);
			objCol.innerHTML="<img name='"+_objTableId+"plusForDelete' onkeypress='onPressingEnter(this,event)' tabIndex=1 src='/HIS/hisglobal/images/avai/minus.gif' style='cursor: pointer;' title='Delete Row' onclick='deleteRowFromTable(\""+ _objTableId +"\", this, 1,"+_nMinRowMustExist+")'>";
			if(nCurrentRowNo%2==0)
				objCol.className="multiControl1";
			else
				objCol.className="multiControl";
