...
function getDeltList(e)
 {
	...
	document.forms[0].strHiddenWardCode.value=document.forms[0].wardName.value ;
...
$(document).ready(function(){
	$('#patSideListBtn').click(function(){
			...
			$('.patientListBlock').each(function(){
			    if($(this).hasClass('isAttended_18'))
				{
			    	waitingPatCount++;
...
$('[data-toggle="offcanvas"]').click(function () {
  ...
  $('.sidebar-nav li:not(:first-child)').click(function(){
	...
	function inTilePresc(e,patName,patCrNo,episodeCode,patGenAgeCat,patAdmNo,patRoomNo,patWard,patBed,patAdmDate,patCompleteGeneralDtl){  
			     ...
			     /* $(this).parent().parent().css('border-left','10px solid #91061D'); */
					...
					if(temp == 0){
			            ...
			            $(e).parent().parent().parent().find('.showPatientDtl').load('/HISDRDESK_MC/new_ipd/prescription.jsp', function(){
	...
	function showPrescription(e){  
			...
			/*localStorage.removeItem("lastThreeVisitJson_0");
			localStorage.removeItem("lastThreeVisitJson_1");
			localStorage.removeItem("lastThreeVisitJson_2");*/
			...
			*/
			...
			var patDeptName = $(e).closest('.patientListBlock').find('.patDeptUnit').text().split('/')[0];
			var patUnitName = $(e).closest('.patientListBlock').find('.patDeptUnit').text().split('/')[1];
			...
			$.ajax({url:'/HISDRDESK_MC/services/restful/patdata/getPatUmidData?Modval=9&CrNo='+patCrNo+'&hosp_code='+hospitalCode +'',
				...
				success:function(result){ 
					//console.log('getPatUmidData :  '+JSON.stringify(result.pat_details[0].UMID_DATA));
					if(result.status == "1"){
			...
			var patWard = $(e).closest('.patientListBlock').find('.patWard').text().split('/')[0];
			var patRoomNo = $(e).closest('.patientListBlock').find('.patWard').text().split('/')[1];
	        ...
	        { console.log('page mode presc'+prescMode);
				...
				$('.rightPanel').parent().append('<h2 id="rightPanelLoadMsgId" style="margin-top:20%; margin-left:5%"><i class="fa fa-spinner fa-spin"></i> Loading Patient Prescription....</h3>');
				 $.ajax({
					url: '/HISDRDESK_MC/new_ipd/prescription.jsp',
					async:true,
					success: function(result){
						$('.rightPanel .showPatientDtl').html(result);
							$('#patNamePrescriptionPanel').text(patName);
				      		$('#patCrNoPrescriptionPanel').text(patCrNo);
				      		$('#preparedBy').val($('input[name=strHiddenUsrName]').val());
				      		...
				      		$('#prescriptionPanelPatStatus').text($(e).closest('.patientListBlock').hasClass('isAttended_12')?'Admitted':'Discharge Approval').css('color',$(e).closest('.patientListBlock').hasClass('isAttended_12')?'#1bbf23':'red');
				      		$('#patGaurdianNamePrescriptionPanel').text(patGaurdianName);
				      		$('#patDeptName').text(patDeptName);
				      		$('#patUnitName').text(patUnitName);
				      		...
				      		$('input[name=eTeleconsultancyreq]').val(teleConsultancyRequestId);
				      		$('#isAdmittedHiddenValId').val($(e).closest('.patientListBlock').hasClass('isAttended_12')?'12':'18');		      		
				 ...
				 /* $('.rightPanel .showPatientDtl').load('/HISDRDESK_MC/new_opd/prescription.jsp', function(){
			      		$('#patNamePrescriptionPanel').text(patName);
			      		$('#patCrNoPrescriptionPanel').text(patCrNo);
			      		$('#patSummaryTileImg').attr('src',$(e).closest('.patientListBlock').find('.patProfileImg').attr('src'));
			      		$('#patEpisodeVisitNoPrescriptionPanel').text(patEpisodeVisitNo);
	        ...
	        { console.log('modal mode presc'+prescMode);
				...
				$('.prescModalBody').append(str);
	      		$('.prescModalBody .showPatientDtl').load('/HISDRDESK_MC/new_ipd/prescription.jsp', function(){
			      		$('#patNamePrescriptionPanel').text(patName);
			      		$('#patCrNoPrescriptionPanel').text(patCrNo);
			      		$('#patGenAgeCatPrescriptionPanel').text(patGenAgeCat);
			      		$('#patEpisodeCodePrescriptionPanel').text(episodeCode);
...
function getMotherInfo(e,admNo,isNewBornFlag){
	var hospCode = $(e).closest('.patientListBlock').find('.patHospitalCode').text();
	console.log("hospCode=="+hospCode+" admNo=="+admNo+" isNewBornFlag=="+isNewBornFlag);
		$.ajax({
	        type: "GET",
	        url: '/HISDRDESK_MC/services/restful/IpdDrDesk/getMotherAdmDtl?patAdmNo='+admNo+'&hospCode='+hospCode,
	        dataType:'json',
	        async: true,
	        success: function(item) {
	            //console.log(item);
	            if(item.status==1 && item.json_details!=null)
	         	   {
	             	   if(item.json_details.length>0)
	             	   {
	             		  if(isNewBornFlag=='0')
	             			  modalTitle = "Baby Details";
	             		  else
	             			  modalTitle = "Mother Details";
	             		  
	             		  $('#headerTitle').text('');
	             		  $('#headerTitle').append(modalTitle);
	             		  // console.log(JSON.stringify(item.json_details));
	             		  
	             		  item.json_details.forEach(function(data){
			             		 if(data.PATCRNO!=null && data.PATCRNO!='')
			             			$('#patMotherBabyCrNo').html("<b>CR No : </b>"+data.PATCRNO);
			             		 
			             		if(data.PATNAME!=null && data.PATNAME!='')
			             			$('#patMotherBabyName').html("<b>Name : </b>"+data.PATNAME);
			             		
			             		if(data.PATAGE!=null && data.PATAGE!='' && data.PATGENDER!=null && data.PATGENDER!='')
			             			$('#patMotherBabyGenderAge').html("<b>Gender/Age : </b>"+data.PATGENDER+'/'+data.PATAGE);
			             		
			             		if(data.PATDOB!=null && data.PATDOB!='')
			             			$('#patMotherBabyDob').html("<b>DOB  : </b>"+data.PATDOB);
			             		
			             		if(data.PATADMNO!=null && data.PATADMNO!='')
			             			$('#patMotherBabyAdmNo').html("<b>Adm. No : </b>"+data.PATADMNO);
			             		
			             		if(data.ADMDATETIME!=null && data.ADMDATETIME!='')
			             			$('#patMotherBabyAdmOn').html("<b>Admitted On : </b>"+data.ADMDATETIME);
			             		
			             		if(data.DEPARTMENTNAME!=null && data.DEPARTMENTNAME!='')
			             			$('#patMotherBabyDept').html("<b>Department : </b>"+data.DEPARTMENTNAME);
			             		
			             		if(data.DEPARTMENTUNITNAME!=null && data.DEPARTMENTUNITNAME!='')
			             			$('#patMotherBabyUnit').html("<b>Unit : </b>"+data.DEPARTMENTUNITNAME);
			             		
			             		if(data.PATCAT!=null && data.PATCAT!='')
			             			$('#patMotherBabyCat').html("<b>Category : </b>"+data.PATCAT);
			             		var status ='';
			             		var statuscode = data.ADMISSIONSTATUSCODE;
			             		if(statuscode!=null && statuscode=="12")
			             		{
			             			status = "Admitted";
			             		}
			             		else if(statuscode!=null && statuscode=="16")
			             		{
			             			status = "Discharged";
			             		}
			             		else if(statuscode!=null && statuscode=="18")
			             		{
			             			status = "Discharge Request Generated";
			             		}
			             		else
			             		{
			             			status = "Not Accepted";
			             		...
			             		if(data.PATCRNO!=null && data.PATCRNO!='')
			             			$('#patMotherBabyStatus').html("<b>Status : </b>"+status);
			             		
			             		if(data.WARDNAME!=null && data.WARDNAME!='')
			             			$('#patMotherBabyWard').html("<b>Ward : </b>"+data.WARDNAME);
			             		
			             		if(data.IPDROOMNAME!=null && data.IPDROOMNAME!='')
			             			$('#patMotherBabyRoom').html("<b>Room : </b>"+data.IPDROOMNAME);
			             		
			             		if(data.BEDNAME!=null && data.BEDNAME!='')
			             			$('#patMotherBabyBed').html("<b>Bed : </b>"+data.BEDNAME);
			             		
			             		if(data.DISDATETIME!=null && data.DISDATETIME!='')
			             			if(statuscode!=null && statuscode=="16")
			             				$('#patMotherBabyDisDt').html("<b>Discharge Date & Time : </b>"+data.DISDATETIME);
	             		 });
	             	   }
	             	  else
	        		   {
	             		 // $('#drugRoute').append($('<option>', {value: '',
							// text: 'Select'}));
	        		   }
	         	   }
	        },
	        error: function (f) {
	            alert('Error Received: ' + f);
	          },
	    });
		$('#myModal_motherBabyLinkage').modal('show');
	}
