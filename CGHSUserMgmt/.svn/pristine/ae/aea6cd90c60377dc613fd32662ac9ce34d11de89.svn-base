/*listPageConfigJson={
   "containerId":"LISTPAGE",
	"listPageHeading": "Role Acceptance",
	"listServiceName": "/getData/getSeatAcceptanceList",
...
function populateTransferDetails(hospitalId) {
    const configJson = {
        serviceName: "/getData/getTransferDetailsByHospital",
    ...
    if (dataJson.data && dataJson.data.length > 0) {
        ...
        dataJson.data.forEach((row, index) => {
            ...
            if (row.status === 'Pending') {
                actionHtml = '<input type="radio" id="radioid_'+index+'" onclick="checkRadio(' + `'${orderNo}'` + ');" />';
...
function saveTransfer() {
    try {
        ...
        var config = {
            serviceName: "updateTransferRequest",
            inputData: inputData,
...
function backToListPage(){
	...
	$('#masterKey').val("TransferAcceptance");
	submitFormWithResetTextField("CallMasterPage");
}


function checkRadio(orderId){
